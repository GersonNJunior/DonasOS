const App=(()=>{
  ...
  function resumoHoje(){const pagos=pedidosPagos().filter(p=>(p.dataPagamento||p.data||'').slice(0,10)===hoje()),movs=movimentosFinanceiros().filter(m=>(m.data||'').slice(0,10)===hoje()),entradas=movs.filter(m=>m.tipo==='entrada').reduce((a,m)=>a+Number(m.valor||0),0),saidas=movs.filter(m=>m.tipo==='saida').reduce((a,m)=>a+Number(m.valor||0),0),lucro=pagos.reduce((a,p)=>a+Number(p.lucro||0),0),porPg={};pagos.forEach(p=>porPg[p.pagamento||'Não informado']=(porPg[p.pagamento||'Não informado']||0)+Number(p.total||0));return{pagos,entradas,saidas,lucro,porPg}}
