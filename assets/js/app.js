const App=(()=>{
  ...
  function limparPortalCliente(){portalPedido={itens:{},qtd:{}};portalPratos=[];portalClienteIdentificado=null;['portalCliente','portalTelefone','portalTelefoneFinal','portalCep','portalRua','portalNumero','portalComplemento','portalEndereco','portalBairro','portalCidade','portalUf','portalObs'].forEach(id=>{const x=el(id);if(x)x.value=''});if(el('portalConfirmaEndereco'))el('portalConfirmaEndereco').checked=false;if(el('portalClienteStatus')){el('portalClienteStatus').className='portal-welcome muted';el('portalClienteStatus').innerHTML='Informe o telefone para buscar cadastro, histórico e último pedido.'}if(el('portalTipo'))el('portalTipo').value='Retirada';if(el('portalPagamento'))el('portalPagamento').value='PIX';if(el('portalTalher'))el('portalTalher').value='Não';if(el('portalWhatsapp'))el('portalWhatsapp').textContent='Nenhum pedido externo criado ainda.';renderPortalCliente();toast('Portal limpo')}
