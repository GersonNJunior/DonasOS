const App=(()=>{
  ...
  const OPERACAO_PAGES=['pedidos','cozinha','pdv','estoque','online'];
  ...
  function limparSelecaoPrato(){pedido.itens={};pedido.qtd={};if(el('compraData')&&!el('compraData').value)el('compraData').value=hoje();if(el('insightDataIni')&&!el('insightDataIni').value)el('insightDataIni').value=hojeLocal();if(el('insightDataFim')&&!el('insightDataFim').value)el('insightDataFim').value=hojeLocal();renderAll();aplicarModoOperacao()}
  ...
  async function finalizarPedido(){syncPedido();let pratos=[...pedido.pratos];const atual=currentPrato();if(atual.itens.length){if(!validarPrato(selectedItems()))return;pratos.push(atual)}if(!pratos.length)return toast('Adicione ao menos um prato ao pedido');const todos=pratos.flatMap(p=>p.itens);if(!await validarEstoqueItensAtualizado(todos))return;const subtotal=pratos.reduce((a,p)=>a+p.total,0);const entrega=pedido.tipo==='Delivery'?Number(db.config.taxaEntrega||0):0;const bruto=subtotal+entrega;if(pedido.cupom&&supabaseLojaId)await supabaseCarregarCupons();const cup=usarCupomSeValido(pedido.cupom,bruto);if(pedido.cupom&&!cup.ok)return toast(cup.msg||'Cupom inválido');const desconto=Number(cup.desconto||0);const totalFinal=Math.max(0,bruto-desconto);const custo=pratos.reduce((a,p)=>a+p.custo,0);const numero=pedido.editandoNumero||await proximoNumeroPedidoSeguro();const pago=pedido.pagamentoRecebido===true;const agora=new Date().toISOString();const p={id:pedido.editandoId||uid(),onlineId:pedido.editandoOnlineId||('ADM-'+hoje().replace(/-/g,'')+'-'+numero),numero,cliente:pedido.cliente||'Cliente balcão',telefone:pedido.telefone,tipo:pedido.tipo,pagamento:pago?(pedido.pagamentoPrevisto||'PIX'):'',pagamentoPrevisto:pedido.pagamentoPrevisto||'PIX',pagamentoRecebido:pago,obs:pedido.obs,cep:pedido.cep,rua:pedido.rua,numeroEndereco:pedido.numero,complemento:pedido.complemento,endereco:pedido.endereco,bairro:pedido.bairro,cidade:pedido.cidade,uf:pedido.uf,clienteId:pedido.clienteId||'',pratos,itens:todos,total:totalFinal,subtotalBruto:bruto,desconto,cupom:cup.codigo||'',cupomId:cup.cupom?.id||'',custo,lucro:totalFinal-custo,status:'Pedido Feito',financeiroLancado:pago,dataPagamento:pago?agora:'',estoqueBaixado:true,origem:'interno',data:agora,timeline:[{status:pedido.editandoNumero?'Pedido editado':'Pedido Feito',data:agora, pagamentoRecebido:pago}]};db.pedidos.unshift(p);log(pedido.editandoNumero?'Pedido editado':'Pedido criado','#'+p.numero+' • '+fmt(p.total)+(desconto?' • cupom '+cup.codigo:'')+(pago?' • pago':' • pagamento pendente'));baixarEstoque(todos);editBuffer=null;salvarClienteAutomatico(p);limparPedido(false);save();const enviado=await supabaseEnviarPedido(p);if(enviado!==false&&cup.codigo&&!pedido.editandoNumero){const usoOk=await registrarUsoCupom(cup.codigo);if(!usoOk)toast('Pedido salvo, mas não consegui atualizar o uso do cupom. Confira no ADM.')}toast(pedido.editandoNumero?'Pedido editado e reenviado para produção.':(pago?'Pedido registrado e contabilizado no financeiro.':'Pedido registrado. Aguardando preparo e pagamento.'))}
  ...
  function pedidoCard(p){return `<div class="pedido-card"><div class="pedido-top"><strong>${pedidoNumeroLabel(p)}</strong><span class="pill origin-pill ${origemPedidoClass(p)}">${origemPedidoLabel(p)}</span><span class="status ${statusClass(p.status)}">${p.status}</span></div><small>${new Date(p.data).toLocaleString('pt-BR')} • ${p.tipo} • prev. ${p.pagamentoPrevisto||'PIX'}</small><p><b>${p.cliente}</b></p>${pedidoPratos(p)}<div class="pedido-bottom"><b>${fmt(p.total)}</b><div class="list-actions">${statusActions(p)}<button class="mini secondary" onclick="App.abrirEditarDadosPedido('${p.id}')">dados</button><button class="mini secondary" onclick="App.editarPedido('${p.id}')">editar</button><button class="mini secondary" onclick="App.verPedido('${p.id}')">detalhes</button></div></div></div>`}
  function renderKanban(){
    ...
    k.innerHTML=STATUS.map(st=>{
      const ps=db.pedidos.filter(p=>(p.status||'Pedido Feito')===st);
  ...
  async function supabaseAtualizarPedido(p){
    try{
      ...
      let rows=await supabaseRequest('/pedidos?id=eq.'+encodeURIComponent(sid)+'&select=id,codigo,status,pago,status_pagamento,pago_em,forma_pagamento,valor_total',{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify(payload)});
      if((!rows||!rows.length)&&codigo){
        rows=await supabaseRequest('/pedidos?codigo=eq.'+encodeURIComponent(codigo)+'&select=id,codigo,status,pago,status_pagamento,pago_em,forma_pagamento,valor_total',{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify(payload)});
      }
  ...
  function renderAll(){
    ...
    renderSelects();renderFavoritos();renderSteps();renderResumo();renderListas();renderDashboard();renderConfig();renderInsights();renderRelatorios();renderOnlineReady();renderPortalCliente();renderOperacao();renderCupons();renderAssistentePorcoes();aplicarPerfilAcesso();
    if(keepScroll!==null){
      const restore=()=>{
