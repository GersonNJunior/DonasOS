const App=(()=>{
  ...
  function movimentosFinanceiros(){
    ...
    (db.pedidos||[]).filter(p=>p.financeiroLancado||p.pagamentoRecebido===true).forEach(p=>{
      movs.push({id:'auto_pedido_'+(p.id||p.onlineId||p.numero),tipo:'entrada',desc:'Pedido #'+(p.numero||'')+' ('+(p.pagamento||p.pagamentoPrevisto||'PIX')+')',valor:Number(p.total||0),data:p.dataPagamento||p.data||new Date().toISOString(),origem:'pedido',pedidoId:p.id});
    });
  ...
  async function finalizarPedido(){
    ...
    if(editando){
      ...
      const mov=db.financeiro.find(m=>m.origem==='pedido'&&m.pedidoId===p.id);if(mov)mov.valor=p.total;
  ...
  function abrirPagamento(id,entregar=false){const p=db.pedidos.find(x=>x.id===id);if(!p)return;el('modalContent').innerHTML=`<div class="receipt"><h2>${entregar?'Finalizar':'Registrar pagamento do'} Pedido #${p.numero}</h2><p class="muted">Forma prevista: <b>${p.pagamentoPrevisto||'PIX'}</b>. A receita entra no financeiro quando o pagamento for confirmado.</p><div class="field"><label>Forma de pagamento recebida</label><select id="pagamentoFinal">${PAGAMENTOS.map(pg=>`<option ${pg===(p.pagamento||p.pagamentoPrevisto||'PIX')?'selected':''}>${pg}</option>`).join('')}</select></div><div class="summary-item"><strong>Total</strong><strong>${fmt(p.total)}</strong></div><button class="primary full" onclick="App.confirmarPagamento('${id}',${entregar?'true':'false'})">${entregar?'Confirmar pagamento':'Confirmar pagamento'}</button><button class="secondary full" onclick="App.fecharModal()">Cancelar</button></div>`;el('modal').classList.remove('hidden')}
  async function confirmarPagamento(id,entregar=true){const p=db.pedidos.find(x=>x.id===id);if(!p)return;const anterior=deepCopy(p);const pg=val('pagamentoFinal')||p.pagamentoPrevisto||'PIX';const agora=new Date().toISOString();if(entregar)p.status='Entregue';p.pagamento=pg;p.pagamentoRecebido=true;p.financeiroLancado=true;p.dataPagamento=p.dataPagamento||agora;p.timeline=p.timeline||[];p.timeline.push({status:entregar?'Entregue':'Pagamento recebido',data:agora,pagamento:pg});save();renderAll();const ok=await supabaseAtualizarPedido(p);if(!ok){Object.assign(p,anterior);save();renderAll();return toast('Não foi possível registrar o pagamento no banco. Tente novamente.')}await supabaseCarregarPedidos();renderAll();fecharModal();toast('Pagamento registrado no financeiro')}
