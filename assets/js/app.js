const App=(()=>{
  ...
  function seed(){return{version:'1.2.0-operacao',config:{nome:'Donas da Massa',nomeInterno:'Donas OS',slogan:'Seu macarrão. Seu jeito.',cnpj:'',email:'',enderecoEmpresa:'',instagram:'',pix:'',whatsapp:'',taxaEntrega:0,raioEntrega:'',tempoPreparo:20,complementosGratis:2,proteinasGratis:1,metaDiaria:1000,modoOperacao:false},itens:[
  ...
  async function removerItem(id){if(!confirm('Remover este item?'))return;const item=db.itens.find(i=>i.id===id);db.itens=db.itens.filter(i=>i.id!==id);save();await supabaseRemoverItem(item);toast('Item removido')}
  ...
  function mesclarItensSupabase(rows){
    ...
    const locais=(db.itens||[]).filter(i=>!i.supabase_id);
    const porNomeCat=new Map();
    [...remotos,...locais].forEach(i=>{const chave=(String(i.categoria||'')+'|'+String(i.nome||'')).toLowerCase();if(!porNomeCat.has(chave))porNomeCat.set(chave,i)});
  ...
  async function supabaseRemoverItem(i){
    ...
    catch(e){console.warn('Remover item Supabase:',e)}
  }
  ...
  function setPortalPorcao(id,delta){const it=db.itens.find(i=>i.id===id);if(!it)return;if(['Massa','Molho'].includes(it.categoria))return;const atual=porcoesPortalItem(it);const prox=Math.max(1,atual+Number(delta||0));portalPedido.qtd[id]=prox*tamanhoPorcaoEstoque(it);renderPortalCliente()}
  function setPortalQtd(id,v){const it=db.itens.find(i=>i.id===id);if(!it)return;if(['Massa','Molho'].includes(it.categoria)){portalPedido.qtd[id]=defaultQtd(it)}else{const porcoes=Math.max(1,Math.round(Number(v||1)));portalPedido.qtd[id]=porcoes*tamanhoPorcaoEstoque(it)}renderPortalCliente()}
