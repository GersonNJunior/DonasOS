const App=(()=>{
  ...
  function seed(){return{version:'4.0.2-estoque-supabase',config:{nome:'Donas da Massa',nomeInterno:'Donas OS',slogan:'Seu macarrão. Seu jeito.',cnpj:'',email:'',enderecoEmpresa:'',instagram:'',pix:'',whatsapp:'',taxaEntrega:0,raioEntrega:'',tempoPreparo:20,complementosGratis:2,proteinasGratis:1,metaDiaria:1000,modoOperacao:false},itens:[
  ...
  function novoPedido(){return{itens:{},qtd:{},pratos:[],cliente:'',telefone:'',tipo:'Balcão',pagamentoPrevisto:'PIX',obs:'',cep:'',rua:'',numero:'',complemento:'',endereco:'',bairro:'',cidade:'',uf:'',clienteId:'',editandoId:'',editandoNumero:''}}
  ...
  function syncPedido(){pedido.cliente=val('clientePedido');pedido.telefone=val('telefonePedido');pedido.tipo=val('tipoPedido')||'Balcão';pedido.pagamentoPrevisto=val('pagamentoPedido')||'PIX';pedido.pagamentoRecebido=val('pagamentoRecebidoPedido')==='sim';pedido.obs=val('obsPedido');pedido.cep=val('pedidoCep');pedido.rua=val('pedidoRua');pedido.numero=val('pedidoNumeroEndereco');pedido.complemento=val('pedidoComplemento');pedido.bairro=val('pedidoBairro')||val('bairroPedido');pedido.cidade=val('pedidoCidade');pedido.uf=val('pedidoUf');pedido.endereco=enderecoCompleto(pedido)||val('enderecoPedido')}
  ...
  function renderInsights(){
    ...
    const pedidosBase=pedidosPagos().filter(p=>p.supabase_id||p.origem==='supabase');
    const dataPedidoInsights=p=>dataLocalISO(p.dataPagamento||p.pagoEm||p.data);
    const pedidosHoje=pedidosBase.filter(p=>dataPedidoInsights(p)===today);
    const ontem=new Date();ontem.setDate(ontem.getDate()-1);const ontemStr=dataLocalISO(ontem);
  ...
  async function supabaseSalvarItem(i){
    if(!i)return;
    try{await supabaseGetLoja();const payload=payloadItemSupabase(i);const sid=i.supabase_id||(String(i.id||'').startsWith('sbit_')?String(i.id).replace('sbit_',''):null);if(sid){await supabaseRequest('/estoque_itens?id=eq.'+encodeURIComponent(sid),{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify(payload)});return;}const existente=await supabaseRequest('/estoque_itens?select=id&nome=eq.'+encodeURIComponent(i.nome||'')+'&categoria=eq.'+encodeURIComponent(i.categoria||'')+'&limit=1');if(existente?.[0]?.id){i.supabase_id=existente[0].id;await supabaseRequest('/estoque_itens?id=eq.'+encodeURIComponent(existente[0].id),{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify(payload)});}else{const criado=await supabaseRequest('/estoque_itens?select=id',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(payload)});if(criado?.[0]?.id)i.supabase_id=criado[0].id;}}
    catch(e){console.warn('Salvar item Supabase:',e);toast('Item salvo localmente, mas não sincronizou com Supabase.')}
  }
  ...
  async function supabaseAtualizarEstoqueItens(itens){
    try{await supabaseGetLoja();for(const i of (itens||[])){await supabaseSalvarItem(i)}}catch(e){console.warn('Atualizar estoque Supabase:',e)}
  }
  ...
  async function supabaseInicializar(){
    try{await supabaseGetLoja();await supabaseCarregarItens();if(!isPublicClient()){await supabaseSincronizarItensLocais();await supabaseCarregarCompras();await supabaseSincronizarComprasLocais();await supabaseCarregarReceitas();await supabaseSincronizarReceitasLocais();await supabaseCarregarProducoes();await supabaseSincronizarProducoesLocais();await supabaseCarregarMovimentacoes();await supabaseSincronizarMovimentacoesLocais();}await supabaseCarregarPedidos();setInterval(()=>{if(!isPublicClient()){supabaseCarregarItens();supabaseCarregarCompras();supabaseCarregarReceitas();supabaseCarregarProducoes();supabaseCarregarMovimentacoes();supabaseCarregarPedidos()}},8000);}catch(e){console.warn('Supabase init:',e)}
  }
  ...
  function limparPortalCliente(){if(!confirm('Deseja realmente limpar seu pedido?'))return;portalPedido={itens:{},qtd:{}};portalPratos=[];portalClienteIdentificado=null;['portalCliente','portalTelefone','portalTelefoneFinal','portalCep','portalRua','portalNumero','portalComplemento','portalEndereco','portalBairro','portalCidade','portalUf','portalObs'].forEach(id=>{const x=el(id);if(x)x.value=''});if(el('portalConfirmaEndereco'))el('portalConfirmaEndereco').checked=false;if(el('portalClienteStatus')){el('portalClienteStatus').className='portal-welcome muted';el('portalClienteStatus').innerHTML='Informe o telefone para localizar cadastro, histórico e último pedido.'}if(el('portalTipo'))el('portalTipo').value='Retirada';if(el('portalPagamento'))el('portalPagamento').value='PIX';if(el('portalPagamentoRecebido'))el('portalPagamentoRecebido').value='nao';if(el('portalTalher'))el('portalTalher').value='Não';if(el('portalWhatsapp'))el('portalWhatsapp').textContent='Nenhum pedido externo criado ainda.';renderPortalCliente();setTimeout(()=>scrollPortalTo('page-portal'),120);toast('Pedido limpo')}
  function mensagemWhatsPedido(p,itens,total){
    ...
    linhas.push('', `Total previsto: ${fmt(total)}`, `Pagamento previsto: ${p.pagamentoPrevisto||'A combinar'}`, `Pagamento: ${(p.financeiroLancado||p.pagamentoRecebido)?'já pago':'a pagar na entrega/retirada'}`, '', 'Aguardando confirmação.');
    return linhas.join('\n');
  }
  ...
  async function criarPedidoPortal(){
    ...
    const pago=val('portalPagamentoRecebido')==='sim';const agora=new Date().toISOString();const pagamentoPrevisto=val('portalPagamento')||'PIX';const p={id:uid(),onlineId:onlineId('PED'),numero,cliente,telefone,tipo:tipo==='Retirada'?'Balcão':tipo,pagamento:pago?pagamentoPrevisto:'',pagamentoPrevisto,talher,obs:val('portalObs'),cep:val('portalCep'),rua:val('portalRua'),numeroEndereco:val('portalNumero'),complemento:val('portalComplemento'),endereco:enderecoCompleto({rua:val('portalRua'),numero:val('portalNumero'),complemento:val('portalComplemento'),endereco:val('portalEndereco')}),bairro:val('portalBairro').trim(),cidade:val('portalCidade'),uf:val('portalUf'),clienteId:portalClienteIdentificado?.id||'',pratos:pratos.map((pr,idx)=>({...pr,nome:pr.nome||('Prato '+(idx+1))})),itens,total,custo,lucro:total-custo,status:'Aguardando confirmação',pagamentoRecebido:pago,financeiroLancado:pago,dataPagamento:pago?agora:'',estoqueBaixado:false,origem:'portal-local',data:agora,timeline:[{status:'Aguardando confirmação',data:agora,pagamentoRecebido:pago}],syncStatus:'local'};
