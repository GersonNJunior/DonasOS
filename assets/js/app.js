const App=(()=>{
  ...
  function novoPedido(){return{itens:{},qtd:{},pratos:[],pratoEditandoIndex:-1,cliente:'',telefone:'',tipo:'Balcão',atendimento:'imediato',reservaData:'',reservaHorario:'',pagamentoPrevisto:'PIX',obs:'',cep:'',rua:'',numero:'',complemento:'',endereco:'',bairro:'',cidade:'',uf:'',clienteId:'',editandoId:'',editandoNumero:'',cupom:'',cupomId:'',desconto:0,subtotalBruto:0}}
  ...
  function pedidoTotal(){syncPedido();const editIdx=Number(pedido.pratoEditandoIndex);let t=pedido.pratos.reduce((a,p,idx)=>a+(idx===editIdx?0:Number(p.total||0)),0);const atual=currentPrato();if(atual.itens.length)t+=atual.total;if(pedido.tipo==='Delivery')t+=freteAtual('pedido');return t}
  ...
  async function finalizarPedido(){
    ...
    const entrega=pedido.tipo==='Delivery'?freteAtual('pedido'):0;
    ...
    const p={
      ...
      pratos,itens:todos,total:totalFinal,subtotalBruto:bruto,desconto,cupom:cup.codigo||'',cupomId:cup.cupom?.id||'',custo,lucro:totalFinal-custo,
    ...
    editBuffer=null;salvarClienteAutomatico(p);limparPedido(false);save();
  ...
  function editarPedido(id){const p=db.pedidos.find(x=>x.id===id);if(!p)return;if(!confirm('Editar os itens do pedido #'+p.numero+'? Ele continuará na mesma posição da fila até você salvar.'))return;editBuffer={pedido:deepCopy(p)};pedido=novoPedido();pedido.pratos=deepCopy(p.pratos||[{nome:'Prato 1',itens:p.itens||[]}]);pedido.pratoEditandoIndex=-1;pedido.cliente=p.cliente||'';pedido.telefone=p.telefone||'';pedido.tipo=p.tipo||'Balcão';pedido.atendimento=p.atendimento||'imediato';pedido.reservaData=p.reservaData||'';pedido.reservaHorario=p.reservaHorario||'';pedido.pagamentoPrevisto=p.pagamentoPrevisto||p.pagamento||'PIX';pedido.trocoPara=Number(p.trocoPara||extrairTrocoObs(p.obs)||0);pedido.pagamentoRecebido=!!(p.financeiroLancado||p.pagamentoRecebido);pedido.obs=p.obs||'';pedido.cep=p.cep||'';pedido.rua=p.rua||'';pedido.numero=p.numeroEndereco||p.numero||'';pedido.complemento=p.complemento||'';pedido.endereco=p.endereco||'';pedido.bairro=p.bairro||'';pedido.cidade=p.cidade||'';pedido.uf=p.uf||'';pedido.clienteId=p.clienteId||'';pedido.editandoId=p.id;pedido.editandoNumero=p.numero;pedido.editandoOnlineId=p.onlineId||'';const mapEdit={clientePedido:'cliente',telefonePedido:'telefone',obsPedido:'obs',pedidoCep:'cep',pedidoRua:'rua',pedidoNumeroEndereco:'numero',pedidoComplemento:'complemento',enderecoPedido:'endereco',pedidoBairro:'bairro',bairroPedido:'bairro',pedidoCidade:'cidade',pedidoUf:'uf'};Object.entries(mapEdit).forEach(([id2,k])=>{if(el(id2))el(id2).value=pedido[k]||''});if(el('tipoPedido'))el('tipoPedido').value=pedido.tipo==='Delivery'?'Delivery':'Balcão';if(el('atendimentoPedido'))el('atendimentoPedido').value=pedido.atendimento;if(el('reservaDataPedido'))el('reservaDataPedido').value=pedido.reservaData;if(el('reservaHorarioPedido'))el('reservaHorarioPedido').value=pedido.reservaHorario;alternarCamposReserva();if(el('pagamentoPedido'))el('pagamentoPedido').value=pedido.pagamentoPrevisto;if(el('pedidoTrocoPara'))el('pedidoTrocoPara').value=pedido.trocoPara||'';alternarCamposTroco('pedido');if(el('pagamentoRecebidoPedido'))el('pagamentoRecebidoPedido').value=pedido.pagamentoRecebido?'sim':'nao';log('Edição iniciada','#'+p.numero);fecharModal();page('pdv');renderAll();toast('Pedido carregado completo. Ele permanece na fila até você salvar as alterações.')}
  ...
  async function supabaseAtualizarPedido(p){
    try{
      ...
      const payload={
        ...
        cidade:p.cidade||'',
        uf:p.uf||'',
        observacao:[reservaMarcador(p),p.obs||''].filter(Boolean).join(' '),
        pago,
  ...
  async function registrarAcessoLoja(){
    ...
    try{
      const response=await supabaseFetch('/'+ACCESS_TABLE,{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({session_id:accessSessionId(),created_at:new Date().toISOString()})});
  ...
  async function atualizarPainelAcessos(force=false){
    ...
    try{
      ...
      const rows=await supabaseFetch('/'+ACCESS_TABLE+'?select=created_at&created_at=gte.'+encodeURIComponent(desde)+'&order=created_at.desc&limit=10000');
