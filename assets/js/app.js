const App=(()=>{
  ...
  const CATS_ESTOQUE_NUMERICO=['Massa','Bebida'];
  ...
  function applyPrices(sel){
    const limites={Complemento:limiteGratisCategoria('Complemento'), 'Proteína':limiteGratisCategoria('Proteína'), 'Finalização':limiteGratisCategoria('Finalização')};
    ...
    return sel.map(i=>{
      ...
      const valorCobrado=(i.categoria==='Complemento'||i.categoria==='Proteína'||i.categoria==='Finalização')?Number(i.preco||0)*porcoesPagas:Number(i.preco||0)*mult;
  ...
  function currentPrato(){const itens=pricedItems();const idx=Number(pedido.pratoEditandoIndex);const nome=idx>=0&&pedido.pratos[idx]?pedido.pratos[idx].nome:'Prato '+(pedido.pratos.length+1);return{nome,itens,total:itens.reduce((a,i)=>a+Number(i.valorCobrado||0),0),custo:itens.reduce((a,i)=>a+Number(i.custoCalculado||0),0)}}
  ...
  function observacaoPersistidaPedido(p){return [reservaMarcador(p),freteGratisMarcador(p),p.obs||''].filter(Boolean).join(' ')}
  ...
  function editarPedido(id){const p=db.pedidos.find(x=>x.id===id);if(!p)return;if(!confirm('Editar os itens do pedido #'+p.numero+'? Ele continuará na mesma posição da fila até você salvar.'))return;editBuffer={pedido:deepCopy(p)};pedido=novoPedido();pedido.pratos=deepCopy(p.pratos||[{nome:'Prato 1',itens:p.itens||[]}]);pedido.pratoEditandoIndex=-1;pedido.cliente=p.cliente||'';pedido.telefone=p.telefone||'';pedido.tipo=p.tipo||'Balcão';pedido.atendimento=p.atendimento||'imediato';pedido.reservaData=p.reservaData||'';pedido.reservaHorario=p.reservaHorario||'';pedido.pagamentoPrevisto=p.pagamentoPrevisto||p.pagamento||'PIX';pedido.trocoPara=Number(p.trocoPara||extrairTrocoObs(p.obs)||0);pedido.pagamentoRecebido=!!(p.financeiroLancado||p.pagamentoRecebido);pedido.obs=p.obs||'';pedido.cep=p.cep||'';pedido.rua=p.rua||'';pedido.numero=p.numeroEndereco||p.numero||'';pedido.complemento=p.complemento||'';pedido.endereco=p.endereco||'';pedido.bairro=p.bairro||'';pedido.cidade=p.cidade||'';pedido.uf=p.uf||'';pedido.clienteId=p.clienteId||'';pedido.editandoId=p.id;pedido.editandoNumero=p.numero;pedido.editandoOnlineId=p.onlineId||'';pedido.freteGratis=!!p.freteGratis;pedido.freteOriginal=Number(p.freteOriginal||0);const mapEdit={clientePedido:'cliente',telefonePedido:'telefone',obsPedido:'obs',pedidoCep:'cep',pedidoRua:'rua',pedidoNumeroEndereco:'numero',pedidoComplemento:'complemento',enderecoPedido:'endereco',pedidoBairro:'bairro',bairroPedido:'bairro',pedidoCidade:'cidade',pedidoUf:'uf'};Object.entries(mapEdit).forEach(([id2,k])=>{if(el(id2))el(id2).value=pedido[k]||''});if(el('tipoPedido'))el('tipoPedido').value=pedido.tipo==='Delivery'?'Delivery':'Balcão';if(el('atendimentoPedido'))el('atendimentoPedido').value=pedido.atendimento;if(el('reservaDataPedido'))el('reservaDataPedido').value=pedido.reservaData;if(el('reservaHorarioPedido'))el('reservaHorarioPedido').value=pedido.reservaHorario;alternarCamposReserva();if(el('pagamentoPedido'))el('pagamentoPedido').value=pedido.pagamentoPrevisto;if(el('pedidoTrocoPara'))el('pedidoTrocoPara').value=pedido.trocoPara||'';alternarCamposTroco('pedido');if(el('pagamentoRecebidoPedido'))el('pagamentoRecebidoPedido').value=pedido.pagamentoRecebido?'sim':'nao';if(el('pedidoFreteGratis'))el('pedidoFreteGratis').checked=pedido.freteGratis;log('Edição iniciada','#'+p.numero);fecharModal();page('pdv');renderAll();toast('Pedido carregado completo. Ele permanece na fila até você salvar as alterações.')}
  ...
  async function supabaseAtualizarPedido(p){
    try{
      ...
      // A edição precisa substituir também os itens do pedido. Sem isso, o próximo refresh
      // trazia novamente os itens antigos e parecia desfazer a edição.
  ...
  function portalGratisLabel(cat){
    return labelGratisCategoria(cat);
  }
  ...
  function renderAll(){
    ...
    renderSelects();renderFavoritos();renderSteps();renderResumo();renderListas();renderReservados();renderDashboard();renderConfig();renderInsights();renderRelatorios();renderOnlineReady();renderPortalCliente();renderOperacao();renderCupons();renderOnlineCount();renderListaPratosFeitos();renderAssistentePorcoes();aplicarPerfilAcesso();
    if(keepScroll!==null){
      const restore=()=>{
