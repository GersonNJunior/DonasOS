const App=(()=>{
  ...
  function seed(){return{version:'1.2.0-operacao',config:{nome:'Donas da Massa',nomeInterno:'Donas OS',slogan:'Seu macarrão. Seu jeito.',cnpj:'',email:'',enderecoEmpresa:'',instagram:'',pix:'',whatsapp:'',taxaEntrega:0,raioEntrega:'',tempoPreparo:20,complementosGratis:2,proteinasGratis:1,finalizacoesGratis:1,metaDiaria:1000,modoOperacao:false,adminSenha:'donas2026'},itens:[
  ...
  function validarEstoqueItens(itens){
    ...
    for(const {item,uso} of agrupado.values()){
      if(Number(item.estoque||0)<uso){toast('Estoque insuficiente: '+item.nome);return false}
    }
    return true;
  }
  ...
  function setPorcaoItem(id,delta){const item=db.itens.find(i=>i.id===id);if(!item)return;if(['Massa','Molho','Finalização'].includes(item.categoria))return;const atual=porcoesPedidoItem(item);const prox=Math.max(1,atual+Number(delta||0));const consumo=prox*tamanhoPorcaoEstoque(item);if(consumo>Number(item.estoque||0))toast('Atenção: quantidade maior que o estoque atual');pedido.qtd[id]=consumo;renderResumo();renderProgress();renderSteps()}
  ...
  function baixarEstoque(itens){const alterados=[];(itens||[]).forEach(i=>{const item=encontrarItemEstoque(i);if(item){item.estoque=Number(item.estoque||0)-Number(i.qtdUsada||1);alterados.push(item)}});if(alterados.length)supabaseAtualizarEstoqueItens(alterados)}
  function devolverEstoque(itens){(itens||[]).forEach(i=>{const item=encontrarItemEstoque(i);if(item)item.estoque=Number(item.estoque||0)+Number(i.qtdUsada||1)})}
  ...
  async function confirmarExcluirPedido(id){const p=db.pedidos.find(x=>x.id===id);if(!p)return;const semRetorno=!!el('excluirSemRetorno')?.checked;const anterior=deepCopy({pedidos:db.pedidos,itens:db.itens,financeiro:db.financeiro});if(!semRetorno){(p.itens||[]).forEach(i=>{const item=db.itens.find(x=>x.id===i.id);if(item)item.estoque=Number(item.estoque||0)+Number(i.qtdUsada||1)})}db.financeiro=db.financeiro.filter(m=>m.pedidoId!==id);db.pedidos=db.pedidos.filter(x=>x.id!==id);save();renderAll();const ok=await supabaseExcluirPedido(p);if(!ok){db.pedidos=anterior.pedidos;db.itens=anterior.itens;db.financeiro=anterior.financeiro;save();renderAll();return toast('Exclusão desfeita porque não foi confirmada no banco.')}await supabaseCarregarPedidos();log('Pedido excluído','#'+p.numero+(semRetorno?' • sem retorno ao estoque':' • estoque devolvido'));save();fecharModal();toast(semRetorno?'Pedido excluído sem retorno de estoque':'Pedido excluído e estoque restaurado')}

  function escolherIcone(icon){if(icon&&el('itemIcone'))el('itemIcone').value=icon}
  async function salvarItem(){const id=val('itemId'),nome=val('itemNome').trim();if(!nome)return toast('Informe o nome');const snapshot=deepCopy(db.itens||[]);const data={nome,categoria:val('itemCategoria'),preco:num('itemPreco'),custo:num('itemCusto'),estoque:num('itemEstoque'),minimo:num('itemMinimo'),unidade:val('itemUnidade')||'un',porcao:num('itemPorcao')||1,porcaoUnidade:val('itemPorcaoUnidade')||val('itemUnidade')||'un',icone:val('itemIcone')||val('itemIconeLista')||iconeCat(val('itemCategoria')),imagem:val('itemImagem'),obs:val('itemObs'),ativo:true};if(id){data.custo=custoMedioComprasItem(id,data.custo)}let savedId=id;if(id){const idx=db.itens.findIndex(i=>i.id===id);if(idx>=0)db.itens[idx]={...db.itens[idx],...data,updatedAt:new Date().toISOString(),syncStatus:'local'}}else{savedId=uid();db.itens.push({id:savedId,...data,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),syncStatus:'local'})}const itemSalvo=db.itens.find(i=>i.id===savedId);const ok=await supabaseSalvarItem(itemSalvo);if(supabaseLojaId&&!ok){db.itens=snapshot;save();renderAll();return toast('Não salvei o item porque o Supabase recusou a alteração.')}save();await supabaseCarregarItens();limparFormItem(false);renderAll();setTimeout(()=>{const row=document.querySelector(`[data-item-id="${savedId}"]`);if(row){row.classList.add('editing-item');setTimeout(()=>row.classList.remove('editing-item'),1200)}},80);toast('Item salvo no banco')}
  function editarItem(id){const i=db.itens.find(x=>x.id===id);if(!i)return;el('formItemTitulo').textContent='✏️ Editando: '+i.nome;el('formItemTitulo').classList.add('editing-title');el('itemId').value=i.id;el('itemNome').value=i.nome;el('itemCategoria').value=i.categoria;el('itemPreco').value=i.preco||0;el('itemCusto').value=i.custo||0;el('itemEstoque').value=i.estoque||0;el('itemMinimo').value=i.minimo||0;el('itemUnidade').value=i.unidade||'un';if(el('itemPorcao'))el('itemPorcao').value=i.porcao||1;if(el('itemPorcaoUnidade'))el('itemPorcaoUnidade').value=i.porcaoUnidade||i.unidade||'un';el('itemIcone').value=i.icone||'';if(el('itemIconeLista'))el('itemIconeLista').value=i.icone||'';if(el('itemImagem'))el('itemImagem').value=i.imagem||'';el('itemObs').value=i.obs||'';page('estoque');renderListas();setTimeout(()=>{el('itemNome')?.focus();document.querySelector(`[data-item-id="${id}"]`)?.scrollIntoView({block:'nearest',behavior:'smooth'});},80)}
  function limparFormItem(show=true){['itemId','itemNome','itemPreco','itemCusto','itemEstoque','itemMinimo','itemPorcao','itemPorcaoUnidade','itemIcone','itemImagem','itemObs'].forEach(id=>{if(el(id))el(id).value=''});if(el('itemCategoria'))el('itemCategoria').value='Massa';if(el('itemUnidade'))el('itemUnidade').value='un';if(el('itemPorcaoUnidade'))el('itemPorcaoUnidade').value='un';if(el('itemIconeLista'))el('itemIconeLista').value='';if(el('formItemTitulo')){el('formItemTitulo').textContent='Cadastrar item';el('formItemTitulo').classList.remove('editing-title')}renderListas();if(show)toast('Formulário limpo')}
  ...
  function pedidoCard(p){return `<div class="pedido-card"><div class="pedido-top"><strong>${pedidoNumeroLabel(p)}</strong><span class="pill origin-pill ${origemPedidoClass(p)}">${origemPedidoLabel(p)}</span><span class="status ${statusClass(p.status)}">${p.status}</span></div><small>${new Date(p.data).toLocaleString('pt-BR')} • ${p.tipo} • prev. ${p.pagamentoPrevisto||'PIX'}</small><p><b>${p.cliente}</b></p><div class="pedido-itens">${p.itens.map(i=>`<span>${labelItemCliente(i)}</span>`).join('')}</div><div class="pedido-bottom"><b>${fmt(p.total)}</b><div class="list-actions">${statusActions(p)}<button class="mini secondary" onclick="App.abrirEditarDadosPedido('${p.id}')">dados</button><button class="mini secondary" onclick="App.editarPedido('${p.id}')">editar</button><button class="mini secondary" onclick="App.verPedido('${p.id}')">detalhes</button></div></div></div>`}
  ...
  async function buscarClientePortal(showToast=false){
    ...
    box.className='portal-welcome ok-box';box.innerHTML=`Cadastro encontrado: <button class="link-button" onclick="App.selecionarClientePortalOnline('${c.id}')"><strong>${c.nome||'Cliente'}</strong></button><br><small>Clique no nome para usar esse cadastro neste pedido.</small>`;if(showToast)toast('Cadastro encontrado. Clique no nome para selecionar.');return c
  }
  ...
  async function confirmarPedidoPortal(id){const p=db.pedidos.find(x=>x.id===id);if(!p)return;if(!validarEstoqueItens(p.itens||[]))return;const anterior=deepCopy(p);if(!p.estoqueBaixado){baixarEstoque(p.itens||[]);p.estoqueBaixado=true}p.status='Pedido Feito';p.timeline=p.timeline||[];p.timeline.push({status:'Pedido Feito',data:new Date().toISOString(),detalhes:'Pedido externo confirmado'});log('Pedido externo confirmado','#'+p.numero);save();renderAll();const ok=await supabaseAtualizarPedido(p);if(!ok){Object.assign(p,anterior);save();renderAll();return toast('Não foi possível confirmar no banco. A alteração foi desfeita.')}await supabaseCarregarPedidos();renderAll();toast('Pedido confirmado e enviado para preparo')}
  ...
  function mostrarLoginAdmin(){
    ...
    wrap.innerHTML=`<div class="admin-login-card"><h2>🍝 Donas OS</h2><p class="muted">Acesso administrativo</p><input id="adminLoginSenha" type="password" placeholder="Senha do ADM" autocomplete="current-password"><button class="primary full" id="adminLoginBtn" type="button">Entrar</button><small class="hint">Cliente não precisa de login. Esta proteção é apenas para o painel interno.</small></div>`;
    document.body.appendChild(wrap);
    const entrar=()=>{const senha=(el('adminLoginSenha')?.value||'').trim();const correta=(db?.config?.adminSenha||ADMIN_DEFAULT_PASS);if(senha===correta){sessionStorage.setItem(ADMIN_SESSION_KEY,'ok');wrap.remove();document.body.classList.remove('login-required');init();}else{toast('Senha incorreta.')}};
    setTimeout(()=>{el('adminLoginSenha')?.focus();el('adminLoginBtn')?.addEventListener('click',entrar);el('adminLoginSenha')?.addEventListener('keydown',e=>{if(e.key==='Enter')entrar();});},50);
  }
  ...
  function renderAll(){
    ...
    renderSelects();renderFavoritos();renderSteps();renderResumo();renderListas();renderDashboard();renderConfig();renderInsights();renderRelatorios();renderOnlineReady();renderPortalCliente();renderOperacao();renderCupons();renderAssistentePorcoes();
    if(keepScroll!==null){
      const restore=()=>{
