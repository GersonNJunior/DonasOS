const App=(()=>{
  ...
  function porcoesPortalItem(it){const qtd=Number(portalPedido.qtd[it.id]||defaultQtd(it));const base=Math.max(Number(it.porcao||1),0.0001);return Math.max(1,Math.round(qtd/base));}
  function setPortalPorcao(id,delta){const it=db.itens.find(i=>i.id===id);if(!it)return;if(['Massa','Molho'].includes(it.categoria))return;const atual=porcoesPortalItem(it);const prox=Math.max(1,atual+Number(delta||0));portalPedido.qtd[id]=prox*Math.max(Number(it.porcao||1),1);renderPortalCliente()}
  function setPortalQtd(id,v){const it=db.itens.find(i=>i.id===id);if(!it)return;if(['Massa','Molho'].includes(it.categoria)){portalPedido.qtd[id]=defaultQtd(it)}else{const porcoes=Math.max(1,Math.round(Number(v||1)));portalPedido.qtd[id]=porcoes*Math.max(Number(it.porcao||1),1)}renderPortalCliente()}
