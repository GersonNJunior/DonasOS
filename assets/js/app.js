const App=(()=>{
  ...
  function syncPedido(){pedido.cliente=val('clientePedido');pedido.telefone=val('telefonePedido');pedido.tipo=val('tipoPedido')||'Balcão';pedido.pagamentoPrevisto=val('pagamentoPedido')||'PIX';pedido.obs=val('obsPedido');pedido.cep=val('pedidoCep');pedido.rua=val('pedidoRua');pedido.numero=val('pedidoNumeroEndereco');pedido.complemento=val('pedidoComplemento');pedido.bairro=val('pedidoBairro')||val('bairroPedido');pedido.cidade=val('pedidoCidade');pedido.uf=val('pedidoUf');pedido.endereco=enderecoCompleto(pedido)||val('enderecoPedido')}
  ...
  function movimentosFinanceiros(){
    const movs=[];
    (db.pedidos||[]).filter(p=>p.financeiroLancado||p.status==='Entregue').forEach(p=>{
  ...
  function limparPortalCliente(){if(!confirm('Deseja realmente limpar seu pedido?'))return;portalPedido={itens:{},qtd:{}};portalPratos=[];portalClienteIdentificado=null;['portalCliente','portalTelefone','portalTelefoneFinal','portalCep','portalRua','portalNumero','portalComplemento','portalEndereco','portalBairro','portalCidade','portalUf','portalObs'].forEach(id=>{const x=el(id);if(x)x.value=''});if(el('portalConfirmaEndereco'))el('portalConfirmaEndereco').checked=false;if(el('portalClienteStatus')){el('portalClienteStatus').className='portal-welcome muted';el('portalClienteStatus').innerHTML='Informe o telefone para localizar cadastro, histórico e último pedido.'}if(el('portalTipo'))el('portalTipo').value='Retirada';if(el('portalPagamento'))el('portalPagamento').value='PIX';if(el('portalTalher'))el('portalTalher').value='Não';if(el('portalWhatsapp'))el('portalWhatsapp').textContent='Nenhum pedido externo criado ainda.';renderPortalCliente();setTimeout(()=>scrollPortalTo('page-portal'),120);toast('Pedido limpo')}
  ...
  async function criarPedidoPortal(){
    ...
    const p={id:uid(),onlineId:onlineId('PED'),numero,cliente,telefone,tipo:tipo==='Retirada'?'Balcão':tipo,pagamento:'',pagamentoPrevisto:val('portalPagamento')||'PIX',talher,obs:val('portalObs'),cep:val('portalCep'),rua:val('portalRua'),numeroEndereco:val('portalNumero'),complemento:val('portalComplemento'),endereco:enderecoCompleto({rua:val('portalRua'),numero:val('portalNumero'),complemento:val('portalComplemento'),endereco:val('portalEndereco')}),bairro:val('portalBairro').trim(),cidade:val('portalCidade'),uf:val('portalUf'),clienteId:portalClienteIdentificado?.id||'',pratos:pratos.map((pr,idx)=>({...pr,nome:pr.nome||('Prato '+(idx+1))})),itens,total,custo,lucro:total-custo,status:'Aguardando confirmação',financeiroLancado:false,estoqueBaixado:false,origem:'portal-local',data:new Date().toISOString(),timeline:[{status:'Aguardando confirmação',data:new Date().toISOString()}],syncStatus:'local'};
  ...
  function confirmarPedidoPortal(id){const p=db.pedidos.find(x=>x.id===id);if(!p)return;if(!validarEstoqueItens(p.itens||[]))return;if(!p.estoqueBaixado){baixarEstoque(p.itens||[]);p.estoqueBaixado=true}p.status='Pedido Feito';p.timeline=p.timeline||[];p.timeline.push({status:'Pedido Feito',data:new Date().toISOString(),detalhes:'Pedido externo confirmado'});log('Pedido externo confirmado','#'+p.numero);save();supabaseAtualizarPedido(p);toast('Pedido confirmado e enviado para preparo')}
