const App=(()=>{
  ...
  function pedidoTotal(){syncPedido();const editIdx=Number(pedido.pratoEditandoIndex);let t=pedido.pratos.reduce((a,p,idx)=>a+(idx===editIdx?0:Number(p.total||0)),0);const atual=currentPrato();if(atual.itens.length)t+=atual.total;if(pedido.tipo==='Delivery')t+=Number(db.config.taxaEntrega||0);return t}
  ...
  async function finalizarPedido(){
    ...
    const entrega=pedido.tipo==='Delivery'?Number(db.config.taxaEntrega||0):0;
  ...
  function limparPedido(show=true){if(show&&editBuffer&&!confirm('Cancelar a edição? O pedido original será mantido sem alterações.'))return;editBuffer=null;pedido=novoPedido();['clientePedido','telefonePedido','obsPedido','pedidoCep','pedidoRua','pedidoNumeroEndereco','pedidoComplemento','enderecoPedido','pedidoBairro','bairroPedido','pedidoCidade','pedidoUf'].forEach(id=>{if(el(id))el(id).value=''});if(el('tipoPedido'))el('tipoPedido').value='Balcão';if(el('atendimentoPedido'))el('atendimentoPedido').value='imediato';if(el('reservaDataPedido'))el('reservaDataPedido').value='';if(el('reservaHorarioPedido'))el('reservaHorarioPedido').value='';alternarCamposReserva();if(el('pagamentoPedido'))el('pagamentoPedido').value='PIX';if(el('pagamentoRecebidoPedido'))el('pagamentoRecebidoPedido').value='nao';const box=el('clientesEncontrados');if(box){box.className='client-results hidden';box.innerHTML=''}renderAll();if(show)toast('Pedido limpo')}
  ...
  function limparPortalCliente(){if(!confirm('Deseja realmente limpar seu pedido?'))return;portalPedido={itens:{},qtd:{}};portalPratos=[];portalClienteIdentificado=null;['portalCliente','portalTelefone','portalTelefoneFinal','portalCep','portalRua','portalNumero','portalComplemento','portalEndereco','portalBairro','portalCidade','portalUf','portalObs','portalCupom'].forEach(id=>{const x=el(id);if(x)x.value=''});if(el('portalConfirmaEndereco'))el('portalConfirmaEndereco').checked=false;if(el('portalClienteStatus')){el('portalClienteStatus').className='portal-welcome muted';el('portalClienteStatus').innerHTML='Informe o telefone para localizar cadastro, histórico e último pedido.'}if(el('portalTipo'))el('portalTipo').value='Retirada';if(el('portalPagamento'))el('portalPagamento').value='PIX';if(el('portalTalher'))el('portalTalher').value='Não';if(el('portalWhatsapp'))el('portalWhatsapp').textContent='Nenhum pedido externo criado ainda.';renderPortalCliente();setTimeout(()=>scrollPortalTo('page-portal'),120);toast('Pedido limpo')}
  ...
  async function criarPedidoPortal(){
    ...
    const entrega=tipo==='Delivery'?Number(db.config.taxaEntrega||0):0;const bruto=pratos.reduce((a,pr)=>a+Number(pr.total||0),0)+entrega;if(val('portalCupom')&&supabaseLojaId)await supabaseCarregarCupons();const cup=usarCupomSeValido(val('portalCupom'),bruto);if(val('portalCupom')&&!cup.ok)return toast(cup.msg||'Cupom inválido');const desconto=Number(cup.desconto||0);const total=Math.max(0,bruto-desconto);const custo=pratos.reduce((a,pr)=>a+Number(pr.custo||0),0);
    const numero=await proximoNumeroPedidoSeguro();
    const pago=false;const agora=new Date().toISOString();const pagamentoPrevisto=val('portalPagamento')||'PIX';const p={id:uid(),onlineId:'PED-'+hoje().replace(/-/g,'')+'-'+numero,numero,cliente,telefone,tipo:tipo==='Retirada'?'Balcão':tipo,pagamento:pago?pagamentoPrevisto:'',pagamentoPrevisto,talher,obs:val('portalObs','portalCupom'),cep:val('portalCep'),rua:val('portalRua'),numeroEndereco:val('portalNumero'),complemento:val('portalComplemento'),endereco:enderecoCompleto({rua:val('portalRua'),numero:val('portalNumero'),complemento:val('portalComplemento'),endereco:val('portalEndereco')}),bairro:val('portalBairro').trim(),cidade:val('portalCidade'),uf:val('portalUf'),clienteId:portalClienteIdentificado?.id||'',pratos:pratos.map((pr,idx)=>({...pr,nome:pr.nome||('Prato '+(idx+1))})),itens,total,subtotalBruto:bruto,desconto,cupom:cup.codigo||'',cupomId:cup.cupom?.id||'',custo,lucro:total-custo,status:'Aguardando confirmação',pagamentoRecebido:pago,financeiroLancado:pago,dataPagamento:pago?agora:'',estoqueBaixado:false,origem:'online',data:agora,timeline:[{status:'Aguardando confirmação',data:agora,pagamentoRecebido:pago}],syncStatus:'local'};
