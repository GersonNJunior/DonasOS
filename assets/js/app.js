const App=(()=>{
  ...
  async function supabaseSalvarClientePedido(p){
    try{
      ...
      if(tel){
        try{
          const pend=await supabaseRequest('/clientes_pendentes?select=id&telefone=eq.'+encodeURIComponent(tel)+'&status=eq.pendente&limit=1');
          if(pend?.[0]?.id){
