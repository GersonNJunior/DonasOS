const App=(()=>{
  ...
  let accessRealtimeClient=null;
  let accessPresenceChannel=null;
  let accessOnlineCount=0;
  let accessHistoryCache=[];
  let accessHistoryLoading=false;
  const ACCESS_TABLE='site_access_events';
  const ACCESS_SESSION_KEY='donasos_access_session';
  ...
  let configLastUserInput=0;
  let fretePortal={status:'idle',distancia:0,valor:0,mensagem:''};
  let fretePedido={status:'idle',distancia:0,valor:0,mensagem:''};
  let freteTimerPortal=null;
  let freteTimerPedido=null;
  const ADMIN_SESSION_KEY='donas_admin_ok_v1';
  const ADMIN_ROLE_KEY='donas_admin_role_v1';
  const ADMIN_DEFAULT_PASS='donas2026';
  const OPERACAO_DEFAULT_PASS='massa2026';
  const OPERACAO_PAGES=['pedidos','reservados','cozinha','pdv','estoque','online'];
  ...
  const fmtQtd=(v,d=2)=>{const n=Number(v||0);if(!isFinite(n))return '0';return n.toLocaleString('pt-BR',{minimumFractionDigits:0,maximumFractionDigits:d})};
  const origemPedidoLabel=p=>{const cod=String(p?.onlineId||p?.codigo||'');const orig=String(p?.origem||'').toLowerCase();return (cod.startsWith('PED-')||orig.includes('portal')||orig==='online')?'🌐 Online':'🏪 Interno'};
  const origemPedidoClass=p=>origemPedidoLabel(p).includes('Online')?'origin-online':'origin-interno';
  const pedidoNumeroLabel=p=>`Pedido #${p?.numero||String(p?.onlineId||p?.codigo||'').split('-').pop()||'0000'}`;
  const parsePedidoNumero=p=>{const cand=[p?.numero,String(p?.onlineId||p?.codigo||'').split('-').pop()].find(v=>/^\d+$/.test(String(v||'')));return Number(cand||0)||0};
  const formatPedidoNumero=n=>String(Number(n||0)||0).padStart(4,'0');
  const proximoNumeroPedido=()=>formatPedidoNumero(((db?.pedidos||[]).reduce((m,p)=>Math.max(m,parsePedidoNumero(p)),0)+1));
  async function proximoNumeroPedidoSeguro(){
    try{
      const loja=await supabaseGetLoja();
      const n=await supabaseRequest('/rpc/donas_proximo_pedido',{method:'POST',body:JSON.stringify({p_loja_id:loja})});
      const num=Array.isArray(n)?n[0]:n;
      return formatPedidoNumero(num);
    }catch(e){
      console.warn('Sequência Supabase indisponível, usando fallback local:',e);
      return proximoNumeroPedido();
    }
  }
  const finalizacoesGratisLivre=()=>String(db?.config?.finalizacoesGratis??'').trim()==='';
  const limiteGratisCategoria=cat=>{if(cat==='Complemento')return Number(db.config.complementosGratis||0);if(cat==='Proteína')return Number(db.config.proteinasGratis||0);if(cat==='Finalização')return finalizacoesGratisLivre()?Infinity:Number(db.config.finalizacoesGratis||0);return 0};
  const labelGratisCategoria=cat=>{if(cat==='Complemento')return `Escolha grátis ${db.config.complementosGratis||0}`;if(cat==='Proteína')return `Escolha grátis ${db.config.proteinasGratis||0}`;if(cat==='Finalização')return finalizacoesGratisLivre()?'Selecionar grátis':`Escolha grátis ${db.config.finalizacoesGratis||0}`;return ''};
  const tamanhosAtivos=()=>((db.config.tamanhosPrato||[]).filter(t=>t.ativo!==false).sort((a,b)=>Number(a.ordem||0)-Number(b.ordem||0)));
  function tamanhoPorId(id){return (db.config.tamanhosPrato||[]).find(t=>String(t.id)===String(id))||tamanhosAtivos()[0]||null}
  function tamanhoPedidoAtual(contexto='interno'){const alvo=contexto==='portal'?portalPedido:pedido;if(!alvo.tamanhoId){const p=tamanhosAtivos()[0];if(p)alvo.tamanhoId=p.id;}return tamanhoPorId(alvo.tamanhoId)}
  function limiteCategoriaTamanho(cat,contexto='interno'){const t=tamanhoPedidoAtual(contexto);if(cat==='Proteína')return Number(t?.proteinas??db.config.proteinasGratis??0);if(cat==='Complemento')return Number(t?.complementos??db.config.complementosGratis??0);return limiteGratisCategoria(cat)}
  function snapshotTamanho(t){return t?{id:t.id,nome:t.nome,preco:Number(t.preco||0),pesoAprox:t.pesoAprox||'',descricao:t.descricao||'',massaGramas:Number(t.massaGramas||0),proteinas:Number(t.proteinas||0),complementos:Number(t.complementos||0)}:null}
  ...
  const dataLocalISO=d=>{const dt=new Date(d||Date.now());if(isNaN(dt.getTime()))return String(d||'').slice(0,10);const y=dt.getFullYear(),m=String(dt.getMonth()+1).padStart(2,'0'),day=String(dt.getDate()).padStart(2,'0');return `${y}-${m}-${day}`};
  const hojeLocal=()=>dataLocalISO(new Date());
  const STATUS=['Aguardando confirmação','Pedido Feito','Preparando','Pedido Pronto','Entregue'];
  const PAGAMENTOS=['PIX','Dinheiro','Débito (aproximação)','Crédito (aproximação)'];
  ...
  function chaveTelefone(v){const d=normalizarTelefone(v);return d.length>11?d.slice(-11):d}
  function telefoneCombinaForte(a,b){
    const A=chaveTelefone(a),B=chaveTelefone(b);
    if(!A||!B)return false;
    if(A===B)return true;
  ...
  function encontrarClientePorTelefone(telefone){
    const tel=chaveTelefone(telefone);
    if(!tel)return null;
    return (db.clientes||[]).find(c=>telefoneCombinaForte(tel,c.telefone));
  }
  const enderecoCompleto=(o={})=>{const partes=[];const rua=(o.rua||'').trim();const numero=(o.numero||'').trim();const comp=(o.complemento||'').trim();if(rua)partes.push(rua+(numero?', '+numero:''));else if(o.endereco)partes.push(String(o.endereco||'').trim());if(comp)partes.push(comp);return partes.filter(Boolean).join(' - ')};
  const cepLimpo=v=>norm(v).slice(0,8);
  ...
  function seed(){return{version:'1.2.0-operacao',config:{nome:'Donas da Massa',nomeInterno:'Donas OS',slogan:'Seu macarrão. Seu jeito.',cnpj:'',email:'',enderecoEmpresa:'',instagram:'',pix:'',mensagemPagamento:'Olá, {cliente}! Seu pedido #{pedido} foi confirmado.\n\nValor: {valor}\nForma de pagamento: {pagamento}.{detalhes_pagamento}\n\nObrigado pela preferência! 🍝',whatsapp:'',taxaEntrega:0,raioEntrega:'',enderecoSaida:'',origemCep:'',origemNumero:'',origemLat:null,origemLng:null,freteAutomatico:true,faixasFrete:[{ateKm:1,valor:3},{ateKm:2,valor:5},{ateKm:3,valor:7},{ateKm:5,valor:10}],tempoPreparo:20,complementosGratis:2,proteinasGratis:1,finalizacoesGratis:1,metaDiaria:1000,modoOperacao:false,pratosFeitosAtivo:true,tamanhosPrato:[{id:'tradicional',nome:'Tradicional',preco:25,pesoAprox:'Aproximadamente 500 g',descricao:'Escolha até 2 proteínas e 4 complementos.',massaGramas:100,proteinas:2,complementos:4,ativo:true,ordem:1},{id:'grande',nome:'Grande',preco:35,pesoAprox:'Aproximadamente 750 g',descricao:'Escolha até 3 proteínas e 5 complementos.',massaGramas:150,proteinas:3,complementos:5,ativo:true,ordem:2}],adminSenha:'donas2026',operacaoSenha:'massa2026'},itens:[
  ...
  function migrate(x){const base=seed();x.version='1.0.0-rc-inauguracao';x.config={...base.config,...(x.config||{})}; if(x.config.whatsapp===undefined)x.config.whatsapp='';if(x.config.mensagemPagamento===undefined)x.config.mensagemPagamento='Olá, {cliente}! Seu pedido #{pedido} foi confirmado.\n\nValor: {valor}\nForma de pagamento: {pagamento}.{detalhes_pagamento}\n\nObrigado pela preferência! 🍝';if(x.config.pratosFeitosAtivo===undefined)x.config.pratosFeitosAtivo=true;if(!Array.isArray(x.config.tamanhosPrato)||!x.config.tamanhosPrato.length)x.config.tamanhosPrato=base.config.tamanhosPrato.map(t=>({...t,proteinas:t.id==='tradicional'?Number(x.config.proteinasGratis||2):t.proteinas,complementos:t.id==='tradicional'?Number(x.config.complementosGratis||4):t.complementos}));if(x.config.finalizacoesGratis===undefined)x.config.finalizacoesGratis=1;if(x.config.adminSenha===undefined)x.config.adminSenha='donas2026';if(x.config.operacaoSenha===undefined)x.config.operacaoSenha='massa2026';if(x.config.enderecoSaida===undefined)x.config.enderecoSaida=x.config.enderecoEmpresa||'';if(x.config.origemCep===undefined)x.config.origemCep='';if(x.config.origemNumero===undefined)x.config.origemNumero='';if(x.config.origemLat===undefined)x.config.origemLat=null;if(x.config.origemLng===undefined)x.config.origemLng=null;if(x.config.freteAutomatico===undefined)x.config.freteAutomatico=true;if(!Array.isArray(x.config.faixasFrete))x.config.faixasFrete=[{ateKm:1,valor:3},{ateKm:2,valor:5},{ateKm:3,valor:7},{ateKm:5,valor:10}];x.itens=(x.itens||base.itens).map(i=>{let icon=i.icone||iconeCat(i.categoria);if((i.nome||'').toLowerCase().includes('calabresa')&&icon==='🌭')icon='🍖';return{...i,unidade:i.unidade||'un',porcao:Number(i.porcao||1),porcaoUnidade:i.porcaoUnidade||i.porcao_unidade||i.unidade||'un',icone:icon,imagem:(i.imagem||(((i.nome||'').toLowerCase().includes('calabresa'))?'assets/img/ingredientes/calabresa.png':'')),ativo:i.ativo!==false,obs:i.obs||''}});x.pedidos=(x.pedidos||[]).map(p=>{const st=(p.status==='Concluído')?'Entregue':(p.status||'Entregue');let itens=(p.itens||[]).map(i=>({...i,qtdUsada:Number(i.qtdUsada||i.qtd||i.quantidadeUsada||1),porcao:Number(i.porcao||1)}));const rv=extrairReserva(p.obs||'');return{...p,itens,atendimento:p.atendimento||rv.atendimento,reservaData:p.reservaData||rv.reservaData,reservaHorario:p.reservaHorario||rv.reservaHorario,reservaIniciada:p.reservaIniciada??rv.reservaIniciada??!((p.atendimento||rv.atendimento)==='reserva'&&st==='Pedido Feito'),obs:p.atendimento?p.obs:rv.obs,pratos:p.pratos||[{nome:'Prato 1',itens}],status:st,financeiroLancado:p.financeiroLancado??p.pagamentoRecebido??((x.financeiro||[]).some(m=>m.origem==='pedido'&&m.pedidoId===p.id)),pagamentoRecebido:p.pagamentoRecebido??p.financeiroLancado??false,dataPagamento:p.dataPagamento||'',pagamento:p.pagamento||'',pagamentoPrevisto:p.pagamentoPrevisto||p.pagamento||'PIX'}});x.clientes=x.clientes||[];x.clientesPendentes=x.clientesPendentes||[];x.clientes=x.clientes.map(c=>({...c,cep:c.cep||'',rua:c.rua||'',numero:c.numero||'',complemento:c.complemento||'',bairro:c.bairro||'',cidade:c.cidade||'',uf:c.uf||'',endereco:c.endereco||enderecoCompleto(c)}));x.clientesPendentes=x.clientesPendentes.map(c=>({...c,status:c.status||'pendente',cep:c.cep||'',rua:c.rua||'',numero:c.numero||'',complemento:c.complemento||'',bairro:c.bairro||'',cidade:c.cidade||'',uf:c.uf||'',endereco:c.endereco||enderecoCompleto(c)}));x.financeiro=x.financeiro||[];x.marketing=(x.marketing||[]).map(m=>({
      ...m,
      ativo:m.ativo!==false,
  ...
  let receitaTemp=[];
  let portalPedido={itens:{},qtd:{},tamanhoId:''};
  let portalPratos=[];
  ...
  let pontoOrigemTemporario=null;
  ...
  function novoPedido(){return{itens:{},qtd:{},pratos:[],pratoEditandoIndex:-1,cliente:'',telefone:'',tipo:'Balcão',atendimento:'imediato',reservaData:'',reservaHorario:'',pagamentoPrevisto:'PIX',obs:'',cep:'',rua:'',numero:'',complemento:'',endereco:'',bairro:'',cidade:'',uf:'',clienteId:'',editandoId:'',editandoNumero:'',cupom:'',cupomId:'',desconto:0,subtotalBruto:0,freteGratis:false,freteOriginal:0,tamanhoId:'',tamanhoSnapshot:null}}
  ...
  async function buscarCep(prefix){const cep=cepLimpo(val(prefix+'Cep'));if(!cep)return;if(cep.length!==8)return toast('CEP deve ter 8 números, ou deixe em branco.');try{const r=await fetch('https://viacep.com.br/ws/'+cep+'/json/');const data=await r.json();if(data.erro)return toast('CEP não encontrado. Você pode preencher manualmente.');if(el(prefix+'Rua')&&data.logradouro)el(prefix+'Rua').value=data.logradouro;if(el(prefix+'Bairro')&&data.bairro)el(prefix+'Bairro').value=data.bairro;if(el(prefix+'Cidade')&&data.localidade)el(prefix+'Cidade').value=data.localidade;if(el(prefix+'Uf')&&data.uf)el(prefix+'Uf').value=data.uf;if(prefix==='pedido')syncPedido();if(prefix==='pedido'||prefix==='portal')agendarCalculoFrete(prefix);toast('Endereço preenchido pelo CEP')}catch(e){toast('Não consegui buscar o CEP agora. Preencha manualmente.')}}

  function faixasFreteOrdenadas(){return (db.config.faixasFrete||[]).map(f=>({ateKm:Number(f.ateKm||0),valor:Number(f.valor||0)})).filter(f=>f.ateKm>0).sort((a,b)=>a.ateKm-b.ateKm)}
  function calcularValorFrete(distancia){const faixa=faixasFreteOrdenadas().find(f=>distancia<=f.ateKm+0.001);return faixa?{ok:true,valor:faixa.valor,ateKm:faixa.ateKm}:{ok:false,valor:0}}
  function enderecoParaCalculo(prefix){const rua=val(prefix+'Rua').trim(),numero=val(prefix+(prefix==='pedido'?'NumeroEndereco':'Numero')).trim(),bairro=val(prefix+'Bairro').trim(),cidade=val(prefix+'Cidade').trim(),uf=val(prefix+'Uf').trim();return [rua,numero,bairro,cidade,uf,'Brasil'].filter(Boolean).join(', ')}
  async function geocodificarEndereco(texto){const q=String(texto||'').trim();if(!q)return null;const url='https://nominatim.openstreetmap.org/search?format=jsonv2&limit=1&countrycodes=br&q='+encodeURIComponent(q);const r=await fetch(url,{headers:{'Accept-Language':'pt-BR'}});if(!r.ok)throw new Error('Falha ao localizar endereço');const data=await r.json();if(!data?.[0])return null;return{lat:Number(data[0].lat),lng:Number(data[0].lon),nome:data[0].display_name||q}}
  function distanciaRetaKm(a,b){const R=6371,toRad=x=>x*Math.PI/180,dLat=toRad(b.lat-a.lat),dLng=toRad(b.lng-a.lng);const h=Math.sin(dLat/2)**2+Math.cos(toRad(a.lat))*Math.cos(toRad(b.lat))*Math.sin(dLng/2)**2;return 2*R*Math.asin(Math.sqrt(h))}
  async function distanciaRotaKm(origem,destino){try{const url=`https://router.project-osrm.org/route/v1/driving/${origem.lng},${origem.lat};${destino.lng},${destino.lat}?overview=false`;const r=await fetch(url);const data=await r.json();const metros=Number(data?.routes?.[0]?.distance||0);if(metros>0)return metros/1000}catch(e){console.warn('Rota indisponível; usando estimativa geográfica.',e)}return distanciaRetaKm(origem,destino)*1.25}
  function atualizarFreteStatus(prefix,estado){const box=el(prefix+'FreteStatus');if(!box)return;box.className='delivery-calc-status '+(estado.status==='ok'?'ok':estado.status==='fora'?'alert':'');box.textContent=estado.mensagem||'Preencha o endereço para calcular o frete.'}
  function freteAtual(contexto){const estado=contexto==='portal'?fretePortal:fretePedido;if(estado.status==='ok')return Number(estado.valor||0);return Number(db.config.taxaEntrega||0)}
  function dadosEnderecoCliente(prefix){const numeroId=prefix==='pedido'?'NumeroEndereco':'Numero';return{rua:val(prefix+'Rua').trim(),numero:val(prefix+numeroId).trim(),bairro:val(prefix+'Bairro').trim(),cidade:val(prefix+'Cidade').trim(),uf:val(prefix+'Uf').trim(),cep:cepLimpo(val(prefix+'Cep'))}}
  async function buscarDestinoClientePorTentativas(prefix){const d=dadosEnderecoCliente(prefix);const tentativas=[
    {nivel:'exato',partes:[d.rua,d.numero,d.bairro,d.cidade,d.uf,'Brasil']},
    {nivel:'rua',partes:[d.rua,d.bairro,d.cidade,d.uf,'Brasil']},
    {nivel:'rua',partes:[d.rua,d.cidade,d.uf,'Brasil']},
    {nivel:'cep',partes:[d.cep,d.cidade,d.uf,'Brasil']},
    {nivel:'bairro',partes:[d.bairro,d.cidade,d.uf,'Brasil']}
  ];const usados=new Set();for(const tentativa of tentativas){const q=tentativa.partes.filter(Boolean).join(', ');if(!q||usados.has(q.toLowerCase()))continue;usados.add(q.toLowerCase());try{const pos=await geocodificarEndereco(q);if(pos)return{...pos,nivel:tentativa.nivel,consulta:q}}catch(e){console.warn('Tentativa de localização do cliente falhou:',q,e)}}return null}
  async function calcularFreteEndereco(prefix){const isPortal=prefix==='portal',setEstado=e=>{if(isPortal)fretePortal=e;else fretePedido=e;atualizarFreteStatus(prefix,e);if(isPortal)renderPortalCliente();else renderResumo()};if((isPortal?val('portalTipo'):val('tipoPedido'))!=='Delivery'){setEstado({status:'idle',distancia:0,valor:0,mensagem:'Retirada selecionada: sem frete.'});return}const origem={lat:Number(db.config.origemLat),lng:Number(db.config.origemLng)};if(!origem.lat||!origem.lng){setEstado({status:'erro',distancia:0,valor:Number(db.config.taxaEntrega||0),mensagem:'Frete automático ainda não configurado. Será usada a taxa fixa.'});return}const dados=dadosEnderecoCliente(prefix);if(!dados.rua||!dados.cidade){setEstado({status:'idle',distancia:0,valor:0,mensagem:'Preencha rua, número, bairro e cidade para calcular.'});return}setEstado({status:'calculando',distancia:0,valor:0,mensagem:'Validando o endereço e calculando o frete...'});try{const destino=await buscarDestinoClientePorTentativas(prefix);if(!destino)throw new Error('Endereço e região não localizados');const distancia=await distanciaRotaKm(origem,destino);const faixa=calcularValorFrete(distancia);const aproximado=destino.nivel!=='exato';const detalhe=aproximado?'Cálculo aproximado pela região do endereço. ':'';if(!faixa.ok){setEstado({status:'fora',distancia,valor:0,aproximado,mensagem:`${detalhe}Endereço fora da área de entrega (${distancia.toFixed(1)} km).`});return}setEstado({status:'ok',distancia,valor:faixa.valor,aproximado,mensagem:`${detalhe}Distância aproximada: ${distancia.toFixed(1)} km • Frete: ${fmt(faixa.valor)}`})}catch(e){console.warn(e);setEstado({status:'erro',distancia:0,valor:Number(db.config.taxaEntrega||0),mensagem:'Não foi possível calcular automaticamente. Será usada a taxa fixa de segurança.'})}}
  function agendarCalculoFrete(prefix){const isPortal=prefix==='portal';clearTimeout(isPortal?freteTimerPortal:freteTimerPedido);const t=setTimeout(()=>calcularFreteEndereco(prefix),700);if(isPortal)freteTimerPortal=t;else freteTimerPedido=t}
  async function buscarPontoOrigemPorTentativas(dados,cep,numero){const tentativas=[
    [dados.logradouro,numero,dados.bairro,dados.localidade,dados.uf,'Brasil'],
    [dados.logradouro,dados.bairro,dados.localidade,dados.uf,'Brasil'],
    [dados.logradouro,dados.localidade,dados.uf,'Brasil'],
    [cep,dados.localidade,dados.uf,'Brasil'],
    [dados.bairro,dados.localidade,dados.uf,'Brasil'],
    [dados.localidade,dados.uf,'Brasil']
  ];for(const partes of tentativas){const q=partes.filter(Boolean).join(', ');if(!q)continue;try{const pos=await geocodificarEndereco(q);if(pos)return pos}catch(e){console.warn('Tentativa de localização falhou:',q,e)}}return null}
  function atualizarPontoMapaOrigem(lat,lng){lat=Number(lat);lng=Number(lng);if(!Number.isFinite(lat)||!Number.isFinite(lng))return;pontoOrigemTemporario={lat,lng};if(marcadorOrigem)marcadorOrigem.setLatLng([lat,lng]);const st=el('cfgOrigemStatus');if(st){st.className='delivery-origin-status';st.textContent=`Ponto selecionado: ${lat.toFixed(5)}, ${lng.toFixed(5)}. Confirme o ponto abaixo.`}configEditing=true;configLastUserInput=Date.now()}
  function abrirMapaOrigem(pos){const box=el('cfgMapaOrigemBox');if(!box)return;if(typeof L==='undefined'){box.classList.add('hidden');toast('O mapa não conseguiu carregar. Verifique a internet e tente novamente.');return}box.classList.remove('hidden');const lat=Number(pos?.lat||db.config.origemLat||-21.1704),lng=Number(pos?.lng||db.config.origemLng||-47.8103);pontoOrigemTemporario={lat,lng};setTimeout(()=>{if(!mapaOrigem){mapaOrigem=L.map('cfgMapaOrigem').setView([lat,lng],16);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'&copy; OpenStreetMap'}).addTo(mapaOrigem);marcadorOrigem=L.marker([lat,lng],{draggable:true}).addTo(mapaOrigem);marcadorOrigem.on('dragend',()=>{const p=marcadorOrigem.getLatLng();atualizarPontoMapaOrigem(p.lat,p.lng)});mapaOrigem.on('click',e=>atualizarPontoMapaOrigem(e.latlng.lat,e.latlng.lng))}else{mapaOrigem.setView([lat,lng],16);marcadorOrigem.setLatLng([lat,lng])}mapaOrigem.invalidateSize()},120)}
  function centralizarMapaOrigem(){const p=pontoOrigemTemporario||{lat:Number(db.config.origemLat),lng:Number(db.config.origemLng)};if(!p.lat||!p.lng)return toast('Ainda não existe um ponto para centralizar.');abrirMapaOrigem(p)}
  function confirmarPontoOrigem(){const p=pontoOrigemTemporario;if(!p||!Number.isFinite(Number(p.lat))||!Number.isFinite(Number(p.lng)))return toast('Selecione um ponto no mapa.');db.config.origemLat=Number(p.lat);db.config.origemLng=Number(p.lng);const st=el('cfgOrigemStatus');if(st){st.className='delivery-origin-status ok';st.textContent=`Ponto confirmado: ${db.config.origemLat.toFixed(5)}, ${db.config.origemLng.toFixed(5)}. Agora salve as configurações.`}configEditing=true;configLastUserInput=Date.now();toast('Ponto de saída confirmado. Agora salve as configurações.')}
  async function localizarEnderecoSaida(){const cep=cepLimpo(val('cfgOrigemCep'));const numero=val('cfgOrigemNumero').trim();const st=el('cfgOrigemStatus');if(cep.length!==8)return toast('Informe um CEP válido com 8 números.');if(!numero)return toast('Informe o número do endereço de saída.');if(st){st.className='delivery-origin-status';st.textContent='Buscando CEP e preparando o mapa...'}try{const r=await fetch('https://viacep.com.br/ws/'+cep+'/json/');if(!r.ok)throw new Error('Falha na busca do CEP');const dados=await r.json();if(dados.erro||!dados.localidade||!dados.uf)throw new Error('CEP não encontrado');const endereco=[dados.logradouro,numero,dados.bairro,`${dados.localidade} - ${dados.uf}`].filter(Boolean).join(', ');db.config.origemCep=cep;db.config.origemNumero=numero;db.config.enderecoSaida=endereco;if(el('cfgEnderecoSaida'))el('cfgEnderecoSaida').value=endereco;let pos=await buscarPontoOrigemPorTentativas(dados,cep,numero);if(!pos){pos={lat:Number(db.config.origemLat)||-21.1704,lng:Number(db.config.origemLng)||-47.8103};if(st){st.className='delivery-origin-status alert';st.textContent='CEP encontrado, mas o mapa não achou a rua. Ajuste manualmente o marcador.'}}else if(st){st.className='delivery-origin-status';st.textContent='Local aproximado encontrado. Confira e ajuste o marcador no mapa.'}abrirMapaOrigem(pos);configEditing=true;configLastUserInput=Date.now();toast('CEP encontrado. Confirme o local exato no mapa.')}catch(e){console.warn(e);if(st){st.className='delivery-origin-status alert';st.textContent='Não foi possível consultar esse CEP agora.'}toast('Não foi possível buscar o CEP. Verifique a conexão e tente novamente.')}}
  function renderFaixasFrete(){const box=el('cfgFaixasFrete');if(!box)return;const faixas=faixasFreteOrdenadas();box.innerHTML=faixas.map((f,i)=>`<div class="delivery-range-row"><div class="field"><label>Até (km)</label><input type="number" min="0.1" step="0.1" value="${f.ateKm}" data-frete-km="${i}"></div><div class="field"><label>Valor</label><input type="number" min="0" step="0.01" value="${f.valor}" data-frete-valor="${i}"></div><button type="button" class="secondary mini" onclick="App.removerFaixaFrete(${i})">Remover</button></div>`).join('')||'<p class="muted">Nenhuma faixa cadastrada.</p>';box.querySelectorAll('input').forEach(inp=>inp.addEventListener('input',()=>{configEditing=true;configLastUserInput=Date.now()}))}
  function lerFaixasFreteFormulario(){const box=el('cfgFaixasFrete');if(!box)return db.config.faixasFrete||[];return [...box.querySelectorAll('.delivery-range-row')].map(row=>({ateKm:Number(row.querySelector('[data-frete-km]')?.value||0),valor:Number(row.querySelector('[data-frete-valor]')?.value||0)})).filter(f=>f.ateKm>0).sort((a,b)=>a.ateKm-b.ateKm)}
  function adicionarFaixaFrete(){db.config.faixasFrete=lerFaixasFreteFormulario();const ultima=db.config.faixasFrete.at(-1);db.config.faixasFrete.push({ateKm:Number(ultima?.ateKm||0)+1,valor:Number(ultima?.valor||0)});renderFaixasFrete();configEditing=true;configLastUserInput=Date.now()}
  function removerFaixaFrete(index){db.config.faixasFrete=lerFaixasFreteFormulario();db.config.faixasFrete.splice(index,1);renderFaixasFrete();configEditing=true;configLastUserInput=Date.now()}
  ...
  function page(id){if(isPublicClient())id='portal';if(!isPublicClient()&&id!=='pdv'&&pedido.editandoId){cancelarEdicaoPedido(true);}if(!isPublicClient()&&perfilAtual()==='operacao'&&!paginaPermitidaOperacao(id)){toast('Acesso restrito ao perfil Operação.');id='pedidos';}document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));el('page-'+id)?.classList.add('active');document.querySelectorAll('.nav button').forEach(b=>b.classList.toggle('active',b.dataset.page===id));document.querySelectorAll('.mobile-op-nav button').forEach(b=>b.classList.toggle('active',b.dataset.opPage===id));const activeBtn=document.querySelector(`.nav button[data-page="${id}"]`);if(activeBtn){const parent=activeBtn.closest('details');if(parent)parent.open=true;}try{localStorage.setItem(KEY+'_last_page',id)}catch(e){}el('pageTitle').textContent={pdv:'Montar Pedido',dashboard:'Início',operacao:'Operação',insights:'Insights',relatorios:'Relatórios',marketing:'Campanhas',cupons:'Cupons',pedidos:'Pedidos',reservados:'Reservados',estoque:'Estoque',compras:'Compras',producao:'Pratos Feitos',clientes:'Clientes',financeiro:'Financeiro',tamanhos:'Tamanhos dos Pratos',config:'Configurações',online:'Online Ready',acessos:'Acessos',portal:'Donas Online',cozinha:'Cozinha'}[id]||id;if(id==='insights'&&!isPublicClient())atualizarInsightsSupabase();if(id==='acessos'&&!isPublicClient())atualizarPainelAcessos();const main=document.querySelector('.main');if(main)main.scrollTop=0;window.scrollTo({top:0,left:0,behavior:'auto'});const pg=el('page-'+id);if(pg)pg.scrollTop=0}
  function extrairTrocoObs(obs=''){const m=String(obs||'').match(/\[TROCO_PARA:([0-9.,]+)\]/i);return m?Number(String(m[1]).replace(',','.')):0}
  function limparTrocoObs(obs=''){return String(obs||'').replace(/\s*\[TROCO_PARA:[^\]]+\]/gi,'').trim()}
  function obsComTroco(obs,troco){const limpa=limparTrocoObs(obs);return troco>0?[limpa,`[TROCO_PARA:${Number(troco).toFixed(2)}]`].filter(Boolean).join(' '):limpa}
  function alternarCamposTroco(prefix){const isPortal=prefix==='portal',pg=val(isPortal?'portalPagamento':'pagamentoPedido'),box=el(isPortal?'portalTrocoBox':'pedidoTrocoBox');if(box)box.classList.toggle('hidden',pg!=='Dinheiro');if(pg!=='Dinheiro'){const input=el(isPortal?'portalTrocoPara':'pedidoTrocoPara');if(input)input.value=''}}
  async function copiarTexto(texto,mensagem='Copiado!'){try{await navigator.clipboard.writeText(String(texto||''));toast(mensagem)}catch(e){const ta=document.createElement('textarea');ta.value=String(texto||'');ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.select();document.execCommand('copy');ta.remove();toast(mensagem)}}
  function detalhesPagamentoPedido(p){const pg=String(p.pagamentoPrevisto||p.pagamento||'Não informado');if(pg==='PIX')return '\nA chave PIX será enviada separadamente para facilitar a cópia.';if(pg.toLowerCase().includes('débito')||pg.toLowerCase().includes('crédito'))return `\nO pagamento via ${pg.toLowerCase()} será realizado na ${p.tipo==='Delivery'?'entrega':'retirada'}.`;if(pg==='Dinheiro'){const troco=Number(p.trocoPara||extrairTrocoObs(p.obs));return troco>0?`\nPagamento em dinheiro. Precisa de troco para ${fmt(troco)}.`:'\nPagamento em dinheiro. Não precisa de troco.'}return ''}
  function mensagemPagamentoPedido(p){const modelo=db.config.mensagemPagamento||'Olá, {cliente}! Seu pedido #{pedido} foi confirmado.\n\nValor: {valor}\nForma de pagamento: {pagamento}.{detalhes_pagamento}';return modelo.replaceAll('{cliente}',p.cliente||'cliente').replaceAll('{pedido}',String(p.numero||'')).replaceAll('{valor}',fmt(p.total||0)).replaceAll('{pagamento}',p.pagamentoPrevisto||p.pagamento||'Não informado').replaceAll('{detalhes_pagamento}',detalhesPagamentoPedido(p))}
  function copiarMensagemPagamento(id){const p=db.pedidos.find(x=>x.id===id);if(!p)return toast('Pedido não encontrado');copiarTexto(mensagemPagamentoPedido(p),'Mensagem do pedido copiada')}
  function copiarChavePix(){const chave=String(db.config.pix||'').trim();if(!chave)return toast('Configure a chave PIX em Configurações');copiarTexto(chave,'Chave PIX copiada')}
  function syncPedido(){pedido.cliente=val('clientePedido');pedido.telefone=val('telefonePedido');pedido.tipo=val('tipoPedido')||'Balcão';pedido.atendimento=val('atendimentoPedido')||'imediato';pedido.reservaData=val('reservaDataPedido');pedido.reservaHorario=val('reservaHorarioPedido');pedido.pagamentoPrevisto=val('pagamentoPedido')||'PIX';pedido.trocoPara=pedido.pagamentoPrevisto==='Dinheiro'?num('pedidoTrocoPara'):0;pedido.pagamentoRecebido=val('pagamentoRecebidoPedido')==='sim';pedido.obs=obsComTroco(val('obsPedido'),pedido.trocoPara);pedido.cupom=val('cupomPedido').trim().toUpperCase();pedido.cep=val('pedidoCep');pedido.rua=val('pedidoRua');pedido.numero=val('pedidoNumeroEndereco');pedido.complemento=val('pedidoComplemento');pedido.bairro=val('pedidoBairro')||val('bairroPedido');pedido.cidade=val('pedidoCidade');pedido.uf=val('pedidoUf');pedido.endereco=enderecoCompleto(pedido)||val('enderecoPedido');pedido.freteGratis=!!el('pedidoFreteGratis')?.checked;if(pedido.tipo!=='Delivery')pedido.freteGratis=false}
  ...
  const CATS_ESTOQUE_NUMERICO=['Bebida'];
  function controlaEstoqueNumerico(item){return CATS_ESTOQUE_NUMERICO.includes(String(item?.categoria||''))}
  function itemDisponivelVenda(item){if(!item||item.ativo===false)return false;return controlaEstoqueNumerico(item)?Number(item.estoque||0)>0:true}
  function statusEstoqueOperacional(item){return controlaEstoqueNumerico(item)?`${fmtQtd(item.estoque)} ${item.unidade||'un'}`:(item.ativo===false?'Indisponível':'Disponível')}
  function statusClassDisponibilidade(item){return controlaEstoqueNumerico(item)?(Number(item.estoque||0)<=Number(item.minimo||0)?'red':'green'):(item.ativo===false?'red':'green')}

  function unidadeBase(u){u=String(u||'').trim().toLowerCase();if(u==='l'||u==='lt'||u==='litro'||u==='litros')return 'ml';if(u==='kg'||u==='quilo'||u==='quilos')return 'g';return u;}
  function fatorParaBase(u){u=String(u||'').trim().toLowerCase();if(u==='kg'||u==='quilo'||u==='quilos')return 1000;if(u==='l'||u==='lt'||u==='litro'||u==='litros')return 1000;return 1;}
  function converterUnidadeEstoque(qtd,unidadeOrigem,unidadeDestino){
    qtd=Number(qtd||0);const o=unidadeBase(unidadeOrigem),d=unidadeBase(unidadeDestino);
    if(!qtd)return 0;if(o===d)return qtd*fatorParaBase(unidadeOrigem)/fatorParaBase(unidadeDestino);
    return qtd;
  }
  function tamanhoPorcaoEstoque(item){
    const qtd=Number(item?.porcao||1);
    const unidadePorcao=item?.porcaoUnidade||item?.porcao_unidade||item?.unidade||'un';
    const unidadeEstoque=item?.unidade||'un';
    const convertido=converterUnidadeEstoque(qtd,unidadePorcao,unidadeEstoque);
    return Number(convertido||qtd||1);
  }
  function defaultQtd(item){return tamanhoPorcaoEstoque(item)}
  function normalizarQtd(item,q){let n=Number(q||0);if(unidadeInteira(item))return Math.max(1,Math.round(n||1));return Math.max(0.001,Math.round((n||0.001)*1000)/1000)}
  function porcoesItem(i){const base=Math.max(tamanhoPorcaoEstoque(i),0.0001);return Math.max(1,Math.round(Number(i.qtdUsada||base)/base))}
  ...
  function porcoesInsightsItem(i){const cat=String(i?.categoria||i?.tipo||'').trim().toLowerCase();if(cat==='massa')return 1;const calculadas=porcoesPreparoItem(i);const salvas=Number(i?.porcoes||0);return Math.max(1,Number.isFinite(calculadas)?calculadas:(salvas||1))}
  ...
  function encontrarItemEstoque(ref){
    ...
    const rsid=String(ref.supabase_id||'');
    let item=(db.itens||[]).find(x=>x.id===ref.id || (ref.supabase_id&&x.supabase_id===ref.supabase_id));
    if(item)return item;
    if(rid.startsWith('sbit_')){
      const sid=rid.replace('sbit_','');
      item=(db.itens||[]).find(x=>String(x.supabase_id||'')===sid || String(x.id||'')==='sbit_'+sid);
      if(item)return item;
    }
    if(rsid){
      item=(db.itens||[]).find(x=>String(x.supabase_id||'')===rsid || String(x.id||'')==='sbit_'+rsid);
      if(item)return item;
    }
    const nome=String(ref.nome||'').trim().toLowerCase();
    const cat=String(ref.categoria||'').trim().toLowerCase();
    if(nome&&cat){
      item=(db.itens||[]).find(x=>String(x.nome||'').trim().toLowerCase()===nome && String(x.categoria||'').trim().toLowerCase()===cat);
      if(item)return item;
    }
    return null;
  }
  function validarEstoqueItens(itens){
    const agrupado=new Map();
    for(const i of (itens||[])){
      const item=encontrarItemEstoque(i);
      if(!item){toast('Item não encontrado no estoque: '+(i.nome||'item'));return false}
      if(item.ativo===false){toast('Item indisponível: '+item.nome);return false}
      if(!controlaEstoqueNumerico(item))continue;
      const chave=item.supabase_id?('sb|'+item.supabase_id):chaveEstoqueItem(item);
      const atual=agrupado.get(chave)||{item,uso:0};
      atual.uso+=Number(i.qtdUsada||1);
      agrupado.set(chave,atual);
    }
    for(const {item,uso} of agrupado.values()){
      if(Number(item.estoque||0)<uso){toast(estoqueDisponivelTexto(item));return false}
    }
    return true;
  }

  async function validarEstoqueItensAtualizado(itens, mensagem=true){
    try{
      if(supabaseAtivo&&supabaseAtivo()){
        await supabaseCarregarItens();
        if(isPublicClient())limparSelecoesIndisponiveisPortal(true);
      }
    }catch(e){console.warn('Atualizar estoque antes de validar pedido:',e)}
    const agrupado=new Map();
    for(const i of (itens||[])){
      const item=encontrarItemEstoque(i);
      if(!item){if(mensagem)toast('Item não encontrado no cardápio/estoque: '+(i.nome||'item'));return false}
      if(item.ativo===false){if(mensagem)toast('O item ficou indisponível enquanto o pedido era montado: '+item.nome);return false}
      if(!controlaEstoqueNumerico(item))continue;
      const chave=item.supabase_id?('sb|'+item.supabase_id):chaveEstoqueItem(item);
      const atual=agrupado.get(chave)||{item,uso:0};
      atual.uso+=Number(i.qtdUsada||1);
    ...
    for(const {item,uso} of agrupado.values()){
      if(Number(item.estoque||0)<uso){if(mensagem)toast(estoqueDisponivelTexto(item));return false}
    }
    return true;
  }

  async function devolverEstoquePedido(p){
    const alterados=[];
    (p?.itens||[]).forEach(i=>{
      const item=encontrarItemEstoque(i);
      if(item&&controlaEstoqueNumerico(item)){
        item.estoque=Number(item.estoque||0)+Number(i.qtdUsada||1);
        if(!alterados.includes(item))alterados.push(item);
      }
    });
    if(alterados.length){
      const ok=await supabaseAtualizarEstoqueItens(alterados);
      if(ok===false)throw new Error('Não foi possível devolver estoque no Supabase');
    }
    return alterados;
  }

  function toggleItem(id){const item=db.itens.find(i=>i.id===id);if(!item)return;if(!itemDisponivelVenda(item)&&!isSelected(item))return toast(controlaEstoqueNumerico(item)?'Item sem estoque':'Item indisponível');if(multiple(item.categoria)){pedido.itens[item.categoria]=pedido.itens[item.categoria]||[];const arr=pedido.itens[item.categoria];if(arr.includes(id)){pedido.itens[item.categoria]=arr.filter(x=>x!==id);delete pedido.qtd[id]}else{pedido.itens[item.categoria]=[...arr,id];pedido.qtd[id]=defaultQtd(item)}}else{if(pedido.itens[item.categoria]===id){pedido.itens[item.categoria]=null;delete pedido.qtd[id]}else{pedido.itens[item.categoria]=id;pedido.qtd[id]=defaultQtd(item)}}if(el('compraData')&&!el('compraData').value)el('compraData').value=hoje();if(el('insightDataIni')&&!el('insightDataIni').value)el('insightDataIni').value=hojeLocal();if(el('insightDataFim')&&!el('insightDataFim').value)el('insightDataFim').value=hojeLocal();renderAll();aplicarModoOperacao()}
  function setQtdItem(id,q){const item=db.itens.find(i=>i.id===id);if(!item)return;const n=normalizarQtd(item,q);if(controlaEstoqueNumerico(item)&&n>Number(item.estoque||0))toast('Atenção: quantidade maior que o estoque atual');pedido.qtd[id]=n;renderResumo();renderProgress();renderSteps()}
  function setPorcaoItem(id,delta){const item=db.itens.find(i=>i.id===id);if(!item)return;if(['Massa','Molho','Finalização'].includes(item.categoria))return;const atual=porcoesPedidoItem(item);const prox=Math.max(1,atual+Number(delta||0));const consumo=prox*tamanhoPorcaoEstoque(item);if(controlaEstoqueNumerico(item)&&consumo>Number(item.estoque||0))toast('Atenção: quantidade maior que o estoque atual');pedido.qtd[id]=consumo;renderResumo();renderProgress();renderSteps()}
  function selectedItems(){const tamanho=tamanhoPedidoAtual('interno');return Object.entries(pedido.itens).flatMap(([cat,v])=>Array.isArray(v)?v.map(id=>db.itens.find(i=>i.id===id)).filter(Boolean):[db.itens.find(i=>i.id===v)].filter(Boolean)).map(i=>({...i,qtdUsada:i.categoria==='Massa'&&Number(tamanho?.massaGramas)>0?Number(tamanho.massaGramas):Number(pedido.qtd[i.id]||defaultQtd(i)),porcao:Number(i.porcao||1)}))}
  function applyPricesComTamanho(sel,tamanho){
    const limites={Complemento:Number(tamanho?.complementos??db.config.complementosGratis??0), 'Proteína':Number(tamanho?.proteinas??db.config.proteinasGratis??0), 'Finalização':limiteGratisCategoria('Finalização')};
    const usados={Complemento:0, 'Proteína':0, 'Finalização':0};
    return sel.map(i=>{
      const basePorcao=Math.max(tamanhoPorcaoEstoque(i),0.0001);
      const mult=Math.max(1,Number(i.qtdUsada||basePorcao)/basePorcao);
      const porcoes=porcoesItem(i);
      let porcoesGratis=0;
      if(i.categoria==='Complemento'||i.categoria==='Proteína'||i.categoria==='Finalização'){
        const limite=limites[i.categoria];
        const rest=limite===Infinity?Infinity:Math.max(0,(limite||0)-(usados[i.categoria]||0));
      ...
      const valorCobrado=(i.categoria==='Massa'||i.categoria==='Molho')?0:((i.categoria==='Complemento'||i.categoria==='Proteína'||i.categoria==='Finalização')?Number(i.preco||0)*porcoesPagas:Number(i.preco||0)*mult);
      return{...i,porcoes,porcoesGratis,porcoesPagas,gratis,valorCobrado,custoCalculado:Number(i.custo||0)*mult}
    })
  }
  function applyPrices(sel,contexto='interno'){return applyPricesComTamanho(sel,tamanhoPedidoAtual(contexto))}
  function tamanhoDoPratoSalvo(pr){
    if(!pr)return tamanhosAtivos()[0]||null;
    const snap=pr.tamanho||pr.tamanhoSnapshot||null;
    const id=snap?.id||pr.tamanhoId||'';
    const nome=snap?.nome||pr.tamanhoNome||'';
    let t=(id?tamanhoPorId(id):null)||(nome?(db.config.tamanhosPrato||[]).find(x=>String(x.nome||'').trim().toLowerCase()===String(nome).trim().toLowerCase()):null);
    if(!t&&snap)t={...snap};
    if(!t){
      const massa=(pr.itens||[]).find(i=>(i.categoria||i.tipo)==='Massa');
      const qtd=Number(massa?.qtdUsada||massa?.quantidade||massa?.qtd||0);
      if(qtd>0){
        t=[...(db.config.tamanhosPrato||[])].sort((a,b)=>Math.abs(Number(a.massaGramas||0)-qtd)-Math.abs(Number(b.massaGramas||0)-qtd))[0]||null;
      }
    }
    if(!t&&Number(pr.total||0)>0){
      t=[...(db.config.tamanhosPrato||[])].sort((a,b)=>Math.abs(Number(a.preco||0)-Number(pr.total||0))-Math.abs(Number(b.preco||0)-Number(pr.total||0)))[0]||null;
    }
    return t||tamanhosAtivos()[0]||null;
  }
  function recalcularPratoSalvo(pr,idx=0){
    const tamanhoSalvo=tamanhoDoPratoSalvo(pr);
    const itensBase=(pr?.itens||[]).map(salvo=>{
      const atual=db.itens.find(i=>i.id===salvo.id)||db.itens.find(i=>i.nome===salvo.nome&&i.categoria===salvo.categoria);
      const base=atual?{...salvo,...atual}:{...salvo};
      const qtd=Number(salvo.qtdUsada||salvo.qtd||salvo.quantidadeUsada||defaultQtd(base));
      return {...base,qtdUsada:qtd,porcao:Number(base.porcao||salvo.porcao||1)};
    });
    const itens=applyPricesComTamanho(itensBase,tamanhoSalvo);
    const temBase=itens.some(i=>['Massa','Molho','Proteína','Complemento','Finalização'].includes(i.categoria));
    const total=(temBase?Number(tamanhoSalvo?.preco||0):0)+itens.reduce((a,i)=>a+Number(i.valorCobrado||0),0);
    const custo=itens.reduce((a,i)=>a+Number(i.custoCalculado||0),0);
    return {...pr,nome:pr?.nome||('Prato '+(idx+1)),itens,total,custo,tamanho:snapshotTamanho(tamanhoSalvo)};
  }
  function pricedItems(){return applyPrices(selectedItems(),'interno')}
  function currentPrato(){const itens=pricedItems();const idx=Number(pedido.pratoEditandoIndex);const nome=idx>=0&&pedido.pratos[idx]?pedido.pratos[idx].nome:'Prato '+(pedido.pratos.length+1);const tamanho=tamanhoPedidoAtual('interno');const temPrato=itens.some(i=>['Massa','Molho','Proteína','Complemento','Finalização'].includes(i.categoria));const base=temPrato?Number(tamanho?.preco||0):0;return{nome,itens,total:base+itens.reduce((a,i)=>a+Number(i.valorCobrado||0),0),custo:itens.reduce((a,i)=>a+Number(i.custoCalculado||0),0),tamanho:snapshotTamanho(tamanho)}}
  function pedidoTotal(){syncPedido();const editIdx=Number(pedido.pratoEditandoIndex);let t=pedido.pratos.reduce((a,p,idx)=>a+(idx===editIdx?0:Number(p.total||0)),0);const atual=currentPrato();if(atual.itens.length)t+=atual.total;if(pedido.tipo==='Delivery'&&!pedido.freteGratis)t+=freteAtual('pedido');return t}
  function pedidoCusto(){const editIdx=Number(pedido.pratoEditandoIndex);let c=pedido.pratos.reduce((a,p,idx)=>a+(idx===editIdx?0:Number(p.custo||0)),0);const atual=currentPrato();if(atual.itens.length)c+=atual.custo;return c}
  function taxaPagamento(forma){const f=String(forma||'').toLowerCase();if(f.includes('débito')||f.includes('debito'))return 0.0139;if(f.includes('crédito')||f.includes('credito'))return 0.0379;return 0}
  function resumoPagamento(valor,forma){const bruto=Number(valor||0),taxaPct=taxaPagamento(forma),taxa=Math.round(bruto*taxaPct*100)/100,liquido=Math.round((bruto-taxa)*100)/100;return{bruto,taxaPct,taxa,liquido}}
  function valorLiquidoPedido(p){return resumoPagamento(p?.total,p?.pagamento||p?.pagamentoPrevisto||'PIX').liquido}
  function atualizarResumoPagamento(){const p=db.pedidos.find(x=>x.id===window.pagamentoPedidoAbertoId);if(!p)return;const r=resumoPagamento(p.total,val('pagamentoFinal')||p.pagamentoPrevisto||'PIX');if(el('pagamentoTaxaLinha'))el('pagamentoTaxaLinha').innerHTML=`<span>Taxa da maquininha ${r.taxaPct?`(${(r.taxaPct*100).toFixed(2).replace('.',',')}%)`:''}</span><strong>${r.taxaPct?'- '+fmt(r.taxa):fmt(0)}</strong>`;if(el('pagamentoLiquidoLinha'))el('pagamentoLiquidoLinha').innerHTML=`<strong>Saldo líquido no financeiro</strong><strong>${fmt(r.liquido)}</strong>`}
  function pedidosPagos(){return db.pedidos.filter(p=>p.financeiroLancado||p.pagamentoRecebido===true)}
  function movimentosFinanceiros(){
    const movs=[];
    (db.pedidos||[]).filter(p=>p.financeiroLancado||p.pagamentoRecebido===true).forEach(p=>{
      const forma=p.pagamento||p.pagamentoPrevisto||'PIX',r=resumoPagamento(p.total,forma);
      movs.push({id:'auto_pedido_'+(p.id||p.onlineId||p.numero),tipo:'entrada',desc:'Pedido #'+(p.numero||'')+' ('+forma+')'+(r.taxaPct?' • taxa '+fmt(r.taxa):''),valor:r.liquido,valorBruto:r.bruto,taxaMaquininha:r.taxa,data:p.dataPagamento||p.data||new Date().toISOString(),origem:'pedido',pedidoId:p.id});
    });
    (db.compras||[]).filter(c=>c.financeiro!==false).forEach(c=>{
      movs.push({id:'auto_compra_'+(c.id||c.onlineId||c.produto),tipo:'saida',desc:'Compra: '+(c.produto||'')+(c.local?' • '+c.local:''),valor:Number(c.valor||0),data:c.data?new Date(c.data+'T12:00:00').toISOString():(c.createdAt||new Date().toISOString()),origem:'compra',compraId:c.id});
    });
    (db.financeiro||[]).filter(m=>!['pedido','compra'].includes(m.origem||'')).forEach(m=>movs.push({...m,valor:Number(m.valor||0),tipo:m.tipo||'saida',origem:m.origem||'manual'}));
    const vistos=new Set();
    return movs.filter(m=>{const k=(m.origem||'manual')+'|'+(m.pedidoId||m.compraId||m.onlineId||m.id||m.desc);if(vistos.has(k))return false;vistos.add(k);return true}).sort((a,b)=>new Date(b.data||0)-new Date(a.data||0));
  }
  function validarPrato(itens=selectedItems()){
    ...
    return validarEstoqueItens(itens)
  }
  function limparSelecaoPrato(){pedido.itens={};pedido.qtd={};pedido.pratoEditandoIndex=-1;if(el('compraData')&&!el('compraData').value)el('compraData').value=hoje();if(el('insightDataIni')&&!el('insightDataIni').value)el('insightDataIni').value=hojeLocal();if(el('insightDataFim')&&!el('insightDataFim').value)el('insightDataFim').value=hojeLocal();renderAll();aplicarModoOperacao()}
  function adicionarPratoPedido(){if(Number(pedido.pratoEditandoIndex)>=0)return atualizarPratoPedido();const itens=selectedItems();if(!validarPrato(itens))return;pedido.pratos.push(currentPrato());limparSelecaoPrato();toast('Prato adicionado ao pedido')}
  function carregarPratoPedido(idx){const prato=pedido.pratos[idx];if(!prato)return;const tamanhoCarregado=tamanhoDoPratoSalvo(prato);pedido.tamanhoId=tamanhoCarregado?.id||pedido.tamanhoId||tamanhosAtivos()[0]?.id||'';pedido.itens={};pedido.qtd={};(prato.itens||[]).forEach(salvo=>{const item=db.itens.find(i=>i.id===salvo.id)||db.itens.find(i=>i.nome===salvo.nome&&i.categoria===salvo.categoria);if(!item)return;if(multiple(item.categoria)){pedido.itens[item.categoria]=pedido.itens[item.categoria]||[];pedido.itens[item.categoria].push(item.id)}else pedido.itens[item.categoria]=item.id;pedido.qtd[item.id]=Number(salvo.qtdUsada||salvo.qtd||defaultQtd(item))});pedido.pratoEditandoIndex=idx;renderAll();document.getElementById('steps')?.scrollIntoView({behavior:'smooth',block:'start'});toast((prato.nome||('Prato '+(idx+1)))+' carregado para edição')}
  function atualizarPratoPedido(){const idx=Number(pedido.pratoEditandoIndex);if(idx<0||!pedido.pratos[idx])return toast('Selecione o prato que deseja atualizar');const itens=selectedItems();if(!validarPrato(itens))return;pedido.pratos[idx]=currentPrato();limparSelecaoPrato();toast('Prato atualizado no pedido')}
  ...
  function baixarEstoque(itens){const alterados=[];(itens||[]).forEach(i=>{const item=encontrarItemEstoque(i);if(item&&controlaEstoqueNumerico(item)){item.estoque=Number(item.estoque||0)-Number(i.qtdUsada||1);alterados.push(item)}});if(alterados.length)supabaseAtualizarEstoqueItens(alterados)}
  function devolverEstoque(itens){(itens||[]).forEach(i=>{const item=encontrarItemEstoque(i);if(item&&controlaEstoqueNumerico(item))item.estoque=Number(item.estoque||0)+Number(i.qtdUsada||1)})}
  ...
  function selecionarClientePedido(id){const c=db.clientes.find(x=>x.id===id);if(!c)return;pedido.clienteId=c.id;el('clientePedido').value=c.nome||'';el('telefonePedido').value=c.telefone||'';[['pedidoCep','cep'],['pedidoRua','rua'],['pedidoNumeroEndereco','numero'],['pedidoComplemento','complemento'],['pedidoBairro','bairro'],['bairroPedido','bairro'],['pedidoCidade','cidade'],['pedidoUf','uf']].forEach(([id,k])=>{if(el(id))el(id).value=c[k]||''});if(el('enderecoPedido'))el('enderecoPedido').value=c.endereco||enderecoCompleto(c);const box=el('clientesEncontrados');if(box){box.className='client-results hidden';box.innerHTML=''}toast('Cliente selecionado. Alterações neste pedido não mudam o cadastro.')}
  ...
  function selecionarTamanhoPedido(id,contexto='interno'){const alvo=contexto==='portal'?portalPedido:pedido;alvo.tamanhoId=id;const t=tamanhoPorId(id);if(!t)return;if(contexto==='portal')renderPortalCliente();else renderAll();toast(`Tamanho ${t.nome} selecionado`)}
  function renderSeletorTamanhos(contexto='interno'){const box=el(contexto==='portal'?'portalTamanhos':'tamanhoPedidoBox');if(!box)return;const ativos=tamanhosAtivos();if(!ativos.length){box.innerHTML='<div class="empty-summary">Nenhum tamanho ativo.</div>';return;}const atual=tamanhoPedidoAtual(contexto);box.innerHTML=`<div class="size-selector-title"><strong>📏 Escolha o tamanho</strong><small>O valor inclui massa, molho e as escolhas indicadas.</small></div><div class="size-options">${ativos.map(t=>`<button type="button" class="size-option ${String(atual?.id)===String(t.id)?'selected':''}" onclick="App.selecionarTamanhoPedido('${t.id}','${contexto}')"><span><strong>${escapeHtmlPrato(t.nome)}</strong><b>${fmt(t.preco)}</b></span><small>${escapeHtmlPrato(t.pesoAprox||'')}</small><em>${escapeHtmlPrato(t.descricao||`Até ${t.proteinas} proteínas e ${t.complementos} complementos`)}</em></button>`).join('')}</div>`}
  function renderTamanhosPrato(){const box=el('listaTamanhosPrato');if(!box)return;const lista=db.config.tamanhosPrato||[];box.innerHTML=lista.map((t,idx)=>`<div class="size-admin-card" data-size-index="${idx}"><div class="row"><div class="field"><label>Nome</label><input data-k="nome" value="${escapeHtmlPrato(t.nome||'')}"></div><div class="field"><label>Preço</label><input data-k="preco" type="number" min="0" step="0.01" value="${Number(t.preco||0)}"></div><div class="field"><label>Ordem</label><input data-k="ordem" type="number" min="1" value="${Number(t.ordem||idx+1)}"></div></div><div class="row"><div class="field"><label>Peso aproximado</label><input data-k="pesoAprox" value="${escapeHtmlPrato(t.pesoAprox||'')}"></div><div class="field"><label>Massa crua (g)</label><input data-k="massaGramas" type="number" min="0" value="${Number(t.massaGramas||0)}"></div></div><div class="field"><label>Descrição exibida ao cliente</label><textarea data-k="descricao" rows="2">${escapeHtmlPrato(t.descricao||'')}</textarea></div><div class="row"><div class="field"><label>Proteínas incluídas</label><input data-k="proteinas" type="number" min="0" value="${Number(t.proteinas||0)}"></div><div class="field"><label>Complementos incluídos</label><input data-k="complementos" type="number" min="0" value="${Number(t.complementos||0)}"></div><label class="check-line"><input data-k="ativo" type="checkbox" ${t.ativo!==false?'checked':''}> Ativo</label><button class="mini danger-btn" onclick="App.removerTamanhoPrato(${idx})">Remover</button></div></div>`).join('')||'<p class="muted">Nenhum tamanho cadastrado.</p>'}
  function lerTamanhosFormulario(){return [...document.querySelectorAll('.size-admin-card')].map((card,idx)=>{const get=k=>card.querySelector(`[data-k="${k}"]`);const atual=(db.config.tamanhosPrato||[])[idx]||{};return{id:atual.id||uid(),nome:get('nome').value.trim()||`Tamanho ${idx+1}`,preco:Number(get('preco').value||0),ordem:Number(get('ordem').value||idx+1),pesoAprox:get('pesoAprox').value.trim(),massaGramas:Number(get('massaGramas').value||0),descricao:get('descricao').value.trim(),proteinas:Number(get('proteinas').value||0),complementos:Number(get('complementos').value||0),ativo:!!get('ativo').checked}})}
  function adicionarTamanhoPrato(){db.config.tamanhosPrato=db.config.tamanhosPrato||[];db.config.tamanhosPrato.push({id:uid(),nome:'Novo tamanho',preco:0,pesoAprox:'',descricao:'',massaGramas:100,proteinas:2,complementos:4,ativo:true,ordem:db.config.tamanhosPrato.length+1});renderTamanhosPrato()}
  function removerTamanhoPrato(idx){if((db.config.tamanhosPrato||[]).length<=1)return toast('Mantenha pelo menos um tamanho.');if(!confirm('Remover este tamanho?'))return;db.config.tamanhosPrato.splice(idx,1);renderTamanhosPrato()}
  async function salvarTamanhosPrato(){const lista=lerTamanhosFormulario();if(!lista.length||!lista.some(t=>t.ativo))return toast('Mantenha ao menos um tamanho ativo.');db.config.tamanhosPrato=lista;const padrao=tamanhosAtivos()[0];db.config.proteinasGratis=Number(padrao?.proteinas||0);db.config.complementosGratis=Number(padrao?.complementos||0);save();await supabaseSalvarLoja();renderTamanhosPrato();toast('Tamanhos salvos')}

  function renderSteps(){const root=el('steps');if(!root)return;root.innerHTML=categorias().map(cat=>{const itens=db.itens.filter(i=>i.categoria===cat&&i.ativo!==false);if(!itens.length)return'';const regra=(['Proteína','Complemento'].includes(cat)?`Inclui ${limiteCategoriaTamanho(cat,'interno')} • adicionais cobrados à parte`:labelGratisCategoria(cat))||((cat==='Bebida'||cat==='Sobremesa')?'pode adicionar avulso':multiple(cat)?'seleção múltipla':'escolha 1');return `<section class="step"><div class="step-head"><h3>${iconeCat(cat)} ${cat}</h3><small>${regra}</small></div><div class="option-grid">${itens.map(i=>{const low=!itemDisponivelVenda(i),sel=isSelected(i);const porcoes=porcoesPedidoItem(i);return `<div role="button" tabindex="0" class="option ${sel?'selected':''} ${low?'low':''}" onclick="App.toggleItem('${i.id}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();App.toggleItem('${i.id}')}">${(cat==='Complemento'||cat==='Proteína')&&sel?'<em class="free-tag">regra por porção</em>':''}<div class="icon">${i.imagem?`<img src="${i.imagem}" alt="${i.nome}">`:(i.icone||iconeCat(cat))}</div><strong>${i.nome}</strong><span>${fmt(i.preco)}</span><small class="stock-mini">${controlaEstoqueNumerico(i)?'Estoque: '+fmtQtd(i.estoque)+' '+(i.unidade||'un'):'Disponível'}</small>${sel&&!['Massa','Molho','Finalização'].includes(cat)?`<div class="qty-inline qty-stepper" onclick="event.stopPropagation()"><button type="button" aria-label="Diminuir" onclick="App.setPorcaoItem('${i.id}',-1)">−</button><strong>${porcoes}</strong><button type="button" aria-label="Aumentar" onclick="App.setPorcaoItem('${i.id}',1)">+</button></div>`:''}${sel&&['Massa','Molho','Finalização'].includes(cat)?`<small class="stock-mini">Porção fixa</small>`:''}</div>`}).join('')}</div></section>`}).join('');renderProgress()}
  ...
  function renderResumo(){syncPedido();el('deliveryBox')?.classList.toggle('hidden',pedido.tipo!=='Delivery');if(pedido.tipo!=='Delivery')fretePedido={status:'idle',distancia:0,valor:0,mensagem:'Retirada selecionada: sem frete.'};atualizarFreteStatus('pedido',fretePedido);if(pedido.tipo==='Delivery'&&pedido.freteGratis&&el('pedidoFreteStatus'))el('pedidoFreteStatus').textContent='Frete grátis aplicado neste pedido.';const atual=currentPrato(),box=el('resumoPedido'),editIdx=Number(pedido.pratoEditandoIndex);if(!atual.itens.length){box.className='summary empty-summary';box.textContent=editIdx>=0?'Selecione novamente os itens deste prato':'Selecione os itens do prato'}else{box.className='summary';box.innerHTML=(editIdx>=0?`<div class="editing-plate-label">Editando ${pedido.pratos[editIdx]?.nome||('Prato '+(editIdx+1))}</div>`:'')+atual.itens.map(i=>`<div class="summary-item"><div><strong>${i.nome}</strong><br><small>${i.categoria} • ${i.porcoes||porcoesItem(i)}${(i.categoria==='Complemento'||i.categoria==='Proteína'||i.categoria==='Finalização')?` • ${i.porcoesGratis||0} grátis / ${i.porcoesPagas||0} paga(s)`:''}</small></div><span class="${i.valorCobrado===0?'discount':''}">${i.valorCobrado===0?'Grátis':fmt(i.valorCobrado)}</span></div>`).join('')}const pb=el('pratosPedido');if(pb){pb.classList.toggle('hidden',pedido.pratos.length===0);pb.innerHTML=pedido.pratos.map((p,idx)=>`<div class="plate-chip ${idx===editIdx?'editing-plate':''}" onclick="App.carregarPratoPedido(${idx})" role="button" tabindex="0"><strong>${p.nome}</strong><span>${p.itens.map(i=>i.nome).join(', ')}</span><b>${fmt(p.total)}</b><button onclick="event.stopPropagation();App.removerPratoPedido(${idx})" type="button">×</button></div>`).join('')}const btn=el('btnAdicionarPrato');if(btn){btn.textContent=editIdx>=0?'Atualizar este prato':'Adicionar prato ao pedido';btn.onclick=editIdx>=0?atualizarPratoPedido:adicionarPratoPedido}el('totalPedido').textContent=fmt(pedidoTotal())}
  function removerPratoPedido(idx){pedido.pratos.splice(idx,1);if(Number(pedido.pratoEditandoIndex)===idx){pedido.itens={};pedido.qtd={};pedido.pratoEditandoIndex=-1}else if(Number(pedido.pratoEditandoIndex)>idx)pedido.pratoEditandoIndex--;renderAll();toast('Prato removido do pedido')}

  function freteGratisMarcador(p){if(!p?.freteGratis)return '';const original=Number(p.freteOriginal||0);return `[FRETE_GRATIS:${original.toFixed(2)}]`}
  function extrairFreteGratis(obs=''){const txt=String(obs||'');const m=txt.match(/\[FRETE_GRATIS:([0-9]+(?:[.,][0-9]+)?)\]/);return {freteGratis:!!m,freteOriginal:m?Number(String(m[1]).replace(',','.')):0,obs:txt.replace(/\[FRETE_GRATIS:[^\]]+\]/g,'').trim()}}
  function codificarSnapshotPedido(p){
    try{
      const pratos=((p?.pratos&&p.pratos.length)?p.pratos:[{nome:'Pedido 1',itens:p?.itens||[],total:Number(p?.subtotalBruto||p?.total||0),custo:Number(p?.custo||0)}]).map((pr,idx)=>({
        nome:pr.nome||('Pedido '+(idx+1)),total:Number(pr.total||0),custo:Number(pr.custo||0),
        tamanhoId:(pr.tamanho?.id||pr.tamanhoId||pr.tamanhoSnapshot?.id||''),tamanhoNome:(pr.tamanho?.nome||pr.tamanhoNome||pr.tamanhoSnapshot?.nome||''),tamanhoSnapshot:(pr.tamanho||pr.tamanhoSnapshot||null),tamanho:(pr.tamanho||pr.tamanhoSnapshot||null),
        itens:(pr.itens||[]).map(i=>({id:i.id||'',nome:i.nome||'',tipo:i.tipo||i.categoria||'Item',categoria:i.categoria||i.tipo||'Item',quantidade:Number(i.quantidade||i.qtdUsada||1),qtdUsada:Number(i.qtdUsada||i.quantidade||1),porcoes:Number(i.porcoes||0),porcoesGratis:Number(i.porcoesGratis||0),porcoesPagas:Number(i.porcoesPagas||0),preco:Number(i.preco||0),valorCobrado:Number(i.valorCobrado??i.preco??0),custoCalculado:Number(i.custoCalculado||0),unidadeCombo:Number(i.unidadeCombo||0),unidadeComboNome:i.unidadeComboNome||''}))
      }));
      const json=JSON.stringify({v:1,pratos});
      const bytes=new TextEncoder().encode(json);let bin='';bytes.forEach(b=>bin+=String.fromCharCode(b));
      return btoa(bin).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');
    }catch(e){console.warn('Snapshot do pedido não pôde ser criado:',e);return ''}
  }
  function extrairSnapshotPedido(obs=''){
    const txt=String(obs||'');const m=txt.match(/\[DONAS_PRATOS:([A-Za-z0-9_-]+)\]/);
    if(!m)return {snapshot:null,obs:txt};
    try{let b64=m[1].replace(/-/g,'+').replace(/_/g,'/');while(b64.length%4)b64+='=';const bin=atob(b64);const bytes=Uint8Array.from(bin,c=>c.charCodeAt(0));const snapshot=JSON.parse(new TextDecoder().decode(bytes));return {snapshot,obs:txt.replace(m[0],'').trim()}}catch(e){console.warn('Snapshot do pedido inválido:',e);return {snapshot:null,obs:txt.replace(m[0],'').trim()}}
  }
  function snapshotMarcadorPedido(p){const cod=codificarSnapshotPedido(p);return cod?'[DONAS_PRATOS:'+cod+']':''}
  function observacaoPersistidaPedido(p){return [reservaMarcador(p),freteGratisMarcador(p),snapshotMarcadorPedido(p),p.obs||''].filter(Boolean).join(' ')}
  function alternarFreteGratis(){syncPedido();renderResumo()}
  function reservaMarcador(p){if(!(p?.atendimento==='reserva'&&p?.reservaData&&p?.reservaHorario))return '';return `[RESERVA:${p.reservaData}|${p.reservaHorario}]${p.reservaIniciada===true?' [RESERVA_INICIADA]':''}`}
  function extrairReserva(obs=''){const txt=String(obs||'');const m=txt.match(/\[RESERVA:(\d{4}-\d{2}-\d{2})\|([0-2]\d:[0-5]\d)\]/);const iniciada=txt.includes('[RESERVA_INICIADA]');return m?{atendimento:'reserva',reservaData:m[1],reservaHorario:m[2],reservaIniciada:iniciada,obs:txt.replace(m[0],'').replace('[RESERVA_INICIADA]','').trim()}:{atendimento:'imediato',reservaData:'',reservaHorario:'',reservaIniciada:true,obs:txt.replace('[RESERVA_INICIADA]','').trim()}}
  function alternarCamposReserva(){const reserva=val('atendimentoPedido')==='reserva';el('camposReservaPedido')?.classList.toggle('hidden',!reserva);if(reserva&&el('reservaDataPedido')&&!el('reservaDataPedido').value)el('reservaDataPedido').value=hoje()}
  async function finalizarPedido(){
    syncPedido();
    if(pedido.atendimento==='reserva'&&(!pedido.reservaData||!pedido.reservaHorario))return toast('Informe o dia e o horário da reserva');
    let pratos=[...pedido.pratos];
    const atual=currentPrato();
    if(atual.itens.length){if(!validarPrato(selectedItems()))return;pratos.push(atual)}
    if(!pratos.length)return toast('Adicione ao menos um prato ao pedido');
    const todos=pratos.flatMap(p=>p.itens);
    const original=pedido.editandoId?db.pedidos.find(x=>x.id===pedido.editandoId):null;
    const originalSnapshot=original?deepCopy(original):null;
    const itensSnapshot=original?deepCopy(db.itens):null;
    if(original)devolverEstoque(original.itens||[]);
    const estoqueOk=await validarEstoqueItensAtualizado(todos);
    if(!estoqueOk){if(original)baixarEstoque(original.itens||[]);return}
    const subtotal=pratos.reduce((a,p)=>a+p.total,0);
    const freteCalculado=pedido.tipo==='Delivery'?freteAtual('pedido'):0;const entrega=(pedido.tipo==='Delivery'&&!pedido.freteGratis)?freteCalculado:0;
    const bruto=subtotal+entrega;
    if(pedido.cupom&&supabaseLojaId)await supabaseCarregarCupons();
    const cup=usarCupomSeValido(pedido.cupom,bruto);
    if(pedido.cupom&&!cup.ok){if(original)baixarEstoque(original.itens||[]);return toast(cup.msg||'Cupom inválido')}
    const desconto=Number(cup.desconto||0),totalFinal=Math.max(0,bruto-desconto),custo=pratos.reduce((a,p)=>a+p.custo,0);
    const numero=pedido.editandoNumero||await proximoNumeroPedidoSeguro(),pago=pedido.pagamentoRecebido===true,agora=new Date().toISOString();
    const editando=!!original;
    const p={
      ...(original||{}),id:pedido.editandoId||uid(),onlineId:pedido.editandoOnlineId||('ADM-'+hoje().replace(/-/g,'')+'-'+numero),numero,
      cliente:pedido.cliente||'Cliente balcão',telefone:pedido.telefone,tipo:pedido.tipo,atendimento:pedido.atendimento,
      reservaData:pedido.reservaData,reservaHorario:pedido.reservaHorario,
      // Ao transformar um pedido da fila em reserva (ou reagendar uma reserva já iniciada),
      // ele volta para a aba Reservados e só retorna à fila após "Iniciar pedido".
      reservaIniciada:pedido.atendimento==='reserva'?(original?.atendimento==='reserva'&&original?.reservaIniciada!==true?false:(original?.atendimento==='reserva'&&original?.reservaIniciada===true&&original?.reservaData===pedido.reservaData&&original?.reservaHorario===pedido.reservaHorario)):true,
      pagamento:pago?(pedido.pagamentoPrevisto||'PIX'):(original?.pagamento||''),pagamentoPrevisto:pedido.pagamentoPrevisto||'PIX',pagamentoRecebido:pago,
      obs:pedido.obs,cep:pedido.cep,rua:pedido.rua,numeroEndereco:pedido.numero,complemento:pedido.complemento,endereco:pedido.endereco,bairro:pedido.bairro,cidade:pedido.cidade,uf:pedido.uf,clienteId:pedido.clienteId||'',
      pratos,itens:todos,total:totalFinal,subtotalBruto:bruto,desconto,cupom:cup.codigo||'',cupomId:cup.cupom?.id||'',custo,lucro:totalFinal-custo,freteGratis:pedido.tipo==='Delivery'&&pedido.freteGratis,freteOriginal:pedido.tipo==='Delivery'?(pedido.freteGratis?freteCalculado:entrega):0,
      status:original?.status||'Pedido Feito',financeiroLancado:pago,dataPagamento:pago?(original?.dataPagamento||agora):'',estoqueBaixado:true,origem:original?.origem||'interno',data:original?.data||agora,
      timeline:[...(original?.timeline||[]),{status:editando?'Pedido editado':'Pedido Feito',data:agora,pagamentoRecebido:pago}]
    };
    baixarEstoque(todos);
    if(editando){
      const idx=db.pedidos.findIndex(x=>x.id===original.id);
      if(idx>=0)db.pedidos[idx]=p;
      const mov=db.financeiro.find(m=>m.origem==='pedido'&&m.pedidoId===p.id);if(mov)mov.valor=valorLiquidoPedido(p);
    }else db.pedidos.unshift(p);
    log(editando?'Pedido editado':'Pedido criado','#'+p.numero+' • '+fmt(p.total)+(desconto?' • cupom '+cup.codigo:'')+(pago?' • pago':' • pagamento pendente'));
    salvarClienteAutomatico(p);limparPedido(false);save();
    const enviado=editando?await supabaseAtualizarPedido(p):await supabaseEnviarPedido(p);
    if(enviado===false&&editando&&originalSnapshot){const idx=db.pedidos.findIndex(x=>x.id===p.id);if(idx>=0)db.pedidos[idx]=deepCopy(originalSnapshot);if(itensSnapshot)db.itens=deepCopy(itensSnapshot);save();renderAll();return toast('A edição não foi confirmada no banco e foi desfeita para evitar divergência.')}
    if(enviado!==false&&editando){await supabaseCarregarPedidos();}
    if(enviado!==false&&cup.codigo&&!editando){const usoOk=await registrarUsoCupom(cup.codigo);if(!usoOk)toast('Pedido salvo, mas não consegui atualizar o uso do cupom. Confira no ADM.')}
    editBuffer=null;renderAll();
    toast(editando?'Pedido atualizado sem perder a posição na fila.':(p.atendimento==='reserva'?'Reserva registrada. Inicie pela aba Reservados no horário correto.':(pago?'Pedido registrado e contabilizado no financeiro.':'Pedido registrado. Aguardando preparo e pagamento.')))
  }
  function salvarClienteAutomatico(p){registrarClientePendente(p)}
  function registrarClientePendente(p){if(p.clienteId)return;if(!p.telefone&&!p.cliente)return;const tel=norm(p.telefone);const existe=encontrarClientePorTelefone(tel);if(existe)return;const pendente=(db.clientesPendentes||[]).find(c=>tel&&telefoneCombinaForte(c.telefone,tel)&&(c.status||'pendente')==='pendente');const dados={nome:p.cliente,telefone:p.telefone,cep:p.cep||'',rua:p.rua||'',numero:p.numeroEndereco||p.numero||'',complemento:p.complemento||'',endereco:p.endereco||'',bairro:p.bairro||'',cidade:p.cidade||'',uf:p.uf||'',obs:'Gerado pelo pedido #'+(p.numero||''),pedidoId:p.id,numeroPedido:p.numero,status:'pendente',updatedAt:new Date().toISOString()};if(pendente)Object.assign(pendente,dados);else{db.clientesPendentes=db.clientesPendentes||[];db.clientesPendentes.unshift({id:uid(),createdAt:new Date().toISOString(),...dados});}}
  async function aprovarClientePendente(id){const c=(db.clientesPendentes||[]).find(x=>x.id===id);if(!c)return;const tel=norm(c.telefone);if(tel&&encontrarClientePorTelefone(tel)){c.status='duplicado';c.atualizadoEm=new Date().toISOString();save();return toast('Já existe cliente com este telefone. Revise antes de aprovar.')}const agora=new Date().toISOString();const oficial={id:uid(),nome:c.nome,telefone:c.telefone,cep:c.cep||'',rua:c.rua||'',numero:c.numero||'',complemento:c.complemento||'',endereco:c.endereco||enderecoCompleto(c),bairro:c.bairro||'',cidade:c.cidade||'',uf:c.uf||'',obs:c.obs||'Aprovado a partir de pedido',createdAt:agora,updatedAt:agora};db.clientes.push(oficial);c.status='aprovado';c.aprovadoEm=agora;c.atualizadoEm=agora;save();await supabaseSalvarClienteOficial(oficial);if(c.supabase_id){try{await supabaseRequest('/clientes_pendentes?id=eq.'+encodeURIComponent(c.supabase_id),{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({status:'aprovado',aprovado_em:agora,atualizado_em:agora})});}catch(e){console.warn('Aprovar pendente Supabase:',e)}}await supabaseCarregarClientesPendentes();toast('Cliente aprovado e cadastrado');renderAll()}
  async function reprovarClientePendente(id){const c=(db.clientesPendentes||[]).find(x=>x.id===id);if(!c)return;if(!confirm('Reprovar este cadastro pendente? O pedido continuará normal.'))return;const agora=new Date().toISOString();c.status='reprovado';c.reprovadoEm=agora;c.atualizadoEm=agora;save();if(c.supabase_id){try{await supabaseRequest('/clientes_pendentes?id=eq.'+encodeURIComponent(c.supabase_id),{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({status:'reprovado',reprovado_em:agora,atualizado_em:agora})});}catch(e){console.warn('Reprovar pendente Supabase:',e)}}await supabaseCarregarClientesPendentes();toast('Cadastro reprovado. O pedido foi mantido.');renderAll()}
  function renderEdicaoPedidoBanner(){const box=el('edicaoPedidoBanner');if(!box)return;const ativo=!!pedido.editandoId;box.classList.toggle('hidden',!ativo);if(ativo){const numero=pedido.editandoNumero||db.pedidos.find(p=>p.id===pedido.editandoId)?.numero||'';const label=el('edicaoPedidoNumero');if(label)label.textContent='#'+numero;}}
  function cancelarEdicaoPedido(automatico=false){if(!pedido.editandoId&&!editBuffer)return;if(!automatico&&!confirm('Sair da edição? O pedido original será mantido sem alterações.'))return;limparPedido(false);if(!automatico)toast('Edição cancelada. O pedido original foi mantido.');}
  function limparPedido(show=true){if(show&&editBuffer&&!confirm('Cancelar a edição? O pedido original será mantido sem alterações.'))return;editBuffer=null;pedido=novoPedido();['clientePedido','telefonePedido','obsPedido','pedidoCep','pedidoRua','pedidoNumeroEndereco','pedidoComplemento','enderecoPedido','pedidoBairro','bairroPedido','pedidoCidade','pedidoUf'].forEach(id=>{if(el(id))el(id).value=''});if(el('tipoPedido'))el('tipoPedido').value='Balcão';if(el('atendimentoPedido'))el('atendimentoPedido').value='imediato';if(el('reservaDataPedido'))el('reservaDataPedido').value='';if(el('reservaHorarioPedido'))el('reservaHorarioPedido').value='';alternarCamposReserva();if(el('pagamentoPedido'))el('pagamentoPedido').value='PIX';if(el('pedidoTrocoPara'))el('pedidoTrocoPara').value='';alternarCamposTroco('pedido');if(el('pagamentoRecebidoPedido'))el('pagamentoRecebidoPedido').value='nao';if(el('pedidoFreteGratis'))el('pedidoFreteGratis').checked=false;const box=el('clientesEncontrados');if(box){box.className='client-results hidden';box.innerHTML=''}renderAll();if(show)toast('Pedido limpo')}
  async function atualizarStatus(id,status){const p=db.pedidos.find(x=>x.id===id);if(!p)return;const anterior={status:p.status,timeline:deepCopy(p.timeline||[])};const agora=new Date().toISOString();p.status=status;p.timeline=p.timeline||[];p.timeline.push({status,data:agora});save();renderAll();const ok=await supabaseAtualizarPedido(p);if(!ok){p.status=anterior.status;p.timeline=anterior.timeline;save();renderAll();return toast('Não foi possível salvar no banco. A alteração foi desfeita para evitar divergência.')}await supabaseCarregarPedidos();renderAll();toast(status==='Entregue'?'Pedido marcado como entregue. Pagamento permanece '+((p.financeiroLancado||p.pagamentoRecebido===true)?'registrado.':'pendente.'):('Status atualizado: '+status))}
  function abrirPagamento(id,entregar=false){const p=db.pedidos.find(x=>x.id===id);if(!p)return;window.pagamentoPedidoAbertoId=id;el('modalContent').innerHTML=`<div class="receipt"><h2>${entregar?'Finalizar':'Registrar pagamento do'} Pedido #${p.numero}</h2><p class="muted">Forma prevista: <b>${p.pagamentoPrevisto||'PIX'}</b>. Para cartão, o financeiro recebe automaticamente o valor líquido após a taxa.</p><div class="field"><label>Forma de pagamento recebida</label><select id="pagamentoFinal" onchange="App.atualizarResumoPagamento()">${PAGAMENTOS.map(pg=>`<option ${pg===(p.pagamento||p.pagamentoPrevisto||'PIX')?'selected':''}>${pg}</option>`).join('')}</select></div><div class="summary-item"><strong>Total cobrado do cliente</strong><strong>${fmt(p.total)}</strong></div><div id="pagamentoTaxaLinha" class="summary-item"></div><div id="pagamentoLiquidoLinha" class="summary-item"></div><small class="hint">Débito: 1,39% • Crédito: 3,79%</small><button class="primary full" onclick="App.confirmarPagamento('${id}',${entregar?'true':'false'})">Confirmar pagamento</button><button class="secondary full" onclick="App.fecharModal()">Cancelar</button></div>`;el('modal').classList.remove('hidden');atualizarResumoPagamento()}
  async function confirmarPagamento(id,entregar=true){const p=db.pedidos.find(x=>x.id===id);if(!p)return;const anterior=deepCopy(p);const pg=val('pagamentoFinal')||p.pagamentoPrevisto||'PIX',r=resumoPagamento(p.total,pg);const agora=new Date().toISOString();if(entregar)p.status='Entregue';p.pagamento=pg;p.pagamentoRecebido=true;p.financeiroLancado=true;p.dataPagamento=p.dataPagamento||agora;p.taxaMaquininha=r.taxa;p.valorLiquido=r.liquido;p.timeline=p.timeline||[];p.timeline.push({status:entregar?'Entregue':'Pagamento recebido',data:agora,pagamento:pg,taxaMaquininha:r.taxa,valorLiquido:r.liquido});save();renderAll();const ok=await supabaseAtualizarPedido(p);if(!ok){Object.assign(p,anterior);save();renderAll();return toast('Não foi possível registrar o pagamento no banco. Tente novamente.')}await supabaseCarregarPedidos();renderAll();fecharModal();toast(r.taxaPct?'Pagamento registrado. Saldo líquido: '+fmt(r.liquido):'Pagamento registrado no financeiro')}
  function registrarPagamento(id){abrirPagamento(id,false)}
  ...
  async function confirmarExcluirPedido(id){const p=db.pedidos.find(x=>x.id===id);if(!p)return;const semRetorno=!!el('excluirSemRetorno')?.checked;const anterior=deepCopy({pedidos:db.pedidos,itens:db.itens,financeiro:db.financeiro});try{if(!semRetorno)await devolverEstoquePedido(p);db.financeiro=db.financeiro.filter(m=>m.pedidoId!==id);db.pedidos=db.pedidos.filter(x=>x.id!==id);save();renderAll();const ok=await supabaseExcluirPedido(p);if(!ok)throw new Error('Supabase não confirmou a exclusão');await supabaseCarregarItens();await supabaseCarregarPedidos();log('Pedido excluído','#'+p.numero+(semRetorno?' • sem retorno ao estoque':' • estoque devolvido'));save();fecharModal();toast(semRetorno?'Pedido excluído sem retorno de estoque':'Pedido excluído e estoque restaurado')}catch(e){db.pedidos=anterior.pedidos;db.itens=anterior.itens;db.financeiro=anterior.financeiro;save();renderAll();return toast('Exclusão desfeita porque não foi confirmada no banco/estoque.')}}

  function escolherIcone(icon){if(icon&&el('itemIcone'))el('itemIcone').value=icon}
  async function salvarItem(){const id=val('itemId'),nome=val('itemNome').trim();if(!nome)return toast('Informe o nome');const snapshot=deepCopy(db.itens||[]);const data={nome,categoria:val('itemCategoria'),preco:num('itemPreco'),custo:num('itemCusto'),estoque:num('itemEstoque'),minimo:num('itemMinimo'),unidade:val('itemUnidade')||'un',porcao:num('itemPorcao')||1,porcaoUnidade:val('itemPorcaoUnidade')||val('itemUnidade')||'un',icone:val('itemIcone')||val('itemIconeLista')||iconeCat(val('itemCategoria')),imagem:val('itemImagem'),obs:val('itemObs'),ativo:el('itemAtivo')?!!el('itemAtivo').checked:true};if(id){data.custo=custoMedioComprasItem(id,data.custo)}let savedId=id;if(id){const idx=db.itens.findIndex(i=>i.id===id);if(idx>=0)db.itens[idx]={...db.itens[idx],...data,updatedAt:new Date().toISOString(),syncStatus:'local'}}else{savedId=uid();db.itens.push({id:savedId,...data,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),syncStatus:'local'})}const itemSalvo=db.itens.find(i=>i.id===savedId);const ok=await supabaseSalvarItem(itemSalvo);if(supabaseLojaId&&!ok){db.itens=snapshot;save();renderAll();return toast('Não salvei o item porque o Supabase recusou a alteração.')}save();await supabaseCarregarItens();limparFormItem(false);renderAll();setTimeout(()=>{const row=document.querySelector(`[data-item-id="${savedId}"]`);if(row){row.classList.add('editing-item');setTimeout(()=>row.classList.remove('editing-item'),1200)}},80);toast('Item salvo no banco')}
  function editarItem(id){const i=db.itens.find(x=>x.id===id);if(!i)return;el('formItemTitulo').textContent='✏️ Editando: '+i.nome;el('formItemTitulo').classList.add('editing-title');el('itemId').value=i.id;el('itemNome').value=i.nome;el('itemCategoria').value=i.categoria;if(el('itemAtivo'))el('itemAtivo').checked=i.ativo!==false;el('itemPreco').value=i.preco||0;el('itemCusto').value=i.custo||0;el('itemEstoque').value=i.estoque||0;el('itemMinimo').value=i.minimo||0;el('itemUnidade').value=i.unidade||'un';if(el('itemPorcao'))el('itemPorcao').value=i.porcao||1;if(el('itemPorcaoUnidade'))el('itemPorcaoUnidade').value=i.porcaoUnidade||i.unidade||'un';el('itemIcone').value=i.icone||'';if(el('itemIconeLista'))el('itemIconeLista').value=i.icone||'';if(el('itemImagem'))el('itemImagem').value=i.imagem||'';el('itemObs').value=i.obs||'';page('estoque');renderListas();setTimeout(()=>{el('itemNome')?.focus();document.querySelector(`[data-item-id="${id}"]`)?.scrollIntoView({block:'nearest',behavior:'smooth'});},80)}
  function limparFormItem(show=true){['itemId','itemNome','itemPreco','itemCusto','itemEstoque','itemMinimo','itemPorcao','itemPorcaoUnidade','itemIcone','itemImagem','itemObs'].forEach(id=>{if(el(id))el(id).value=''});if(el('itemCategoria'))el('itemCategoria').value='Massa';if(el('itemAtivo'))el('itemAtivo').checked=true;if(el('itemUnidade'))el('itemUnidade').value='un';if(el('itemPorcaoUnidade'))el('itemPorcaoUnidade').value='un';if(el('itemIconeLista'))el('itemIconeLista').value='';if(el('formItemTitulo')){el('formItemTitulo').textContent='Cadastrar item';el('formItemTitulo').classList.remove('editing-title')}renderListas();if(show)toast('Formulário limpo')}

  async function alternarDisponibilidadeItem(id){
    ...
    save();
    const ok=await supabaseSalvarItem(item);
    if((supabaseLojaId||supabaseAtivo())&&!ok){db.itens=snapshot;save();renderAll();return toast('Não consegui atualizar a disponibilidade no banco.');}
    await supabaseCarregarItens();
    renderAll();
    toast(item.ativo===false?'Item marcado como indisponível':'Item marcado como disponível');
  }
  ...
  function statusActions(p){let a='';if(p.atendimento==='reserva'&&p.reservaIniciada!==true){const pago=(p.financeiroLancado||p.pagamentoRecebido===true);return `<button class="mini primary" onclick="App.iniciarReserva('${p.id}')">Iniciar pedido</button>${pago?`<span class="pill green">Pago${p.pagamento?' • '+p.pagamento:''}</span>`:`<button class="mini" onclick="App.registrarPagamento('${p.id}')">Marcar pago</button>`}<button class="mini danger-btn" onclick="App.excluirPedido('${p.id}')">excluir</button>`}const pago=(p.financeiroLancado||p.pagamentoRecebido===true);const pay=pago?`<span class="pill green">Pago${p.pagamento?' • '+p.pagamento:''}</span>`:`<button class="mini" onclick="App.registrarPagamento('${p.id}')">Marcar pago</button>`;if(p.status==='Aguardando confirmação')a=`<button class="mini primary" onclick="App.confirmarPedidoPortal('${p.id}')">Confirmar pedido</button>`;else if(p.status==='Pedido Feito')a=`<button class="mini primary" onclick="App.atualizarStatus('${p.id}','Preparando')">Preparando</button>`;else if(p.status==='Preparando')a=`<button class="mini primary" onclick="App.atualizarStatus('${p.id}','Pedido Pronto')">Pronto</button>`;else if(p.status==='Pedido Pronto')a=`<button class="mini primary" onclick="App.atualizarStatus('${p.id}','Entregue')">Entregar</button>`;else a=`<span class="pill">${p.status}</span>`;return a+pay+`<button class="mini danger-btn" onclick="App.excluirPedido('${p.id}')">excluir</button>`}
  function pratosDoPedido(p){
    let pratos=(p?.pratos&&p.pratos.length)?deepCopy(p.pratos):[];
    const precisaReagrupar=(!pratos.length)|| (pratos.length===1 && (pratos[0].itens||[]).filter(i=>String(i.categoria||i.tipo||'').toLowerCase()==='massa').length>1);
    if(!precisaReagrupar)return pratos.map((pr,idx)=>({...pr,nome:pr.nome||('Pedido '+(idx+1)),itens:pr.itens||[]}));
    const flat=(p?.itens&&p.itens.length)?p.itens:(pratos[0]?.itens||[]);
    const comIndice=(flat||[]).filter(i=>Number(i.pratoIndex||0)>0);
    if(comIndice.length){
      const by={};comIndice.forEach(i=>{const n=Number(i.pratoIndex||1);by[n]=by[n]||{id:'pr_'+n,nome:'Pedido '+n,itens:[]};by[n].itens.push(i)});
      return Object.keys(by).sort((a,b)=>Number(a)-Number(b)).map(k=>by[k]);
    }
    const grupos=[];let atual=null;
    (flat||[]).forEach(i=>{
      const cat=String(i.categoria||i.tipo||'').trim();
      if(cat==='Massa' && atual && (atual.itens||[]).length){grupos.push(atual);atual=null;}
      if(!atual)atual={id:'pr_'+(grupos.length+1),nome:'Pedido '+(grupos.length+1),itens:[]};
      atual.itens.push(i);
    });
    if(atual&&(atual.itens||[]).length)grupos.push(atual);
    return grupos.length?grupos:[{nome:'Pedido 1',itens:flat||[]}];
  }
  function itensPorCategoriaDoPrato(pr){
    const ordem=['Massa','Molho','Proteína','Complemento','Finalização','Bebida','Sobremesa'];
    const grupos={};
    (pr.itens||[]).forEach(i=>{const cat=i.categoria||i.tipo||'Item';grupos[cat]=grupos[cat]||[];grupos[cat].push(i)});
  ...
  function renderPratoOperacional(pr,idx,compact=false){
    ...
    const tituloBase=(pr.nome && !/^Prato\s*\d+$/i.test(pr.nome))
      ? escapeHtmlPrato(pr.nome)
      : `Pedido ${idx+1}`;
    const tamanhoNome=String(tamanhoDoPratoSalvo(pr)?.nome||'').trim();
    const titulo=tamanhoNome?`${tituloBase} — ${escapeHtmlPrato(tamanhoNome.toUpperCase())}`:tituloBase;
  ...
  function pedidoCard(p){const reserva=p.atendimento==='reserva'&&p.reservaData?`<span class=\"pill reservation-pill\">📅 ${new Date(p.reservaData+'T12:00:00').toLocaleDateString('pt-BR')} • ${p.reservaHorario}</span>`:'';return `<div class="pedido-card"><div class="pedido-top"><strong>${pedidoNumeroLabel(p)}</strong><span class="pill origin-pill ${origemPedidoClass(p)}">${origemPedidoLabel(p)}</span><span class="status ${statusClass(p.status)}">${p.status}</span></div><small>${new Date(p.data).toLocaleString('pt-BR')} • ${p.tipo} • prev. ${p.pagamentoPrevisto||'PIX'}</small><p><b>${p.cliente}</b></p>${reserva}${pedidoPratos(p)}<div class="pedido-bottom"><b>${fmt(p.total)}</b><div class="list-actions">${statusActions(p)}<button class="mini secondary" onclick="App.copiarMensagemPagamento('${p.id}')">copiar texto</button>${String(p.pagamentoPrevisto||p.pagamento||'').toUpperCase()==='PIX'?`<button class="mini secondary" onclick="App.copiarChavePix()">copiar PIX</button>`:''}<button class="mini secondary" onclick="App.abrirEditarDadosPedido('${p.id}')">dados</button><button class="mini secondary" onclick="App.editarPedido('${p.id}')">editar</button><button class="mini secondary" onclick="App.verPedido('${p.id}')">detalhes</button></div></div></div>`}
  function renderKanban(){
    const k=el('kanbanPedidos');if(!k)return;
    const scrollState={};
    k.querySelectorAll('.kanban-col[data-status]').forEach(col=>{
      const sc=col.querySelector('.kanban-scroll');
      if(sc)scrollState[col.dataset.status]=sc.scrollTop;
    });
    k.innerHTML=STATUS.map(st=>{
      const ps=db.pedidos.filter(p=>(p.status||'Pedido Feito')===st&&!(p.atendimento==='reserva'&&p.reservaIniciada!==true));
      return `<div class="kanban-col" data-status="${st}"><h3>${st}<span>${ps.length}</span></h3><div class="kanban-scroll">${ps.map(pedidoCard).join('')||'<p class="muted small-pad">Nenhum pedido</p>'}</div></div>`
    }).join('');
    requestAnimationFrame(()=>{
      k.querySelectorAll('.kanban-col[data-status]').forEach(col=>{
        const sc=col.querySelector('.kanban-scroll');
        const y=scrollState[col.dataset.status];
        if(sc&&typeof y==='number')sc.scrollTop=y;
      });
    });
  }
  async function iniciarReserva(id){const p=db.pedidos.find(x=>x.id===id);if(!p)return;if(p.reservaIniciada===true)return toast('Esta reserva já foi iniciada.');const anterior=deepCopy(p);p.reservaIniciada=true;p.status='Pedido Feito';p.timeline=p.timeline||[];p.timeline.push({status:'Reserva iniciada',data:new Date().toISOString()});save();renderAll();const ok=await supabaseAtualizarPedido(p);if(!ok){Object.assign(p,anterior);save();renderAll();return toast('Não foi possível iniciar a reserva no banco. A alteração foi desfeita.')}await supabaseCarregarPedidos();renderAll();page('pedidos');toast('Reserva iniciada e enviada para a fila de Pedidos.')}
  function renderReservados(){const box=el('listaReservados');if(!box)return;const reservas=(db.pedidos||[]).filter(p=>p.atendimento==='reserva'&&p.reservaData&&p.reservaIniciada!==true).sort((a,b)=>String(a.reservaData+' '+a.reservaHorario).localeCompare(String(b.reservaData+' '+b.reservaHorario)));if(!reservas.length){box.innerHTML='<div class="card"><h3>Nenhuma reserva cadastrada</h3><p class="muted">Ao registrar um pedido como reserva no Montar Pedido, ele aparecerá aqui.</p></div>';return}const grupos={};reservas.forEach(p=>(grupos[p.reservaData]=grupos[p.reservaData]||[]).push(p));box.innerHTML=Object.entries(grupos).map(([data,ps])=>`<section class="reservation-day"><header><div><small>Reservas</small><h3>${new Date(data+'T12:00:00').toLocaleDateString('pt-BR',{weekday:'long',day:'2-digit',month:'2-digit'})}</h3></div><span>${ps.length}</span></header><div class="reservation-list">${ps.map(p=>`<div class="reservation-card"><div class="reservation-time">${p.reservaHorario}</div>${pedidoCard(p)}</div>`).join('')}</div></section>`).join('')}
  function renderCozinha(){const k=el('cozinhaBoard');if(!k)return;const ativos=db.pedidos.filter(p=>!p.financeiroLancado&&p.status!=='Entregue'&&!(p.atendimento==='reserva'&&p.reservaIniciada!==true));if(!ativos.length){k.innerHTML='<div class="card"><h3>Nenhum pedido em preparo</h3><p class="muted">Quando um pedido for registrado, ele aparecerá aqui automaticamente.</p></div>';return}const ordem=['Pedido Feito','Preparando','Pedido Pronto'];k.innerHTML=ordem.map(st=>{const grupo=ativos.filter(p=>p.status===st);return `<section class="kitchen-column ${statusClass(st)}"><header><strong>${st}</strong><span>${grupo.length}</span></header>${grupo.map(p=>`<div class="kitchen-ticket ${statusClass(p.status)}"><div><span class="pill status-pill">${p.status}</span><h3>${pedidoNumeroLabel(p)}</h3><p><span class="pill origin-pill ${origemPedidoClass(p)}">${origemPedidoLabel(p)}</span> • ${p.tipo} • ${p.cliente||'Cliente balcão'}</p></div>${pratosDoPedido(p).map((pr,idx)=>`<div class="kitchen-plate">${renderPratoOperacional(pr,idx,true)}</div>`).join('')}<div class="kitchen-actions">${statusActions(p)}</div></div>`).join('')||'<p class="muted small-column">Nenhum pedido nesta etapa.</p>'}</section>`}).join('')}
  ...
  function renderListas(){const filtro=val('filtroCategoria'),busca=val('buscaItem').toLowerCase();const itens=db.itens.filter(i=>(!filtro||i.categoria===filtro)&&(!busca||i.nome.toLowerCase().includes(busca)));el('listaItens').innerHTML=itens.map(i=>{const pct=Math.min(100,Number(i.estoque||0)/Math.max(Number(i.minimo||1)*3,1)*100);const low=Number(i.estoque||0)<=Number(i.minimo||0);const editing=val('itemId')===i.id;return `<div class="list-item ${editing?'editing-item':''}" data-item-id="${i.id}"><div style="flex:1"><strong>${i.imagem?`<img class="tiny-img" src="${i.imagem}" alt="">`:(i.icone||iconeCat(i.categoria))} ${i.nome}${editing?'<span class="edit-note">editando agora</span>':''}</strong><br><span class="pill">${i.categoria}</span> <span class="pill ${statusClassDisponibilidade(i)}">${statusEstoqueOperacional(i)}</span> <small>${fmt(i.preco)} • custo médio ${fmt(i.custo)} • porção ${i.porcao||1} ${i.porcaoUnidade||i.unidade||'un'}</small>${controlaEstoqueNumerico(i)?`<div class="stockbar ${low?'low':''}"><b style="width:${pct}%"></b></div>`:''}</div><div class="list-actions"><button class="mini secondary" onclick="App.alternarDisponibilidadeItem('${i.id}')">${i.ativo===false?'ativar':'indisp.'}</button><button class="mini secondary" onclick="App.editarItem('${i.id}')">editar</button><button class="mini danger-btn" onclick="App.removerItem('${i.id}')">remover</button></div></div>`}).join('')||'<p class="muted">Nenhum item cadastrado.</p>';const pedidos=db.pedidos.filter(p=>!(p.atendimento==='reserva'&&p.reservaIniciada!==true)).slice(0,60);el('listaPedidos').innerHTML=pedidos.map(p=>`<div class="list-item"><div><strong>${pedidoNumeroLabel(p)} <span class="pill origin-pill ${origemPedidoClass(p)}">${origemPedidoLabel(p)}</span></strong><br><small>${new Date(p.data).toLocaleString('pt-BR')} • ${p.cliente} • ${p.status} • prev. ${p.pagamentoPrevisto||'PIX'} • ${p.pagamento||'pagamento pendente'}${descricaoCupomPedido(p)?'<br>'+descricaoCupomPedido(p):''}</small></div><div class="list-actions"><b>${fmt(p.total)}</b>${statusActions(p)}<button class="mini secondary" onclick="App.copiarMensagemPagamento('${p.id}')">copiar texto</button>${String(p.pagamentoPrevisto||p.pagamento||'').toUpperCase()==='PIX'?`<button class="mini secondary" onclick="App.copiarChavePix()">copiar PIX</button>`:''}<button class="mini secondary" onclick="App.abrirEditarDadosPedido('${p.id}')">dados</button><button class="mini secondary" onclick="App.editarPedido('${p.id}')">editar</button><button class="mini secondary" onclick="App.verPedido('${p.id}')">detalhes</button></div></div>`).join('')||'<p class="muted">Nenhum pedido registrado.</p>';el('ultimosPedidos').innerHTML=db.pedidos.filter(p=>!(p.atendimento==='reserva'&&p.reservaIniciada!==true)).slice(0,8).map(p=>`<div class="list-item"><div><strong>${pedidoNumeroLabel(p)} <span class="pill origin-pill ${origemPedidoClass(p)}">${origemPedidoLabel(p)}</span></strong><br><small>${p.cliente} • ${p.status}</small></div><b>${fmt(p.total)}</b></div>`).join('')||'<p class="muted">Nenhum pedido ainda.</p>';const count={};pedidosPagos().forEach(p=>p.itens.forEach(i=>count[i.nome]=(count[i.nome]||0)+Number(i.qtdUsada||1)));if(el('rankingItens'))el('rankingItens').innerHTML=Object.entries(count).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([n,q])=>`<div class="list-item"><strong>${n}</strong><span class="pill">${q} uso</span></div>`).join('')||'<p class="muted">Sem dados de vendas concluídas.</p>';if(el('alertasEstoque'))el('alertasEstoque').innerHTML=db.itens.filter(i=>(controlaEstoqueNumerico(i)&&Number(i.estoque||0)<=Number(i.minimo||0))||i.ativo===false).map(i=>`<div class="list-item"><div><strong>${i.nome}</strong><br><small>${i.categoria}</small></div><span class="pill red">${controlaEstoqueNumerico(i)?fmtQtd(i.estoque)+' '+(i.unidade||'un'):'Indisponível'}</span></div>`).join('')||'<p class="muted">Nenhum alerta de estoque.</p>';const pendentes=deduplicarClientesPendentes((db.clientesPendentes||[]).filter(c=>(c.status||'pendente')==='pendente'));const historicoPendentes=deduplicarClientesPendentes((db.clientesPendentes||[]).filter(c=>(c.status||'pendente')!=='pendente')).slice(0,30);if(el('listaClientesPendentes'))el('listaClientesPendentes').innerHTML=pendentes.map(c=>`<div class="list-item warn-box"><div><strong>${c.nome||'Sem nome'}</strong><br><small>${c.telefone||'sem telefone'} • Pedido #${c.numeroPedido||''}<br>${c.endereco||enderecoCompleto(c)} ${c.bairro?('• '+c.bairro):''}</small></div><div class="list-actions"><button class="mini secondary" onclick="App.aprovarClientePendente('${c.id}')">aprovar</button><button class="mini danger-btn" onclick="App.reprovarClientePendente('${c.id}')">reprovar</button></div></div>`).join('')||'<p class="muted">Nenhum cliente pendente de aprovação.</p>';if(el('historicoClientesPendentes'))el('historicoClientesPendentes').innerHTML=historicoPendentes.map(c=>{const st=(c.status||'').toUpperCase();const dt=c.aprovadoEm||c.reprovadoEm||c.updatedAt||c.createdAt;return `<div class="list-item"><div><strong>${c.nome||'Sem nome'}</strong><br><small>${c.telefone||'sem telefone'} • Pedido #${c.numeroPedido||''} • ${dt?new Date(dt).toLocaleString('pt-BR'):''}</small></div><span class="pill ${c.status==='reprovado'?'red':''}">${st}</span></div>`}).join('')||'<p class="muted">Nenhum histórico de aprovação ainda.</p>';el('listaClientes').innerHTML=db.clientes.map(c=>{const total=pedidosPagos().filter(p=>p.cliente===c.nome||p.telefone===c.telefone).reduce((a,p)=>a+p.total,0);return `<div class="list-item"><div><strong>${c.nome}</strong><br><small>${c.telefone||'sem telefone'} • ${c.bairro||'sem bairro'} • compras ${fmt(total)}</small></div><div class="list-actions"><button class="mini secondary" onclick="App.editarCliente('${c.id}')">editar</button><button class="mini danger-btn" onclick="App.removerCliente('${c.id}')">remover</button></div></div>`}).join('')||'<p class="muted">Nenhum cliente cadastrado.</p>';el('listaMovimentacoes').innerHTML=movimentosFinanceiros().slice(0,80).map(m=>`<div class="list-item"><div><strong>${m.desc}</strong><br><small>${new Date(m.data).toLocaleString('pt-BR')} • ${m.tipo} • ${m.origem==='manual'?'manual':(m.origem==='pedido'?'pedido':'compra')}</small></div><div class="list-actions"><b class="${m.tipo==='entrada'?'discount':''}">${fmt(m.valor)}</b>${m.origem==='manual'?`<button class="mini danger-btn" onclick="App.removerMovimentacao('${m.id}')">remover</button>`:''}</div></div>`).join('')||'<p class="muted">Nenhuma movimentação.</p>';renderMural();renderFechamentos();renderLogs();renderKanban();renderCozinha();renderMarketing();renderInsights();renderCompras();renderProducao()}

  function renderInsights(){const de=val('insightDataIni');const ate=val('insightDataFim');const inPeriodo=(p)=>{const d=(p.data||p.dataPagamento||'').slice(0,10);return (!de||d>=de)&&(!ate||d<=ate)};const box=el('insightsDia');if(!box)return;const pagos=pedidosPagos();const baixos=db.itens.filter(i=>(controlaEstoqueNumerico(i)&&Number(i.estoque||0)<=Number(i.minimo||0))||i.ativo===false);const count={};pagos.forEach(p=>p.itens.forEach(i=>count[i.nome]=(count[i.nome]||0)+1));const top=Object.entries(count).sort((a,b)=>b[1]-a[1])[0];let arr=[];if(baixos.length)arr.push(`⚠ ${baixos[0].nome} está abaixo do mínimo. Vale repor antes do próximo atendimento.`);if(top)arr.push(`🏆 Item mais usado até agora: ${top[0]}. Pode virar sugestão do chef.`);if(db.pedidos.some(p=>!p.financeiroLancado))arr.push('🍳 Existem pedidos em andamento. Use a aba Cozinha para acompanhar o preparo.');if(!arr.length)arr.push('Tudo pronto para iniciar as vendas. Quando houver pedidos, este painel trará sugestões automáticas.');box.innerHTML=arr.map(x=>`<div class="insight-item">${x}</div>`).join('')}
  ...
  function horarioAtualLoja(){
    ...
    const janelas=[];
    if(h.inicio&&h.fim)janelas.push([h.inicio,h.fim]);
    if(h.inicio2&&h.fim2)janelas.push([h.inicio2,h.fim2]);
    const aberta=janelas.some(([ini,fim])=>atual>=ini&&atual<=fim);
    const desc=janelas.map(([ini,fim])=>`${ini} às ${fim}`).join(' / ')||'sem horário';
    return {aberta,modo:'automatico',detalhe:`${(DIAS_LABEL[dia]||dia)} ${desc}`};
  }
  ...
  function renderDashboard(){const movs=movimentosFinanceiros(),entradas=movs.filter(m=>m.tipo==='entrada').reduce((a,m)=>a+Number(m.valor||0),0),saidas=movs.filter(m=>m.tipo==='saida').reduce((a,m)=>a+Number(m.valor||0),0),pagos=pedidosPagos(),pagosHoje=pagos.filter(p=>(p.dataPagamento||p.data||'').slice(0,10)===hoje()),fat=pagos.reduce((a,p)=>a+Number(p.total||0),0),fatHoje=pagosHoje.reduce((a,p)=>a+Number(p.total||0),0),meta=Number(db.config.metaDiaria||0),pct=meta?Math.min(100,(fatHoje/meta)*100):0;el('sideCaixa').textContent=fmt(entradas-saidas);el('dashPedidosHoje').textContent=pagosHoje.length;el('dashFaturamento').textContent=fmt(fat);el('dashTicket').textContent=fmt(pagos.length?fat/pagos.length:0);el('finEntradas').textContent=fmt(entradas);el('finSaidas').textContent=fmt(saidas);el('finSaldo').textContent=fmt(entradas-saidas);el('pedidoNumero').textContent='#'+(pedido.editandoNumero||proximoNumeroPedido());if(el('metaValorHoje'))el('metaValorHoje').textContent=fmt(meta);if(el('metaPercentual'))el('metaPercentual').textContent=Math.round(pct)+'%';if(el('metaBarra'))el('metaBarra').style.width=pct+'%';if(el('modoOperacaoBtn'))el('modoOperacaoBtn').textContent=db.config.modoOperacao?'Desativar modo operação':'Ativar modo operação';renderOperacao();renderCupons()}
  function renderConfig(force=false){if(!force&&!configSaving&&configEditing&&(Date.now()-configLastUserInput<30000))return;const brandName=document.querySelector('.brand strong');if(brandName)brandName.textContent=db.config.nome||'Donas da Massa';const internal=el('internalName');if(internal)internal.textContent=(db.config.nomeInterno||'Donas OS')+' v1.0';el('cfgNome').value=db.config.nome||'';if(el('cfgSlogan'))el('cfgSlogan').value=db.config.slogan||'';if(el('cfgCnpj'))el('cfgCnpj').value=db.config.cnpj||'';if(el('cfgEmail'))el('cfgEmail').value=db.config.email||'';if(el('cfgEnderecoEmpresa'))el('cfgEnderecoEmpresa').value=db.config.enderecoEmpresa||'';el('cfgInstagram').value=db.config.instagram||'';el('cfgPix').value=db.config.pix||'';if(el('cfgMensagemPagamento'))el('cfgMensagemPagamento').value=db.config.mensagemPagamento||seed().config.mensagemPagamento;if(el('cfgWhatsapp'))el('cfgWhatsapp').value=db.config.whatsapp||'';el('cfgTaxa').value=db.config.taxaEntrega||0;if(el('cfgRaioEntrega'))el('cfgRaioEntrega').value=db.config.raioEntrega||'';if(el('cfgOrigemCep'))el('cfgOrigemCep').value=db.config.origemCep||'';if(el('cfgOrigemNumero'))el('cfgOrigemNumero').value=db.config.origemNumero||'';if(el('cfgEnderecoSaida'))el('cfgEnderecoSaida').value=db.config.enderecoSaida||'';renderFaixasFrete();if(el('cfgOrigemStatus')){const ok=Number(db.config.origemLat)&&Number(db.config.origemLng);el('cfgOrigemStatus').className='delivery-origin-status '+(ok?'ok':'');el('cfgOrigemStatus').textContent=ok?`Origem localizada: ${Number(db.config.origemLat).toFixed(5)}, ${Number(db.config.origemLng).toFixed(5)}`:'Ainda não localizado.';}if(el('cfgTempoPreparo'))el('cfgTempoPreparo').value=db.config.tempoPreparo||0;el('cfgGratis').value=db.config.complementosGratis||0;if(el('cfgProteinasGratis'))el('cfgProteinasGratis').value=db.config.proteinasGratis??1;if(el('cfgFinalizacoesGratis'))el('cfgFinalizacoesGratis').value=(db.config.finalizacoesGratis??1);if(el('cfgMetaDiaria'))el('cfgMetaDiaria').value=db.config.metaDiaria||0;if(el('cfgNomeInterno'))el('cfgNomeInterno').value=db.config.nomeInterno||'Donas OS';if(el('cfgAdminSenha'))el('cfgAdminSenha').value=db.config.adminSenha||ADMIN_DEFAULT_PASS;if(el('cfgOperacaoSenha'))el('cfgOperacaoSenha').value=db.config.operacaoSenha||OPERACAO_DEFAULT_PASS;if(el('cfgModoOperacao'))el('cfgModoOperacao').checked=!!db.config.modoOperacao}
  ...
  function verPedido(id){const p=db.pedidos.find(x=>x.id===id);if(!p)return;el('modalContent').innerHTML=`<div class="receipt"><h2>${pedidoNumeroLabel(p)}</h2><p><span class="pill origin-pill ${origemPedidoClass(p)}">${origemPedidoLabel(p)}</span></p><p class="muted">${new Date(p.data).toLocaleString('pt-BR')} • ${p.tipo} • ${p.status} • previsto ${p.pagamentoPrevisto||'PIX'} • ${p.pagamento||'pagamento pendente'}</p><div class="order-detail"><strong>Cliente: ${p.cliente}</strong>${p.telefone?`<span>Telefone: ${p.telefone}</span>`:''}${p.endereco?`<span>Endereço: ${p.endereco} ${p.bairro||''}</span>`:''}${p.talher?`<span>Talher: ${p.talher}</span>`:''}<hr>${(p.pratos||[{nome:'Prato 1',itens:p.itens||[]}]).map(pr=>`<div class="prato-detail"><strong>${pr.nome}</strong>${pr.itens.map(i=>`<div class="summary-item"><span>${labelItemCliente(i)}<small> • ${i.categoria}</small></span><b>${i.gratis?'Grátis':fmt(i.valorCobrado??i.preco)}</b></div>`).join('')}</div>`).join('')}<hr>${p.cupom?`<div class="summary-item"><span>Subtotal</span><span>${fmt(p.subtotalBruto||p.total+Number(p.desconto||0))}</span></div><div class="summary-item discount"><strong>Cupom ${p.cupom}</strong><strong>- ${fmt(p.desconto||0)}</strong></div>`:''}<div class="summary-item"><strong>Total</strong><strong>${fmt(p.total)}</strong></div>${p.obs?`<p><strong>Obs:</strong> ${p.obs}</p>`:''}<div class="timeline"><strong>Linha do tempo</strong>${(p.timeline||[]).map(t=>`<span>${new Date(t.data).toLocaleString('pt-BR')} • ${t.status}${t.pagamento?' • '+t.pagamento:''}</span>`).join('')}</div><button class="secondary full" onclick="App.salvarFavoritoPedido('${p.id}')">Salvar como receita pronta</button><button class="secondary full" onclick="App.abrirEditarDadosPedido('${p.id}')">Editar dados / entrega</button><button class="secondary full" onclick="App.editarPedido('${p.id}')">Editar itens do pedido</button><button class="danger-btn full" onclick="App.excluirPedido('${p.id}')">Excluir pedido</button></div></div>`;el('modal').classList.remove('hidden')}
  ...
  async function editarPedido(id){const p=db.pedidos.find(x=>x.id===id);if(!p)return;if(!confirm('Editar os itens do pedido #'+p.numero+'? Ele continuará na mesma posição da fila até você salvar.'))return;editBuffer={pedido:deepCopy(p)};pedido=novoPedido();pedido.pratos=deepCopy(p.pratos||[{nome:'Prato 1',itens:p.itens||[]}]).map((pr,idx)=>recalcularPratoSalvo(pr,idx));pedido.pratoEditandoIndex=-1;pedido.cliente=p.cliente||'';pedido.telefone=p.telefone||'';pedido.tipo=p.tipo||'Balcão';pedido.atendimento=p.atendimento||'imediato';pedido.reservaData=p.reservaData||'';pedido.reservaHorario=p.reservaHorario||'';pedido.pagamentoPrevisto=p.pagamentoPrevisto||p.pagamento||'PIX';pedido.trocoPara=Number(p.trocoPara||extrairTrocoObs(p.obs)||0);pedido.pagamentoRecebido=!!(p.financeiroLancado||p.pagamentoRecebido);pedido.obs=p.obs||'';pedido.cep=p.cep||'';pedido.rua=p.rua||'';pedido.numero=p.numeroEndereco||p.numero||'';pedido.complemento=p.complemento||'';pedido.endereco=p.endereco||'';pedido.bairro=p.bairro||'';pedido.cidade=p.cidade||'';pedido.uf=p.uf||'';pedido.clienteId=p.clienteId||'';pedido.editandoId=p.id;pedido.editandoNumero=p.numero;pedido.editandoOnlineId=p.onlineId||'';pedido.freteGratis=!!p.freteGratis;pedido.freteOriginal=Number(p.freteOriginal||0);const mapEdit={clientePedido:'cliente',telefonePedido:'telefone',obsPedido:'obs',pedidoCep:'cep',pedidoRua:'rua',pedidoNumeroEndereco:'numero',pedidoComplemento:'complemento',enderecoPedido:'endereco',pedidoBairro:'bairro',bairroPedido:'bairro',pedidoCidade:'cidade',pedidoUf:'uf'};Object.entries(mapEdit).forEach(([id2,k])=>{if(el(id2))el(id2).value=pedido[k]||''});if(el('tipoPedido'))el('tipoPedido').value=pedido.tipo==='Delivery'?'Delivery':'Balcão';if(el('atendimentoPedido'))el('atendimentoPedido').value=pedido.atendimento;if(el('reservaDataPedido'))el('reservaDataPedido').value=pedido.reservaData;if(el('reservaHorarioPedido'))el('reservaHorarioPedido').value=pedido.reservaHorario;alternarCamposReserva();if(el('pagamentoPedido'))el('pagamentoPedido').value=pedido.pagamentoPrevisto;if(el('pedidoTrocoPara'))el('pedidoTrocoPara').value=pedido.trocoPara||'';alternarCamposTroco('pedido');if(el('pagamentoRecebidoPedido'))el('pagamentoRecebidoPedido').value=pedido.pagamentoRecebido?'sim':'nao';if(el('pedidoFreteGratis'))el('pedidoFreteGratis').checked=pedido.freteGratis;fretePedido={status:p.tipo==='Delivery'?'ok':'idle',distancia:Number(p.distanciaEntregaKm||0),valor:Number(p.freteOriginal||p.frete||0),mensagem:p.tipo==='Delivery'?`Frete carregado do pedido: ${fmt(Number(p.freteOriginal||p.frete||0))}`:'Retirada selecionada: sem frete.'};log('Edição iniciada','#'+p.numero);fecharModal();page('pdv');renderAll();if(p.tipo==='Delivery'&&pedido.rua&&pedido.cidade){await calcularFreteEndereco('pedido')}renderAll();toast('Pedido carregado e valores recalculados automaticamente.')}


  function aplicarModoOperacao(){document.body.classList.toggle('modo-operacao',!!db.config.modoOperacao);document.querySelectorAll('.nav button').forEach(b=>{const keep=['pdv','pedidos','reservados','cozinha','dashboard'].includes(b.dataset.page);b.style.display=(db.config.modoOperacao&&!keep)?'none':''})}
  ...
  function resumoHoje(){const pagos=pedidosPagos().filter(p=>(p.dataPagamento||p.data||'').slice(0,10)===hoje()),movs=movimentosFinanceiros().filter(m=>(m.data||'').slice(0,10)===hoje()),entradas=movs.filter(m=>m.tipo==='entrada').reduce((a,m)=>a+Number(m.valor||0),0),saidas=movs.filter(m=>m.tipo==='saida').reduce((a,m)=>a+Number(m.valor||0),0),saldo=entradas-saidas,porPg={};pagos.forEach(p=>porPg[p.pagamento||'Não informado']=(porPg[p.pagamento||'Não informado']||0)+Number(p.total||0));return{pagos,entradas,saidas,saldo,porPg}}
  function fecharCaixa(){const r=resumoHoje();el('modalContent').innerHTML=`<div class="receipt"><h2>Fechamento do dia</h2><p class="muted">${new Date().toLocaleDateString('pt-BR')} • ${(db.config.nomeInterno||'Donas OS')}</p><div class="stats-grid modal-stats"><article class="stat"><span>Pedidos</span><strong>${r.pagos.length}</strong></article><article class="stat"><span>Entradas</span><strong>${fmt(r.entradas)}</strong></article><article class="stat"><span>Saídas</span><strong>${fmt(r.saidas)}</strong></article><article class="stat"><span>Saldo</span><strong>${fmt(r.saldo)}</strong></article></div><h3>Formas de pagamento</h3>${Object.entries(r.porPg).map(([k,v])=>`<div class="summary-item"><span>${k}</span><b>${fmt(v)}</b></div>`).join('')||'<p class="muted">Nenhum pagamento concluído hoje.</p>'}<hr><button class="primary full" onclick="App.registrarFechamento()">Registrar fechamento</button><button class="secondary full" onclick="window.print()">Imprimir / salvar PDF</button></div>`;el('modal').classList.remove('hidden')}
  function registrarFechamento(){const r=resumoHoje();db.fechamentos=db.fechamentos||[];if(db.fechamentos.some(f=>f.data===hoje())){if(!confirm('Já existe fechamento de hoje. Registrar outro mesmo assim?'))return}db.fechamentos.unshift({id:uid(),data:hoje(),pedidos:r.pagos.length,entradas:r.entradas,saidas:r.saidas,saldo:r.saldo,porPagamento:r.porPg,createdAt:new Date().toISOString()});log('Fechamento registrado',hoje()+' • '+r.pagos.length+' pedidos');save();fecharModal();toast('Fechamento registrado')}
  function renderFechamentos(){const box=el('listaFechamentos');if(!box)return;box.innerHTML=(db.fechamentos||[]).slice(0,12).map(f=>`<div class="list-item"><div><strong>${new Date(f.data+'T12:00:00').toLocaleDateString('pt-BR')}</strong><br><small>${f.pedidos} pedidos • saldo ${fmt(f.saldo??(Number(f.entradas||0)-Number(f.saidas||0)))}</small></div><b>${fmt(f.entradas-f.saidas)}</b></div>`).join('')||'<p class="muted">Nenhum fechamento registrado.</p>'}
  ...
  function comprasValidasItem(itemId){return (db.compras||[]).filter(c=>c.addEstoque&&c.itemId===itemId&&Number(c.qtd||0)>0&&Number(c.valor||0)>=0)}
  function custoMedioComprasItem(itemId,fallback=0){
    const cs=comprasValidasItem(itemId);
    const qtd=cs.reduce((a,c)=>a+Number(c.qtd||0),0);
    const val=cs.reduce((a,c)=>a+Number(c.valor||0),0);
    return qtd>0?Math.round((val/qtd)*100)/100:Number(fallback||0);
  }
  function cupomPorCodigo(codigo){const cod=String(codigo||'').trim().toUpperCase();if(!cod)return null;return (db.cupons||[]).find(c=>String(c.nome||'').toUpperCase()===cod)}
  function validarCupom(c,total=0){
    if(!c)return {ok:false,msg:'Cupom não encontrado'};
    if(c.ativo===false)return {ok:false,msg:'Cupom inativo'};
    if(c.validoAte&&dataLocalISO(new Date())>String(c.validoAte).slice(0,10))return {ok:false,msg:'Cupom vencido'};
    const limite=Number(c.limite||0),usados=Number(c.usados||0);
    if(limite>0&&usados>=limite)return {ok:false,msg:'Cupom esgotado'};
    const desc=c.tipo==='valor'?Math.min(Number(c.desconto||0),Number(total||0)):Number(total||0)*(Number(c.desconto||0)/100);
    return {ok:true,desconto:Math.max(0,Math.round(desc*100)/100),msg:'Cupom aplicado'};
  }
  function aplicarDescontoPedido(codigo,total){const c=cupomPorCodigo(codigo);const r=validarCupom(c,total);return {...r,cupom:c,codigo:codigo?String(codigo).trim().toUpperCase():''}}
  function usarCupomSeValido(codigo,total){const r=aplicarDescontoPedido(codigo,total);return r.ok?r:{ok:false,desconto:0,cupom:null,codigo:String(codigo||'').trim().toUpperCase(),msg:r.msg}}
  function descricaoCupomPedido(p){
    const cod=String(p?.cupom||p?.cupomCodigo||'').trim().toUpperCase();
    if(!cod)return '';
    const desc=Number(p?.desconto||0);
    const bruto=Number(p?.subtotalBruto||0);
    return `Cupom ${cod}${desc?` • desconto ${fmt(desc)}`:''}${bruto?` • subtotal ${fmt(bruto)}`:''}`;
  }
  async function registrarUsoCupom(codigo){
    const cod=String(codigo||'').trim().toUpperCase();
    if(!cod)return true;
    if(supabaseLojaId) await supabaseCarregarCupons();
    const c=cupomPorCodigo(cod);
    const r=validarCupom(c,0);
    if(!r.ok){toast(r.msg||'Cupom não disponível');return false;}
    c.usados=Number(c.usados||0)+1;
    c.updatedAt=new Date().toISOString();
    save();
    const ok=await supabaseSalvarCupom(c);
    renderCupons();
    return ok!==false;
  }
  async function salvarCompra(){
    ...
    if(data.addEstoque && !id){
      ...
      if(item){
        ...
        const custoAnterior=Number(item.custo||0);
        const qtdEstoque=converterUnidadeEstoque(qtd,data.unidade,item.unidade||data.unidade);
        const custoNovo=qtdEstoque>0?Number(valor||0)/qtdEstoque:Number(data.custoUnitario||0);
        item.estoque=estoqueAnterior+qtdEstoque;
        item.custo=(estoqueAnterior>0&&custoAnterior>0)?(((estoqueAnterior*custoAnterior)+(qtdEstoque*custoNovo))/(estoqueAnterior+qtdEstoque||1)):(custoNovo||custoAnterior);
        item.obs=item.obs||data.local||data.obs||'';
        data.qtdEstoque=qtdEstoque;data.unidadeEstoque=item.unidade||data.unidade;data.custoUnitario=custoNovo;
      }else{
        item={id:uid(),nome:produto,categoria:data.categoria,preco:0,custo:data.custoUnitario,estoque:qtd,minimo:0,unidade:data.unidade,icone:iconeCat(data.categoria),imagem:'',porcao:unidadeInteira({unidade:data.unidade})?1:Math.max(0.1,Math.round(qtd*10)/10),porcaoUnidade:data.unidade,obs:data.local||data.obs||'',ativo:true};
        db.itens.push(item);
      }
      await supabaseSalvarItem(item);
      if(compraObj && item?.id){compraObj.itemId=item.id;compraObj.qtdEstoque=data.qtdEstoque||qtd;compraObj.unidadeEstoque=data.unidadeEstoque||data.unidade;compraObj.custoUnitario=data.custoUnitario;}
    }
  ...
  function renderCompras(){
    ...
    el('listaCompras').innerHTML=compras.slice(0,120).map(c=>`<div class="list-item"><div style="flex:1"><strong>${compraIcone(c)} ${c.produto}</strong><br><span class="pill">${c.categoria}</span> <small>${new Date((c.data||hoje())+'T12:00:00').toLocaleDateString('pt-BR')} • ${c.local||'sem local'} • ${c.qtd} ${c.unidade}${c.unidadeEstoque&&c.unidadeEstoque!==c.unidade?` → ${Number(c.qtdEstoque||0).toLocaleString('pt-BR')} ${c.unidadeEstoque}`:''} • ${fmt(c.valor)} • ${fmt(c.custoUnitario)}/${c.unidadeEstoque||c.unidade}</small>${c.obs?`<br><small>${c.obs}</small>`:''}</div><div class="list-actions"><button class="mini secondary" onclick="App.editarCompra('${c.id}')">editar</button><button class="mini danger-btn" onclick="App.removerCompra('${c.id}')">remover</button></div></div>`).join('')||'<p class="muted">Nenhuma compra encontrada.</p>';
  }
  ...
  async function registrarProducao(){const receitaId=val('producaoReceita'),qtd=Number(val('producaoQtd')||0);const r=(db.receitas||[]).find(x=>x.id===receitaId);if(!r)return toast('Selecione uma ficha técnica');if(qtd<=0)return toast('Informe a quantidade produzida');const fator=qtd;
  ...
  function setInsightsSyncStatus(txt){
    const box=el('insightsSyncStatus');
    if(box)box.textContent=txt||'';
  }
  async function atualizarInsightsSupabase(){
    if(isPublicClient()||insightsRefreshing)return;
    insightsRefreshing=true;
    setInsightsSyncStatus('Atualizando dados do Supabase...');
    try{
      await supabaseCarregarItens();
      await supabaseCarregarCompras();
      await supabaseCarregarReceitas();
      await supabaseCarregarProducoes();
      await supabaseCarregarMovimentacoes();
      await supabaseCarregarPedidos();
      setInsightsSyncStatus('Dados atualizados: '+new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}));
      renderDashboard();
      renderInsights();
      renderRelatorios();
    }catch(e){
      console.warn('Atualizar insights Supabase:',e);
      setInsightsSyncStatus('Não foi possível atualizar agora. Tentando novamente na sincronização automática.');
    }finally{
  ...
  function renderInsights(){
    ...
    const dataIni=val('insightDataIni')||today;
    const dataFim=val('insightDataFim')||dataIni;
    const pedidosTodos=(db.pedidos||[]).filter(p=>p.supabase_id||p.origem==='supabase');const pedidosBase=pedidosPagos().filter(p=>p.supabase_id||p.origem==='supabase');
    const dataPedidoInsights=p=>dataLocalISO(p.dataPagamento||p.pagoEm||p.data);
    const pedidosHoje=pedidosBase.filter(p=>{const d=dataPedidoInsights(p);return d>=dataIni&&d<=dataFim});
    const ref=new Date(dataIni+'T12:00:00');ref.setDate(ref.getDate()-1);const ontemStr=dataLocalISO(ref);
    const pedidosOntem=pedidosBase.filter(p=>dataPedidoInsights(p)===ontemStr);
    ...
    if(el('insStatusPedidos')){const periodoTodos=pedidosTodos.filter(p=>{const d=dataLocalISO(p.data);return d>=dataIni&&d<=dataFim});const pagosPeriodo=periodoTodos.filter(p=>p.pagamentoRecebido||p.financeiroLancado).length;const pendentes=periodoTodos.filter(p=>!(p.pagamentoRecebido||p.financeiroLancado)).length;const entregues=periodoTodos.filter(p=>String(p.status||'').toLowerCase().includes('entregue')).length;el('insStatusPedidos').innerHTML=`<div class="stats-grid compact-stats"><article class="stat"><span>Pagos</span><strong>${pagosPeriodo}</strong></article><article class="stat"><span>Pendentes</span><strong>${pendentes}</strong></article><article class="stat"><span>Entregues</span><strong>${entregues}</strong></article></div>`;}
    ...
    const consumo={};
    pedidosHoje.forEach(p=>(p.pratos||[{itens:p.itens||[]}]).forEach(pr=>(pr.itens||[]).forEach(i=>{const cat=i.categoria||'Outros';if(porCat[cat])porCat[cat][i.nome]=(porCat[cat][i.nome]||0)+porcoesInsightsItem(i);consumo[i.nome]=consumo[i.nome]||{qtd:0,unidade:i.unidade||'un',categoria:cat};consumo[i.nome].qtd+=Number(i.qtdUsada||i.qtd||1)})));
    ...
    if(el('insConsumoItens'))el('insConsumoItens').innerHTML=cons.length?cons.map(([n,o])=>`<div class="list-item compact"><div><strong>${n}</strong><br><small>${o.categoria}</small></div><b>${Number(o.qtd).toFixed(o.qtd%1?1:0)} ${o.unidade}</b></div>`).join(''):'<p class="muted">O consumo aparecerá conforme os pedidos forem entregues.</p>';
    ...
    const anteriores=pedidosBase.filter(p=>dataPedidoInsights(p)!==today).map(p=>p.clienteId||norm(p.telefone)||p.cliente).filter(Boolean);
    ...
    if(el('insCompras'))el('insCompras').innerHTML=cards||'<p class="muted">Registre compras para comparar preços por mercado/local.</p>';
    const alertas=[];
    ...
    if(el('insAlertas'))el('insAlertas').innerHTML=alertas.length?alertas.map(x=>`<div class="insight-item">${x}</div>`).join(''):'<p class="muted">Nenhum alerta no momento.</p>';
  ...
  function renderRelatorios(){
    ...
    const pedidos=pedidosPagos().filter(p=>inRange(p.data));
    ...
    const abertos=(db.pedidos||[]).filter(p=>!(p.financeiroLancado||p.pagamentoRecebido===true)).length;
    ...
    el('relOperacional').innerHTML=`<div class="list-item compact"><strong>Pedidos em aberto</strong><b>${abertos}</b></div><div class="list-item compact"><strong>Produções registradas</strong><b>${producoes}</b></div><div class="list-item compact"><strong>Exclusões registradas em logs</strong><b>${cancelados}</b></div>`;
  }
  ...
  function payloadLojaSupabase(){
    return {
      ...
      config:{
        ...
        enderecoSaida:db.config.enderecoSaida||'',
        origemLat:Number(db.config.origemLat)||null,
        origemLng:Number(db.config.origemLng)||null,
        freteAutomatico:db.config.freteAutomatico!==false,
        faixasFrete:faixasFreteOrdenadas(),
  ...
  function mapItemSupabase(row){
    return {id:'sbit_'+row.id,supabase_id:row.id,nome:row.nome||'Item',categoria:row.categoria||'Insumo',preco:Number(row.preco||0),custo:Number(row.custo||0),estoque:Number(row.estoque||0),minimo:Number(row.minimo||0),unidade:row.unidade||'un',porcao:Number(row.porcao||1),porcaoUnidade:row.porcao_unidade||row.unidade||'un',icone:row.icone||iconeCat(row.categoria||'Insumo'),imagem:row.imagem||'',obs:row.observacao||'',ativo:row.ativo!==false,createdAt:row.criado_em||new Date().toISOString(),updatedAt:row.atualizado_em||row.criado_em||new Date().toISOString(),syncStatus:'sincronizado'};
  }
  function payloadItemSupabase(i){return {loja_id:supabaseLojaId,nome:i.nome||'Item',categoria:i.categoria||'Insumo',preco:Number(i.preco||0),custo:Number(i.custo||0),estoque:Number(i.estoque||0),minimo:Number(i.minimo||0),unidade:i.unidade||'un',porcao:Number(i.porcao||1),porcao_unidade:i.porcaoUnidade||i.porcao_unidade||i.unidade||'un',icone:i.icone||iconeCat(i.categoria),imagem:i.imagem||'',observacao:i.obs||'',ativo:i.ativo!==false,atualizado_em:new Date().toISOString()}}
  function mesclarItensSupabase(rows){
    const remotos=(rows||[]).map(mapItemSupabase);
    db.itens=[...remotos].sort((a,b)=>String(a.categoria||'').localeCompare(String(b.categoria||''),'pt-BR')||String(a.nome||'').localeCompare(String(b.nome||''),'pt-BR'));
  }
  async function supabaseCarregarItens(){
    try{await supabaseGetLoja();const rows=await supabaseRequest('/estoque_itens?select=*&loja_id=eq.'+encodeURIComponent(supabaseLojaId)+'&order=categoria.asc,nome.asc&limit=500');if(Array.isArray(rows)){mesclarItensSupabase(rows);Data.save(db);}}
  ...
  async function supabaseSalvarItem(i){
    ...
    try{
      ...
      const payload=payloadItemSupabase(i);
      const sid=i.supabase_id||(String(i.id||'').startsWith('sbit_')?String(i.id).replace('sbit_',''):null);
      let rows=[];
      if(sid){
        ...
        if(existente?.[0]?.id){
          ...
          rows=await supabaseRequest('/estoque_itens?select=*',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(payload)});
        }
      }
      if(!rows||!rows[0])throw new Error('Supabase não retornou confirmação do item');
      const r=rows[0];
      i.supabase_id=r.id;
      i.id=String(i.id||'').startsWith('sbit_')?('sbit_'+r.id):i.id;
      i.custo=Number(r.custo||i.custo||0);
      i.estoque=Number(r.estoque||i.estoque||0);
      i.porcao=Number(r.porcao||i.porcao||1);
      i.porcaoUnidade=r.porcao_unidade||i.porcaoUnidade||i.unidade||'un';
      i.syncStatus='sincronizado';
      return true;
    }
  ...
  async function supabaseRemoverItem(i){
    if(!i)return false;
    try{await supabaseGetLoja();const sid=i.supabase_id||(String(i.id||'').startsWith('sbit_')?String(i.id).replace('sbit_',''):null);if(sid){await supabaseRequest('/estoque_itens?id=eq.'+encodeURIComponent(sid),{method:'DELETE',headers:{Prefer:'return=minimal'}});return true;}return true;}
    catch(e){console.warn('Remover item Supabase:',e);toast('Não consegui remover o item no Supabase.');return false}
  }
  async function supabaseAtualizarEstoqueItens(itens){
    try{await supabaseGetLoja();let ok=true;for(const i of (itens||[])){const r=await supabaseSalvarItem(i);if(r===false)ok=false}return ok}catch(e){console.warn('Atualizar estoque Supabase:',e);return false}
  }
  async function supabaseSincronizarItensLocais(){
  ...
  function mapCompraSupabase(row){
    ...
    return {id:'sbcomp_'+row.id,supabase_id:row.id,onlineId:row.codigo||('COMP-'+row.id),produto:row.produto||'',local:row.local||'',data:row.data_compra||hoje(),qtd:Number(row.quantidade||0),unidade:row.unidade||'un',qtdEstoque:Number(row.quantidade_estoque||0),unidadeEstoque:row.unidade_estoque||row.unidade||'un',valor:Number(row.valor_total||0),categoria:row.categoria||'Insumo',itemId:item?.id||'',addEstoque:!!row.adiciona_estoque,financeiro:!!row.lancar_financeiro,obs:row.observacao||'',custoUnitario:Number(row.custo_unitario||0),createdAt:row.criado_em||new Date().toISOString(),updatedAt:row.atualizado_em||row.criado_em||new Date().toISOString(),syncStatus:'sincronizado'};
  }
  function payloadCompraSupabase(c){
    const item=c.itemId?(db.itens||[]).find(i=>i.id===c.itemId):null;
    return {loja_id:supabaseLojaId,codigo:c.onlineId||c.id||onlineId('COMP'),produto:c.produto||'',local:c.local||'',data_compra:c.data||hoje(),quantidade:Number(c.qtd||0),unidade:c.unidade||'un',quantidade_estoque:Number(c.qtdEstoque||0),unidade_estoque:c.unidadeEstoque||c.unidade||'un',valor_total:Number(c.valor||0),categoria:c.categoria||'Insumo',estoque_item_id:item?.supabase_id||null,adiciona_estoque:!!c.addEstoque,lancar_financeiro:!!c.financeiro,observacao:c.obs||'',custo_unitario:Number(c.custoUnitario||custoUnitarioCompra(c)||0),atualizado_em:new Date().toISOString()};
  }
  ...
  function mapMovimentacaoSupabase(row){
    return {id:'sbfin_'+row.id,supabase_id:row.id,onlineId:row.codigo||('FIN-'+row.id),desc:row.descricao||'',valor:Number(row.valor||0),tipo:row.tipo||'saida',data:row.data_movimentacao||row.criado_em||new Date().toISOString(),origem:'manual',obs:row.observacao||'',syncStatus:'sincronizado'};
  }
  function payloadMovimentacaoSupabase(m){
    return {loja_id:supabaseLojaId,codigo:m.onlineId||m.id||onlineId('FIN'),descricao:m.desc||'',tipo:m.tipo||'saida',valor:Number(m.valor||0),data_movimentacao:m.data||new Date().toISOString(),origem:'manual',observacao:m.obs||'',atualizado_em:new Date().toISOString()};
  }
  function mesclarMovimentacoesSupabase(rows){
    const remotas=(rows||[]).map(mapMovimentacaoSupabase);
    const locais=(db.financeiro||[]).filter(m=>!m.supabase_id&&!['pedido','compra'].includes(m.origem||''));
    const porCodigo=new Map();
    [...remotas,...locais].forEach(m=>{const k=m.onlineId||m.id;if(!porCodigo.has(k))porCodigo.set(k,m)});
    db.financeiro=[...porCodigo.values()].sort((a,b)=>new Date(b.data||0)-new Date(a.data||0));
  }
  async function supabaseCarregarMovimentacoes(){
    try{await supabaseGetLoja();const rows=await supabaseRequest('/financeiro_movimentacoes?select=*&order=data_movimentacao.desc,criado_em.desc&limit=500');mesclarMovimentacoesSupabase(rows||[]);Data.save(db);renderAll();}
    catch(e){console.warn('Supabase financeiro:',e)}
  }
  async function supabaseSalvarMovimentacao(m){
    if(!m||m.origem!=='manual')return;
    try{await supabaseGetLoja();if(!m.onlineId)m.onlineId=m.id||onlineId('FIN');const payload=payloadMovimentacaoSupabase(m);const sid=m.supabase_id||(String(m.id||'').startsWith('sbfin_')?String(m.id).replace('sbfin_',''):null);if(sid){await supabaseRequest('/financeiro_movimentacoes?id=eq.'+encodeURIComponent(sid),{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify(payload)});m.syncStatus='sincronizado';return;}const existente=await supabaseRequest('/financeiro_movimentacoes?select=id&codigo=eq.'+encodeURIComponent(m.onlineId)+'&limit=1');if(existente?.[0]?.id){m.supabase_id=existente[0].id;await supabaseRequest('/financeiro_movimentacoes?id=eq.'+encodeURIComponent(existente[0].id),{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify(payload)});}else{const criado=await supabaseRequest('/financeiro_movimentacoes?select=id',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(payload)});if(criado?.[0]?.id)m.supabase_id=criado[0].id;}m.syncStatus='sincronizado';}
    catch(e){console.warn('Salvar movimentação Supabase:',e);m.syncStatus='erro';toast('Movimentação salva localmente, mas não sincronizou com Supabase.')}
  }
  async function supabaseRemoverMovimentacao(m){
    try{if(!m)return;const sid=m.supabase_id||(String(m.id||'').startsWith('sbfin_')?String(m.id).replace('sbfin_',''):null);if(sid)await supabaseRequest('/financeiro_movimentacoes?id=eq.'+encodeURIComponent(sid),{method:'DELETE'});}
    catch(e){console.warn('Remover movimentação Supabase:',e);toast('Removida localmente, mas não removeu do Supabase.')}
  }
  async function supabaseSincronizarMovimentacoesLocais(){
    try{for(const m of (db.financeiro||[]).filter(x=>!['pedido','compra'].includes(x.origem||''))){if(!m.supabase_id)await supabaseSalvarMovimentacao(m)}db.sync.status='online';db.sync.lastSync=new Date().toISOString();Data.save(db)}catch(e){console.warn('Sincronizar financeiro:',e)}
  }
  ...
  function mapPedidoSupabase(row,itens=[],cliente=null){
    ...
    const snapInfo=extrairSnapshotPedido(row.observacao||'');
    const reserva=extrairReserva(snapInfo.obs);const freteInfo=extrairFreteGratis(reserva.obs);
    let pratosMapeados=[];let mappedItens=[];
    if(Array.isArray(snapInfo.snapshot?.pratos)&&snapInfo.snapshot.pratos.length){
      pratosMapeados=snapInfo.snapshot.pratos.map((pr,idx)=>{
        const itensPrato=(pr.itens||[]).map(i=>({...i,id:i.id||('snap_'+row.id+'_'+idx+'_'+uid()),tipo:i.tipo||i.categoria||'Item',categoria:i.categoria||i.tipo||'Item',quantidade:Number(i.quantidade||i.qtdUsada||1),qtdUsada:Number(i.qtdUsada||i.quantidade||1),preco:Number(i.preco||0),valorCobrado:Number(i.valorCobrado??i.preco??0),pratoIndex:idx+1}));
        const tamanhoSnap=pr.tamanho||pr.tamanhoSnapshot||(pr.tamanhoId||pr.tamanhoNome?{id:pr.tamanhoId||'',nome:pr.tamanhoNome||''}:null);
        return {...pr,id:'pr_'+row.id+'_'+idx,nome:pr.nome||('Pedido '+(idx+1)),itens:itensPrato,total:Number(pr.total||itensPrato.reduce((a,i)=>a+Number(i.valorCobrado||0),0)),custo:Number(pr.custo||itensPrato.reduce((a,i)=>a+Number(i.custoCalculado||0),0)),tamanho:snapshotTamanho(tamanhoDoPratoSalvo({...pr,tamanho:tamanhoSnap,itens:itensPrato}))};
      });
      mappedItens=pratosMapeados.flatMap(pr=>pr.itens||[]);
    }else{
      mappedItens=(itens||[]).map(i=>{const obs=String(i.observacao||'');const pratoMatch=obs.match(/prato:(\d+)/);const comboMatch=obs.match(/combo:(\d+)/);const nomeMatch=obs.match(/comboNome:([^|]+)/);let unidadeComboNome='';try{unidadeComboNome=nomeMatch?.[1]?decodeURIComponent(nomeMatch[1]):''}catch(e){unidadeComboNome=nomeMatch?.[1]||''}return {id:i.id,nome:i.nome,tipo:i.tipo,categoria:i.tipo,quantidade:i.quantidade,qtdUsada:Number(i.quantidade||1),unidade:'un',preco:Number(i.preco||0),valorCobrado:Number(i.preco||0),observacao:i.observacao||'',pratoIndex:pratoMatch?.[1]?Number(pratoMatch[1]):0,unidadeCombo:comboMatch?.[1]?Number(comboMatch[1]):0,unidadeComboNome};});
      pratosMapeados=pratosDoPedido({itens:mappedItens}).map((pr,idx)=>{const itensPrato=pr.itens||[];return {...pr,id:'pr_'+row.id+'_'+idx,nome:'Pedido '+(idx+1),total:itensPrato.reduce((a,i)=>a+Number(i.valorCobrado||0),0),custo:itensPrato.reduce((a,i)=>a+Number(i.custoCalculado||0),0)}});
    }
    const pago=(row.pago===true||row.status_pagamento==='pago');
    return {id:'sb_'+row.id,supabase_id:row.id,onlineId:row.codigo||row.id,numero,cliente:cliente?.nome||row.cliente_nome||'Cliente online',telefone:cliente?.telefone||row.cliente_telefone||'',tipo:row.tipo_entrega==='delivery'?'Delivery':'Balcão',pagamento:pago?(row.forma_pagamento||'PIX'):'',pagamentoPrevisto:row.forma_pagamento||'PIX',pagamentoRecebido:pago,dataPagamento:row.pago_em||'',talher:row.talher?'Sim':'Não',atendimento:reserva.atendimento,reservaData:reserva.reservaData,reservaHorario:reserva.reservaHorario,reservaIniciada:reserva.reservaIniciada,obs:freteInfo.obs,trocoPara:extrairTrocoObs(freteInfo.obs),cep:cliente?.cep||row.cep||'',rua:cliente?.rua||row.rua||'',numeroEndereco:cliente?.numero||row.numero_endereco||'',complemento:cliente?.complemento||row.complemento||'',endereco:cliente?.endereco||row.endereco||'',bairro:cliente?.bairro||row.bairro||'',cidade:cliente?.cidade||row.cidade||'',uf:cliente?.uf||row.uf||'',clienteId:cliente?.id||'',pratos:pratosMapeados,itens:mappedItens,freteGratis:freteInfo.freteGratis,freteOriginal:freteInfo.freteOriginal,total:Number(row.valor_total||0),subtotalBruto:Number(row.subtotal_bruto||row.valor_total||0),desconto:Number(row.desconto||0),cupom:row.cupom_codigo||'',custo:pratosMapeados.reduce((a,p)=>a+Number(p.custo||0),0),lucro:Number(row.valor_total||0)-pratosMapeados.reduce((a,p)=>a+Number(p.custo||0),0),status:({aguardando_confirmacao:'Aguardando confirmação',pedido_feito:'Pedido Feito',preparando:'Preparando',pedido_pronto:'Pedido Pronto',entregue:'Entregue'}[row.status]||row.status||'Aguardando confirmação'),financeiroLancado:pago,estoqueBaixado:false,origem:origemDetectada,data,timeline:[{status:'Importado do Supabase',data}],syncStatus:'sincronizado'};
  }

  function mapClienteSupabase(row){
    return {id:'sbcli_'+row.id,supabase_id:row.id,nome:row.nome||'Cliente',telefone:chaveTelefone(row.telefone||''),cep:row.cep||'',rua:row.rua||'',numero:row.numero||'',complemento:row.complemento||'',endereco:row.endereco||enderecoCompleto(row),bairro:row.bairro||'',cidade:row.cidade||'',uf:row.uf||'',obs:row.observacao||'',createdAt:row.criado_em||row.created_at||new Date().toISOString(),updatedAt:row.atualizado_em||row.updated_at||row.criado_em||new Date().toISOString(),origem:'supabase'};
  }
  ...
  function mapClientePendenteSupabase(row){
    return {id:'sbpend_'+row.id,supabase_id:row.id,nome:row.nome||'',telefone:row.telefone||'',cep:row.cep||'',rua:row.rua||'',numero:row.numero||'',complemento:row.complemento||'',endereco:row.endereco||'',bairro:row.bairro||'',cidade:row.cidade||'',uf:row.uf||'',obs:row.observacao||'',numeroPedido:row.pedido_codigo||'',status:row.status||'pendente',aprovadoEm:row.aprovado_em||'',reprovadoEm:row.reprovado_em||'',createdAt:row.criado_em||new Date().toISOString(),updatedAt:row.atualizado_em||row.criado_em||new Date().toISOString()};
  }
  function chaveClientePendente(c){const tel=norm(c?.telefone||'');return tel?tel:String(c?.id||c?.pedidoId||c?.numeroPedido||'')}
  function deduplicarClientesPendentes(lista){const out=[];const vistos=new Set();(lista||[]).forEach(c=>{const status=(c.status||'pendente');const chave=status+'|'+chaveClientePendente(c);if(vistos.has(chave))return;vistos.add(chave);out.push(c)});return out}
  async function supabaseCarregarClientesPendentes(){
    try{
      const rows=await supabaseRequest('/clientes_pendentes?select=*&order=criado_em.desc&limit=120');
      const remotos=(rows||[]).map(mapClientePendenteSupabase);
      const chavesRemotas=new Set(remotos.map(c=>(c.status||'pendente')+'|'+chaveClientePendente(c)));
      const locais=(db.clientesPendentes||[]).filter(c=>!c.supabase_id&&!chavesRemotas.has((c.status||'pendente')+'|'+chaveClientePendente(c)));
      db.clientesPendentes=deduplicarClientesPendentes([...remotos,...locais]);
      Data.save(db);
  ...
  async function supabaseCarregarPedidos(){
    ...
    try{
      await supabaseGetLoja();
      const rows=await supabaseRequest('/pedidos?select=*&order=criado_em.desc&limit=500');
      ...
      const remotos=(rows||[]).map(r=>{
        ...
        return mapPedidoSupabase(r,pi,cli);
      });
      const chavesRemotas=new Set(remotos.map(p=>p.supabase_id||p.onlineId));
      const locais=(db.pedidos||[]).filter(p=>!p.supabase_id&&!chavesRemotas.has(p.onlineId));
      db.pedidos=[...remotos,...locais].sort((a,b)=>new Date(b.data||0)-new Date(a.data||0));
      await supabaseCarregarClientesOficiais();
      await supabaseCarregarClientesPendentes();
  ...
  async function supabaseSalvarClientePedido(p){
    try{
      ...
      if(!tel&&!p.cliente)return null;

      // Regra de segurança do cadastro:
      // Cliente antigo: usa o cadastro oficial pelo telefone, mas NÃO altera dados automaticamente.
      // Cliente novo: fica em clientes_pendentes para aprovação manual do ADM.
      if(tel){
        try{
          await supabaseCarregarClientesOficiais();
          const localExistente=encontrarClientePorTelefone(tel);
          if(localExistente?.supabase_id)return localExistente.supabase_id;
          if(localExistente?.id)return localExistente.id;
          const existente=await supabaseRequest('/clientes?select=id,nome,telefone&telefone=eq.'+encodeURIComponent(tel)+'&limit=1');
          if(existente?.[0]?.id)return existente[0].id;
        }catch(e){console.warn('Busca cliente oficial Supabase:',e)}
      }

      const payload={loja_id:supabaseLojaId,nome:p.cliente||'Cliente',telefone:tel,cep:p.cep||'',rua:p.rua||'',numero:p.numeroEndereco||p.numero||'',complemento:p.complemento||'',endereco:p.endereco||'',bairro:p.bairro||'',cidade:p.cidade||'',uf:p.uf||'',observacao:p.obs||'',pedido_codigo:p.numero||'',status:'pendente'};

      // Evita criar vários pendentes iguais quando o mesmo telefone faz mais de um pedido antes da aprovação.
      if(tel){
        try{
          const pend=await supabaseRequest('/clientes_pendentes?select=id&telefone=eq.'+encodeURIComponent(tel)+'&status=eq.pendente&order=criado_em.desc&limit=1');
          if(pend?.[0]?.id){
            await supabaseRequest('/clientes_pendentes?id=eq.'+encodeURIComponent(pend[0].id),{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({...payload,atualizado_em:new Date().toISOString()})});
            return null;
          }
        }catch(e){console.warn('Busca cliente pendente Supabase:',e)}
      }

      await supabaseRequest('/clientes_pendentes',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify(payload)});
    }catch(e){console.warn('Cliente pendente Supabase:',e)}
    return null;
  }
  async function supabaseSalvarClienteOficial(c){
    try{
      await supabaseGetLoja();
      const tel=chaveTelefone(c.telefone||'');
      ...
      // Aprovação de cliente novo nunca deve sobrescrever cliente oficial existente.
      // Importante: quando o ADM aprova, o cliente já existe temporariamente no db local.
      // Por isso ignoramos o próprio registro local e só bloqueamos outro cliente ou registro remoto real.
      if(tel){
        try{
          const localExistente=encontrarClientePorTelefone(tel);
          if(localExistente && localExistente.id!==c.id){toast('Telefone já existe em cliente oficial. Cadastro não foi alterado.');return;}
          const f=await supabaseRequest('/clientes?select=id&telefone=eq.'+encodeURIComponent(tel)+'&limit=1');
          if(f?.[0]?.id){toast('Telefone já existe em cliente oficial. Cadastro não foi alterado.');return;}
        }catch(e){}
      }
  ...
  async function supabaseEnviarPedido(p){
    try{
      ...
      const payload={loja_id,cliente_id,codigo,cliente_nome:p.cliente||'',cliente_telefone:chaveTelefone(p.telefone||''),cep:p.cep||'',rua:p.rua||'',numero_endereco:p.numeroEndereco||p.numero||'',complemento:p.complemento||'',endereco:p.endereco||'',bairro:p.bairro||'',cidade:p.cidade||'',uf:p.uf||'',tipo_entrega:(p.tipo==='Delivery'?'delivery':'retirada'),status:statusParaSupabase(p.status||'Aguardando confirmação'),forma_pagamento:p.pagamentoPrevisto||'PIX',talher:(p.talher==='Sim'||p.talher===true),observacao:observacaoPersistidaPedido(p),valor_total:Number(p.total||0),subtotal_bruto:Number(p.subtotalBruto||p.total||0),desconto:Number(p.desconto||0),cupom_codigo:p.cupom||'',pago:!!(p.financeiroLancado||p.pagamentoRecebido),status_pagamento:(p.financeiroLancado||p.pagamentoRecebido)?'pago':'pendente',pago_em:p.dataPagamento||null};
  ...
  async function supabaseAtualizarPedido(p){
    try{
      await supabaseGetLoja();
      let sid=p?.supabase_id||'';
      if(!sid&&String(p?.id||'').startsWith('sb_'))sid=String(p.id).slice(3);
      const codigo=p?.onlineId||p?.codigo||'';
      if(!sid&&codigo){
        const rows=await supabaseRequest('/pedidos?select=id,codigo&codigo=eq.'+encodeURIComponent(codigo)+'&limit=1');
        sid=rows?.[0]?.id||'';
        if(sid)p.supabase_id=sid;
      }
      if(!sid)throw new Error('Pedido sem ID do Supabase. Recarregue a página e tente novamente.');
      const pago=!!(p.financeiroLancado||p.pagamentoRecebido===true);
      const payload={
        status:statusParaSupabase(p.status),
        forma_pagamento:p.pagamento||p.pagamentoPrevisto||'PIX',
        valor_total:Number(p.total||0),
        subtotal_bruto:Number(p.subtotalBruto||p.total||0),
        desconto:Number(p.desconto||0),
        cupom_codigo:p.cupom||'',
        tipo_entrega:(p.tipo==='Delivery'?'delivery':'retirada'),
        cliente_nome:p.cliente||'',
        cliente_telefone:chaveTelefone(p.telefone||''),
        cep:p.cep||'',
        rua:p.rua||'',
        numero_endereco:p.numeroEndereco||p.numero||'',
        complemento:p.complemento||'',
        endereco:p.endereco||'',
        bairro:p.bairro||'',
        cidade:p.cidade||'',
        uf:p.uf||'',
        observacao:observacaoPersistidaPedido(p),
        pago,
        status_pagamento:pago?'pago':'pendente',
        pago_em:pago?(p.dataPagamento||new Date().toISOString()):null
      };
      const retorno='id,codigo,status,pago,status_pagamento,pago_em,forma_pagamento,valor_total,observacao,tipo_entrega';
      let rows=await supabaseRequest('/pedidos?id=eq.'+encodeURIComponent(sid)+'&select='+retorno,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify(payload)});
      if((!rows||!rows.length)&&codigo){
        rows=await supabaseRequest('/pedidos?codigo=eq.'+encodeURIComponent(codigo)+'&select='+retorno,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify(payload)});
      }
      if(!rows||!rows.length)throw new Error('O Supabase não confirmou a alteração. Verifique a política UPDATE da tabela pedidos.');
      // O conteúdo completo dos pratos já foi salvo no campo observação do pedido.
      // A sincronização da tabela pedido_itens fica como compatibilidade e não pode
      // cancelar uma edição válida caso a política DELETE/INSERT dessa tabela bloqueie.
      try{
        await supabaseRequest('/pedido_itens?pedido_id=eq.'+encodeURIComponent(sid),{method:'DELETE',headers:{Prefer:'return=minimal'}});
        const itensAtualizados=((p.pratos&&p.pratos.length)?p.pratos:[{itens:p.itens||[]}]).flatMap((pr,idx)=>(pr.itens||[]).map(i=>{const unidade=Number(i.unidadeCombo||0);const unidadeNome=i.unidadeComboNome||(unidade>0?pr.unidadesCombo?.[unidade-1]?.nome:'')||'';const marcadores=['prato:'+(idx+1)];if(unidade>0){marcadores.push('combo:'+unidade);if(unidadeNome)marcadores.push('comboNome:'+encodeURIComponent(unidadeNome))}return {pedido_id:sid,tipo:i.categoria||i.tipo||'Item',nome:i.nome,quantidade:Number(i.qtdUsada||i.quantidade||1),preco:Number(i.valorCobrado??i.preco??0),observacao:marcadores.join('|')}}));
        if(itensAtualizados.length)await supabaseRequest('/pedido_itens',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify(itensAtualizados)});
      }catch(e){console.warn('Pedido salvo; sincronização auxiliar de pedido_itens foi ignorada:',e)}
      p.supabase_id=rows[0].id;
      p.onlineId=rows[0].codigo||p.onlineId;
      p.syncStatus='sincronizado';
      return true;
    }catch(e){console.warn('Update pedido Supabase:',e);toast('Erro ao salvar pedido no banco: '+String(e.message||e).slice(0,140));return false;}
  }

  async function supabaseExcluirPedido(p){
    try{
      await supabaseGetLoja();
      let sid=p?.supabase_id||'';
      if(!sid&&String(p?.id||'').startsWith('sb_'))sid=String(p.id).slice(3);
      ...
      if(!sid)throw new Error('Pedido sem ID do Supabase para excluir.');
      try{await supabaseRequest('/pedido_itens?pedido_id=eq.'+encodeURIComponent(sid),{method:'DELETE',headers:{Prefer:'return=minimal'}})}catch(e){console.warn('Excluir itens do pedido Supabase:',e)}
      const rows=await supabaseRequest('/pedidos?id=eq.'+encodeURIComponent(sid)+'&select=id',{method:'DELETE',headers:{Prefer:'return=representation'}});
      if(!rows||!rows.length)throw new Error('O Supabase não confirmou a exclusão. Verifique a política DELETE da tabela pedidos.');
      return true;
    }catch(e){console.warn('Excluir pedido Supabase:',e);toast('Erro ao excluir pedido no banco: '+String(e.message||e).slice(0,140));return false;}
  }
  ...
  function adicionarPratoFeitoAoCarrinho(pf,grupos){
    try{if(!pf)return false;const unidades=normalizarUnidadesPratoFeito(pf);if(!Array.isArray(grupos?.[0]))grupos=[grupos||[]];const precoFechado=Math.max(0,Number(pf.preco)||0),itens=[];
      ...
      portalPratos.push({id:uid(),nome:pf.nome||'Prato/Combo',itens,total:precoFechado,custo:itens.reduce((s,i)=>s+(Number(i.custoCalculado)||0),0),tipo:unidades.length>1?'combo':'prato-feito',pratoFeitoId:pf.id,unidadesCombo:unidades.map((u,idx)=>({nome:u.nome,itens:itens.filter(i=>i.unidadeCombo===idx+1)}))});portalPedido={itens:{},qtd:{},tamanhoId:tamanhosAtivos()[0]?.id||''};pratoFeitoConfirmacoesIgnoradas=new Set();renderPortalCliente();return true;
  ...
  function portalSelectedItems(){const tamanho=tamanhoPedidoAtual('portal');const sel=Object.keys(portalPedido.itens||{}).filter(id=>portalPedido.itens[id]).map(id=>{const it=db.itens.find(i=>i.id===id);if(!it)return null;const qtd=it.categoria==='Massa'&&Number(tamanho?.massaGramas)>0?Number(tamanho.massaGramas):normalizarQtd(it,portalPedido.qtd[id]||defaultQtd(it));return {...it,qtdUsada:qtd,porcao:Number(it.porcao||1)};}).filter(Boolean);return applyPrices(sel,'portal')}
  function portalCurrentPrato(){const itens=portalSelectedItems();if(!itens.length)return null;const custo=itens.reduce((a,i)=>a+Number(i.custoCalculado||0),0);const temBase=itens.some(i=>['Massa','Molho','Proteína','Complemento','Finalização'].includes(i.categoria));const tamanho=tamanhoPedidoAtual('portal');const total=(temBase?Number(tamanho?.preco||0):0)+itens.reduce((a,i)=>a+Number(i.valorCobrado||0),0);return {id:uid(),nome:temBase?'Prato '+(portalPratos.length+1):'Itens avulsos',itens,total,custo,tamanho:snapshotTamanho(tamanho)};}
  ...
  function adicionarPratoPortal(){if(!horarioAtualLoja().aberta){mostrarAvisoLojaFechada();return toast('Loja online está fechada.');}const prato=portalCurrentPrato();if(!prato)return toast('Selecione itens para adicionar ao pedido.');portalPratos.push({...prato,nome:'Prato '+(portalPratos.length+1)});portalPedido={itens:{},qtd:{},tamanhoId:tamanhosAtivos()[0]?.id||''};renderPortalCliente();setTimeout(()=>scrollPortalToCategory('Massa'),120);toast('Prato adicionado ao carrinho. Você pode montar outro.');}
  ...
  function limparSelecoesIndisponiveisPortal(mostrarAviso=false){
    let removidos=[];
    Object.keys(portalPedido.itens||{}).forEach(id=>{
      if(!portalPedido.itens[id])return;
      const item=db.itens.find(i=>i.id===id);
      if(!item||!itemDisponivelVenda(item)){
        removidos.push(item?.nome||'item removido');
        delete portalPedido.itens[id];
        delete portalPedido.qtd[id];
      }
    });
    if(removidos.length&&mostrarAviso)toast('Atualizamos seu prato: '+removidos.join(', ')+' ficou indisponível.');
    return removidos;
  }
  function estoqueDisponivelTexto(item){
    const qtd=fmtQtd(item?.estoque||0);
    const nome=item?.nome||'item';
    const un=String(item?.unidade||'un').toLowerCase();
    if(item?.categoria==='Massa')return `Apenas ${qtd} ${Number(item?.estoque||0)===1?'porção':'porções'} de ${nome} disponíveis.`;
    if(un==='un'||un==='unidade'||un==='unidades')return `Apenas ${qtd} ${nome} disponíveis.`;
    return `Estoque insuficiente para ${nome}. Disponível: ${qtd} ${item?.unidade||''}.`;
  }
  ...
  function scrollPortalTo(id){
    ...
    const extra=id==='portalCarrinho'?130:90;
  ...
  function scrollPortalToCategory(cat){
    ...
    if(target){const y=target.getBoundingClientRect().top+window.pageYOffset-90;window.scrollTo({top:Math.max(0,y),behavior:'smooth'});}
  ...
  function renderAvisoLojaFechada(aberta,forcar=false){
    ...
    const contatos=[
      w?`<a href="https://wa.me/55${w}" target="_blank">📲 Reservar pelo WhatsApp</a>`:'',
      insta?`<a href="https://instagram.com/${insta}" target="_blank">📷 Acompanhe nosso Instagram</a>`:''
    ].filter(Boolean).join('');
    box.classList.remove('hidden');
    box.innerHTML=`<div class="portal-closed-card"><button type="button" class="portal-closed-close" onclick="App.fecharAvisoLojaFechada()">×</button><strong>🍝 No momento estamos fechados</strong><p>Nosso cardápio continua disponível para consulta.<br>Você pode reservar seu pedido pelo WhatsApp ou realizá-lo quando a loja estiver aberta.</p>${contatos?`<div class="portal-closed-links">${contatos}</div>`:''}</div>`;
  }

  function renderPortalCliente(){renderSeletorTamanhos('portal');
    ...
    if(el('onlineTempoPreparo'))el('onlineTempoPreparo').remove();
    ...
    if(val('portalTipo')!=='Delivery'&&el('portalConfirmaEndereco'))el('portalConfirmaEndereco').checked=false;if(val('portalTipo')!=='Delivery')fretePortal={status:'idle',distancia:0,valor:0,mensagem:'Retirada selecionada: sem frete.'};atualizarFreteStatus('portal',fretePortal);
    ...
    if(box){
      if(!grupos.length){
        ...
        box.innerHTML=grupos.map(({cat,itens})=>{const gratisInfo=portalGratisLabel(cat);return `<section class="portal-cat" id="${portalCategoryId(cat)}"><h4>${iconeCat(cat)} ${cat}${gratisInfo?` <span class="portal-free-label">${gratisInfo}</span>`:''}</h4><div class="portal-grid">${itens.map(i=>{const sel=!!portalPedido.itens[i.id];const semEstoque=!itemDisponivelVenda(i);const qtd=portalPedido.qtd[i.id]||defaultQtd(i);return `<div role="button" tabindex="0" class="option compact-option ${sel?'selected':''} ${semEstoque?'disabled-option':''}" onclick="App.togglePortalItem('${i.id}')"><div class="icon small-icon">${i.imagem?`<img src="${i.imagem}" alt="${i.nome}">`:(i.icone||iconeCat(cat))}</div><strong>${i.nome}</strong><span>${semEstoque?(controlaEstoqueNumerico(i)?'Esgotado':'Indisponível'):fmt(i.preco)}</span>${semEstoque?'<small class="stock-mini alert-text">'+(controlaEstoqueNumerico(i)?'Sem estoque. Escolha outro item.':'Item indisponível no momento.')+'</small>':''}${sel&&!semEstoque&&!['Massa','Molho','Finalização'].includes(cat)?`<div class="qty-inline qty-stepper" onclick="event.stopPropagation()"><button type="button" aria-label="Diminuir" onclick="App.setPortalPorcao('${i.id}',-1)">−</button><strong>${porcoesPortalItem(i)}</strong><button type="button" aria-label="Aumentar" onclick="App.setPortalPorcao('${i.id}',1)">+</button></div>`:''}${sel&&!semEstoque&&['Massa','Molho','Finalização'].includes(cat)?`<small class="stock-mini">Porção fixa</small>`:''}</div>`}).join('')}</div></section>`}).join('')
      }
    }
    const current=aberta?portalCurrentPrato():null;const pratos=portalPedidoCompleto();const entrega=(val('portalTipo')==='Delivery'?freteAtual('portal'):0);const brutoPortal=pratos.reduce((a,p)=>a+Number(p.total||0),0)+entrega;const cupPortal=usarCupomSeValido(val('portalCupom'),brutoPortal);const descontoPortal=(val('portalCupom')&&cupPortal.ok)?Number(cupPortal.desconto||0):0;const total=Math.max(0,brutoPortal-descontoPortal);
    const resumo=el('portalResumo');if(resumo){if(!aberta){resumo.className='summary empty-summary';const w=norm(db.config.whatsapp||'');const insta=(db.config.instagram||'').trim().replace(/^@/,'');const contatos=[w?`<a class="inline-link" href="https://wa.me/55${w}" target="_blank">📲 Reservar pelo WhatsApp</a>`:'',insta?`<a class="inline-link" href="https://instagram.com/${insta}" target="_blank">📷 Acompanhe nosso Instagram</a>`:''].filter(Boolean).join('<br>');resumo.innerHTML='<strong>🍝 No momento estamos fechados</strong><br><br>Nosso cardápio continua disponível para consulta.<br>Você pode reservar seu pedido pelo WhatsApp ou realizá-lo quando a loja estiver aberta.'+(contatos?'<br><br>'+contatos:'')}else if(!pratos.length){resumo.className='summary empty-summary';resumo.textContent='Selecione itens para simular o pedido do cliente'}else{resumo.className='summary';resumo.innerHTML=pratos.map((pr,idx)=>`<div class="summary-block"><div class="summary-item"><strong>${pr.nome}${pr.tamanho?.nome?` • ${pr.tamanho.nome}`:''}${idx===portalPratos.length&&current?' (em montagem)':''}</strong>${idx<portalPratos.length?`<button class="mini danger" onclick="App.removerPratoPortal('${pr.id}')">Remover</button>`:''}</div>${(pr.itens||[]).map(i=>`<div class="summary-item"><div><strong>${i.nome}</strong><br><small>${i.categoria} • ${i.porcoes||porcoesItem(i)}${(i.categoria==='Complemento'||i.categoria==='Proteína'||i.categoria==='Finalização')?` • ${i.porcoesGratis||0} grátis / ${i.porcoesPagas||0} paga(s)`:''}</small></div><span class="${i.valorCobrado===0?'discount':''}">${i.valorCobrado===0?'Grátis':fmt(i.valorCobrado)}</span></div>`).join('')}<div class="summary-item"><small>Subtotal</small><strong>${fmt(pr.total||0)}</strong></div></div>`).join('')+(entrega?`<div class="summary-item"><strong>Entrega</strong><strong>${fmt(entrega)}</strong></div>`:'')+(descontoPortal?`<div class="summary-item discount"><strong>Cupom ${cupPortal.codigo}</strong><strong>- ${fmt(descontoPortal)}</strong></div>`:'')+`<div class="summary-item total-line"><strong>Total previsto</strong><strong>${fmt(total)}</strong></div>`}}
  ...
  function togglePortalItem(id){if(!horarioAtualLoja().aberta){mostrarAvisoLojaFechada();return toast('Loja online está fechada.');}const it=db.itens.find(i=>i.id===id);if(!it)return;if(!itemDisponivelVenda(it))return toast(controlaEstoqueNumerico(it)?'Este item está sem estoque. Escolha outro.':'Este item está indisponível no momento.');const vaiSelecionar=!portalPedido.itens[id];if(vaiSelecionar)limparSelecaoExclusivaPortal(it.categoria,id);portalPedido.itens[id]=vaiSelecionar;if(portalPedido.itens[id]&&!portalPedido.qtd[id])portalPedido.qtd[id]=defaultQtd(it||{});if(!portalPedido.itens[id])delete portalPedido.qtd[id];const selecionado=portalPedido.itens[id];renderPortalCliente();if(selecionado){toast((it.categoria==='Massa'||it.categoria==='Molho')?it.nome+' selecionado para o prato':it.nome+' adicionado ao prato');setTimeout(()=>{const btn=[...document.querySelectorAll('.compact-option')].find(b=>b.textContent.includes(it.nome));if(btn){btn.classList.add('just-added');setTimeout(()=>btn.classList.remove('just-added'),700)}},40)}}
  function portalGratisLabel(cat){
    return ['Proteína','Complemento'].includes(cat)?`Inclui ${limiteCategoriaTamanho(cat,'portal')} • escolhas adicionais são cobradas à parte`:labelGratisCategoria(cat);
  }
  function porcoesPortalItem(it){const qtd=Number(portalPedido.qtd[it.id]||defaultQtd(it));const base=Math.max(tamanhoPorcaoEstoque(it),0.0001);return Math.max(1,Math.round(qtd/base));}
  function setPortalPorcao(id,delta){const it=db.itens.find(i=>i.id===id);if(!it)return;if(['Massa','Molho','Finalização'].includes(it.categoria))return;const atual=porcoesPortalItem(it);const prox=Math.max(1,atual+Number(delta||0));portalPedido.qtd[id]=prox*tamanhoPorcaoEstoque(it);renderPortalCliente()}
  function setPortalQtd(id,v){const it=db.itens.find(i=>i.id===id);if(!it)return;if(['Massa','Molho','Finalização'].includes(it.categoria)){portalPedido.qtd[id]=defaultQtd(it)}else{const porcoes=Math.max(1,Math.round(Number(v||1)));portalPedido.qtd[id]=porcoes*tamanhoPorcaoEstoque(it)}renderPortalCliente()}
  ...
  function aplicarClientePortal(c,showToast=false){const box=el('portalClienteStatus');if(!c||!box)return null;portalClienteIdentificado=c;if(el('portalCliente'))el('portalCliente').value=c.nome||'';setPortalTelefone(c.telefone||portalTelefoneValor());[['portalCep','cep'],['portalRua','rua'],['portalNumero','numero'],['portalComplemento','complemento'],['portalEndereco','endereco'],['portalBairro','bairro'],['portalCidade','cidade'],['portalUf','uf']].forEach(([id,k])=>{if(el(id))el(id).value=c[k]||''});const ult=pedidosDoCliente(c)[0];box.className='portal-welcome ok-box';box.innerHTML=`<strong>Bem-vindo de volta, ${c.nome}!</strong><br><small>Dados preenchidos automaticamente. Se for delivery, confirme o endereço antes de enviar o pedido.</small>${formatarUltimoPedido(ult)}`;if(showToast)toast('Cliente localizado');renderPortalCliente();return c}
  function selecionarClientePortalOnline(id){const c=(db.clientes||[]).find(x=>x.id===id);return aplicarClientePortal(c,true)}
  async function buscarClientePortal(showToast=false){
    const tel=portalTelefoneValor();const box=el('portalClienteStatus');if(!box)return null;
    if(tel.length<8){portalClienteIdentificado=null;box.className='portal-welcome muted';box.innerHTML='Informe o telefone para localizar cadastro, histórico e último pedido.';return null}
    let c=encontrarClientePorTelefone(tel);
    if(!c){
      try{await supabaseCarregarClientesOficiais();c=encontrarClientePorTelefone(tel);}catch(e){console.warn('Recarregar clientes para busca:',e)}
    }
    if(!c){
      portalClienteIdentificado=null;box.className='portal-welcome warn-box';box.innerHTML='Não encontramos seu cadastro. Você pode continuar olhando o cardápio; ao pedir, preencherá os dados obrigatórios.';if(showToast)toast('Cliente não encontrado. Continue como novo cliente.');return null
    }
    if(showToast)return aplicarClientePortal(c,true);
    box.className='portal-welcome ok-box';box.innerHTML=`Cadastro encontrado: <button class="link-button" onclick="App.selecionarClientePortalOnline('${c.id}')"><strong>${c.nome||'Cliente'}</strong></button><br><small>Clique no nome para usar esse cadastro neste pedido.</small>`;return c
  }
  function itemEstoqueCompativel(item){if(!item)return null;const itens=db.itens||[];return itens.find(x=>x.id&&item.id&&x.id===item.id)||itens.find(x=>x.supabase_id&&item.supabase_id&&x.supabase_id===item.supabase_id)||itens.find(x=>x.nome===item.nome&&(!item.categoria||x.categoria===item.categoria))||itens.find(x=>norm(x.nome)===norm(item.nome||'')&&(!item.categoria||x.categoria===item.categoria))||null}
  function repetirUltimoPedidoPortal(id){const p=(db.pedidos||[]).find(x=>x.id===id);if(!p)return;portalPedido={itens:{},qtd:{},tamanhoId:tamanhosAtivos()[0]?.id||''};portalPratos=[];const pratosFonte=(p.pratos&&p.pratos.length)?p.pratos:[{nome:'Prato 1',itens:p.itens||[]}];let ignorados=[];pratosFonte.forEach((pr,idx)=>{const itensBase=(pr.itens||[]).map(i=>{const base=itemEstoqueCompativel(i);if(!base){ignorados.push(i.nome||'item');return null}return {...base,qtdUsada:Number(i.qtdUsada||i.qtd||i.quantidadeUsada||1),porcao:Number(i.porcao||base.porcao||1)}}).filter(Boolean);if(!itensBase.length)return;portalPedido.tamanhoId=pr.tamanho?.id||tamanhosAtivos()[0]?.id||'';const precificados=applyPrices(itensBase,'portal');const tam=tamanhoPedidoAtual('portal');const temBase=precificados.some(i=>['Massa','Molho','Proteína','Complemento','Finalização'].includes(i.categoria));portalPratos.push({id:uid(),nome:pr.nome||('Prato '+(idx+1)),itens:precificados,total:(temBase?Number(tam?.preco||0):0)+precificados.reduce((a,i)=>a+Number(i.valorCobrado||0),0),custo:precificados.reduce((a,i)=>a+Number(i.custoCalculado||0),0),tamanho:snapshotTamanho(tam)});});renderPortalCliente();setTimeout(()=>scrollPortalTo('portalCarrinho'),120);if(portalPratos.length){toast(ignorados.length?'Último pedido carregado parcialmente. Alguns itens não existem mais no cardápio.':'Último pedido carregado no carrinho')}else{toast('Não foi possível carregar: os itens do último pedido não existem mais no cardápio atual.')}}

  function limparPortalCliente(){if(!confirm('Deseja realmente limpar seu pedido?'))return;portalPedido={itens:{},qtd:{},tamanhoId:tamanhosAtivos()[0]?.id||''};portalPratos=[];portalClienteIdentificado=null;['portalCliente','portalTelefone','portalTelefoneFinal','portalCep','portalRua','portalNumero','portalComplemento','portalEndereco','portalBairro','portalCidade','portalUf','portalObs','portalCupom'].forEach(id=>{const x=el(id);if(x)x.value=''});if(el('portalConfirmaEndereco'))el('portalConfirmaEndereco').checked=false;if(el('portalClienteStatus')){el('portalClienteStatus').className='portal-welcome muted';el('portalClienteStatus').innerHTML='Informe o telefone para localizar cadastro, histórico e último pedido.'}if(el('portalTipo'))el('portalTipo').value='Retirada';if(el('portalPagamento'))el('portalPagamento').value='PIX';if(el('portalTrocoPara'))el('portalTrocoPara').value='';alternarCamposTroco('portal');if(el('portalTalher'))el('portalTalher').value='Não';if(el('portalWhatsapp'))el('portalWhatsapp').textContent='Nenhum pedido externo criado ainda.';renderPortalCliente();setTimeout(()=>scrollPortalTo('page-portal'),120);toast('Pedido limpo')}
  function mensagemWhatsPedido(p,itens,total){
    ...
    pratosMsg.forEach((pr,idx)=>{
      linhas.push(`${pr.nome||('Prato '+(idx+1))}:`);
      const porCat={};
      (pr.itens||[]).forEach(i=>{const cat=i.categoria||i.tipo||'Item';porCat[cat]=porCat[cat]||new Map();const nome=String(i.nome||'Item');porCat[cat].set(nome,(porCat[cat].get(nome)||0)+porcoesPreparoItem(i));});
      ['Massa','Molho','Proteína','Complemento','Finalização','Bebida','Sobremesa','Item'].forEach(cat=>{
        if(porCat[cat]?.size){
          const arr=[...porCat[cat].entries()].map(([nome,q])=>cat==='Massa'?nome:(q>1?`${q}x ${nome}`:nome));
          linhas.push(`- ${cat}: ${arr.join(', ')}`);
        }
      });
    });
    if(p.cupom){linhas.push('', `Subtotal: ${fmt(p.subtotalBruto||total+Number(p.desconto||0))}`, `Cupom ${p.cupom}: - ${fmt(p.desconto||0)}`);}linhas.push('', `Total previsto: ${fmt(total)}`, `Pagamento previsto: ${p.pagamentoPrevisto||'A combinar'}`, '', 'Aguardando confirmação.');
    return linhas.join('\n');
  }
  ...
  function validarDadosPortal(){
    ...
    const endereco=(enderecoCompleto({rua:val('portalRua'),numero:val('portalNumero'),complemento:val('portalComplemento'),endereco:val('portalEndereco')})||'').trim();
  ...
  async function criarPedidoPortal(){
    if(val('portalTipo')==='Delivery'){if(fretePortal.status==='fora')return toast('Este endereço está fora da área de entrega.');if(fretePortal.status==='calculando')return toast('Aguarde o cálculo do frete.');if(fretePortal.status!=='ok')await calcularFreteEndereco('portal');if(fretePortal.status==='fora')return toast('Este endereço está fora da área de entrega.');}
    ...
    if(!await validarEstoqueItensAtualizado(itens))return;
    ...
    const entrega=tipo==='Delivery'?freteAtual('portal'):0;const bruto=pratos.reduce((a,pr)=>a+Number(pr.total||0),0)+entrega;if(val('portalCupom')&&supabaseLojaId)await supabaseCarregarCupons();const cup=usarCupomSeValido(val('portalCupom'),bruto);if(val('portalCupom')&&!cup.ok)return toast(cup.msg||'Cupom inválido');const desconto=Number(cup.desconto||0);const total=Math.max(0,bruto-desconto);const custo=pratos.reduce((a,pr)=>a+Number(pr.custo||0),0);
    const numero=await proximoNumeroPedidoSeguro();
    const pago=false;const agora=new Date().toISOString();const pagamentoPrevisto=val('portalPagamento')||'PIX';const trocoPara=pagamentoPrevisto==='Dinheiro'?num('portalTrocoPara'):0;const p={id:uid(),onlineId:'PED-'+hoje().replace(/-/g,'')+'-'+numero,numero,cliente,telefone,tipo:tipo==='Retirada'?'Balcão':tipo,pagamento:pago?pagamentoPrevisto:'',pagamentoPrevisto,talher,trocoPara,obs:obsComTroco(val('portalObs','portalCupom'),trocoPara),cep:val('portalCep'),rua:val('portalRua'),numeroEndereco:val('portalNumero'),complemento:val('portalComplemento'),endereco:enderecoCompleto({rua:val('portalRua'),numero:val('portalNumero'),complemento:val('portalComplemento'),endereco:val('portalEndereco')}),bairro:val('portalBairro').trim(),cidade:val('portalCidade'),uf:val('portalUf'),clienteId:portalClienteIdentificado?.id||'',pratos:pratos.map((pr,idx)=>({...pr,nome:pr.nome||('Prato '+(idx+1))})),itens,total,frete:tipo==='Delivery'?freteAtual('portal'):0,distanciaEntregaKm:tipo==='Delivery'?Number(fretePortal.distancia||0):0,subtotalBruto:bruto,desconto,cupom:cup.codigo||'',cupomId:cup.cupom?.id||'',custo,lucro:total-custo,status:'Aguardando confirmação',pagamentoRecebido:pago,financeiroLancado:pago,dataPagamento:pago?agora:'',estoqueBaixado:false,origem:'online',data:agora,timeline:[{status:'Aguardando confirmação',data:agora,pagamentoRecebido:pago}],syncStatus:'local'};
    ...
    if(enviadoSupabase){log('Pedido sincronizado no Supabase','#'+p.numero);if(cup.codigo){const usoOk=await registrarUsoCupom(cup.codigo);if(!usoOk)toast('Pedido salvo, mas o uso do cupom não foi atualizado. Confira no ADM.')}}
    ...
    portalPedido={itens:{},qtd:{},tamanhoId:tamanhosAtivos()[0]?.id||''};portalPratos=[];renderPortalCliente();
  ...
  async function confirmarPedidoPortal(id){const p=db.pedidos.find(x=>x.id===id);if(!p)return;if(!await validarEstoqueItensAtualizado(p.itens||[]))return;const anterior=deepCopy(p);if(!p.estoqueBaixado){baixarEstoque(p.itens||[]);p.estoqueBaixado=true}p.status='Pedido Feito';p.timeline=p.timeline||[];p.timeline.push({status:'Pedido Feito',data:new Date().toISOString(),detalhes:'Pedido externo confirmado'});log('Pedido externo confirmado','#'+p.numero);save();renderAll();const ok=await supabaseAtualizarPedido(p);if(!ok){Object.assign(p,anterior);save();renderAll();return toast('Não foi possível confirmar no banco. A alteração foi desfeita.')}await supabaseCarregarPedidos();renderAll();toast('Pedido confirmado e enviado para preparo')}

  
  async function supabaseSalvarCupom(c){
    if(!supabaseLojaId)return false;
    try{
      const payload={loja_id:supabaseLojaId,nome:c.nome,tipo:c.tipo,desconto:Number(c.desconto||0),ativo:c.ativo!==false,observacao:c.obs||'',valido_ate:c.validoAte||null,limite:Number(c.limite||0),usados:Number(c.usados||0),atualizado_em:new Date().toISOString()};
      const q=c.supabase_id?('/cupons?id=eq.'+encodeURIComponent(c.supabase_id)):('/cupons?nome=eq.'+encodeURIComponent(c.nome));
      let rows=await supabaseRequest(q,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify(payload)}).catch(()=>[]);
      if(!rows||!rows.length)rows=await supabaseRequest('/cupons?select=*',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(payload)});
      if(rows&&rows[0]){c.supabase_id=rows[0].id;c.syncStatus='sincronizado'}
      return true;
    }catch(e){console.warn('Cupom Supabase:',e);return false}
  }
  async function supabaseCarregarCupons(){
    if(!supabaseLojaId)return;
    try{
      const rows=await supabaseRequest('/cupons?select=*&order=criado_em.desc');
      if(Array.isArray(rows))db.cupons=rows.map(r=>({id:'sbcup_'+r.id,supabase_id:r.id,nome:r.nome,tipo:r.tipo||'percentual',desconto:Number(r.desconto||0),ativo:r.ativo!==false,obs:r.observacao||'',validoAte:r.valido_ate||'',limite:Number(r.limite||0),usados:Number(r.usados||0),createdAt:r.criado_em,updatedAt:r.atualizado_em,syncStatus:'sincronizado'}));
      save();renderCupons();renderPortalCliente();
    }catch(e){console.warn('Carregar cupons:',e)}
  }
  async function salvarCupom(){
    const nome=val('cupomNome').trim().toUpperCase();
    if(!nome)return toast('Informe o nome do cupom');
    const desconto=Number(val('cupomDesconto')||0);
    if(desconto<=0)return toast('Informe o desconto');
    const tipo=val('cupomTipo')||'percentual';
    const ativo=!!el('cupomAtivo')?.checked;
    const obs=val('cupomObs');
    const validoAte=val('cupomValidoAte');
    const limite=Math.max(0,Number(val('cupomLimite')||0));
    db.cupons=db.cupons||[];
    const idxEdit=cupomEditandoId?db.cupons.findIndex(c=>c.id===cupomEditandoId):-1;
    const idxNome=db.cupons.findIndex(c=>String(c.nome||'').toUpperCase()===nome);
    if(idxNome>=0&&idxNome!==idxEdit)return toast('Já existe um cupom com esse nome/código');
    const idx=idxEdit>=0?idxEdit:idxNome;
    const anterior=idx>=0?db.cupons[idx]:null;
    const obj={
      id:anterior?anterior.id:uid(),
      supabase_id:anterior?anterior.supabase_id:null,
      nome,tipo,desconto,ativo,obs,validoAte,limite,
      usados:anterior?Number(anterior.usados||0):0,
      createdAt:anterior?anterior.createdAt:new Date().toISOString(),
      updatedAt:new Date().toISOString()
    };
    if(idx>=0)db.cupons[idx]=obj;else db.cupons.unshift(obj);
    save();
    const ok=await supabaseSalvarCupom(obj);
    if(supabaseLojaId&&!ok)return toast('Não consegui salvar o cupom no banco. Confira conexão/permissões.');
    cupomEditandoId=null;
    limparCupomForm(false);
    await supabaseCarregarCupons();
    renderCupons();
    toast('Cupom salvo')
  }
  function editarCupom(id){
    const c=(db.cupons||[]).find(x=>x.id===id);
    if(!c)return toast('Cupom não encontrado');
    cupomEditandoId=id;
    if(el('cupomNome'))el('cupomNome').value=c.nome||'';
    if(el('cupomTipo'))el('cupomTipo').value=c.tipo||'percentual';
    if(el('cupomDesconto'))el('cupomDesconto').value=Number(c.desconto||0);
    if(el('cupomValidoAte'))el('cupomValidoAte').value=c.validoAte||'';
    if(el('cupomLimite'))el('cupomLimite').value=Number(c.limite||0);
    if(el('cupomAtivo'))el('cupomAtivo').checked=c.ativo!==false;
    if(el('cupomObs'))el('cupomObs').value=c.obs||'';
    const btn=document.querySelector('#page-cupons button.primary.full');
    if(btn)btn.textContent='Salvar alterações do cupom';
    toast('Editando cupom '+(c.nome||''));
    try{document.querySelector('#page-cupons .card')?.scrollIntoView({behavior:'smooth',block:'start'})}catch(e){}
  }
  function limparCupomForm(mostrar=true){
    cupomEditandoId=null;
    ['cupomNome','cupomDesconto','cupomValidoAte','cupomLimite','cupomObs'].forEach(id=>{if(el(id))el(id).value=''});
    if(el('cupomTipo'))el('cupomTipo').value='percentual';
    if(el('cupomAtivo'))el('cupomAtivo').checked=true;
    const btn=document.querySelector('#page-cupons button.primary.full');
    if(btn)btn.textContent='Salvar cupom';
    if(mostrar)toast('Formulário de cupom limpo');
  }
  async function supabaseExcluirCupom(c){
    if(!supabaseLojaId||!c)return true;
    try{
      if(c.supabase_id){
        await supabaseRequest('/cupons?id=eq.'+encodeURIComponent(c.supabase_id),{method:'DELETE'});
      }else{
        await supabaseRequest('/cupons?nome=eq.'+encodeURIComponent(c.nome),{method:'DELETE'});
      }
      return true;
    }catch(e){console.warn('Excluir cupom Supabase:',e);return false}
  }
  async function removerCupom(id){
    const c=(db.cupons||[]).find(x=>x.id===id);
    if(!c)return toast('Cupom não encontrado');
    const usado=Number(c.usados||0)>0||(db.pedidos||[]).some(p=>String(p.cupom||'').toUpperCase()===String(c.nome||'').toUpperCase()||String(p.cupomId||'')===String(c.id||'')||String(p.cupomId||'')===String(c.supabase_id||''));
    if(usado){
      if(!confirm('Esse cupom já foi usado em pedido. Para preservar o histórico, ele será apenas inativado. Continuar?'))return;
      c.ativo=false;c.updatedAt=new Date().toISOString();
      save();
      await supabaseSalvarCupom(c);
      await supabaseCarregarCupons();
      renderCupons();
      return toast('Cupom inativado para preservar histórico');
    }
    if(!confirm('Excluir cupom?'))return;
    const ok=await supabaseExcluirCupom(c);
    if(supabaseLojaId&&!ok){
      c.ativo=false;c.updatedAt=new Date().toISOString();
      await supabaseSalvarCupom(c);
      toast('Não consegui excluir fisicamente no banco; cupom foi inativado.');
    }else{
      db.cupons=(db.cupons||[]).filter(x=>x.id!==id);
  ...
  function toggleAssistentePorcoes(){
    const card=el('assistentePorcoesCard');
    if(!card)return;
    const collapsed=card.classList.toggle('collapsed');
    if(el('assistentePorcoesToggle'))el('assistentePorcoesToggle').textContent=collapsed?'Abrir assistente':'Fechar assistente';
    if(!collapsed)renderAssistentePorcoes(true);
  }
  function renderAssistentePorcoes(force=false){
    ...
    box.innerHTML=`<div class="setup-head"><b>Item</b><b>Unidade estoque</b><b>Qtd. por porção</b><b>Unidade porção</b></div>`+itens.map(i=>{
      ...
      const thumb=i.imagem?`<img class="setup-thumb" src="${i.imagem}" alt="${i.nome}">`:`<span class="setup-thumb fallback">${i.icone||iconeCat(i.categoria)}</span>`;
      return `<div class="setup-row" data-porcao-item="${i.id}"><div class="setup-item-info">${thumb}<div class="setup-item-text"><strong>${i.nome}</strong><small>${i.categoria}</small></div></div><select data-field="unidade">${opts.replace('>'+ue+'<',' selected>'+ue+'<')}</select><input data-field="porcao" type="number" step="0.01" value="${pq}"><select data-field="porcaoUnidade">${opts.replace('>'+pu+'<',' selected>'+pu+'<')}</select></div>`;
    }).join('');
  ...
  function logoutAdmin(){
    try{sessionStorage.removeItem(ADMIN_SESSION_KEY);sessionStorage.removeItem(ADMIN_ROLE_KEY);}catch(e){}
    location.href=location.pathname+'#admin';
    location.reload();
  }

  function aplicarPerfilAcesso(){
    const role=perfilAtual();
    document.body.classList.toggle('perfil-operacao', role==='operacao');
    document.body.classList.toggle('perfil-admin', role!=='operacao');
    document.querySelectorAll('.nav button').forEach(btn=>{
      const pg=btn.dataset.page;
      btn.hidden=(role==='operacao'&&!paginaPermitidaOperacao(pg));
    });
    document.querySelectorAll('.nav-group').forEach(group=>{
      const visible=[...group.querySelectorAll('button')].some(b=>!b.hidden);
      group.hidden=!visible;
      if(visible&&role==='operacao')group.open=true;
    });
    const sideMode=el('sideMode');
    if(sideMode)sideMode.textContent=role==='operacao'?'Perfil Operação • Supabase':'Perfil Administrador • Supabase';
    const mobileNav=el('mobileOpNav');
  ...
  function accessSessionId(){
    try{
      ...
      if(!id){id=(crypto?.randomUUID?.()||('sess-'+Date.now()+'-'+Math.random().toString(36).slice(2)));sessionStorage.setItem(ACCESS_SESSION_KEY,id)}
      return id;
    }catch(e){return 'sess-'+Date.now()+'-'+Math.random().toString(36).slice(2)}
  }
  function setAccessStatus(text,error=false){const x=el('acessosStatus');if(x){x.textContent=text;x.className='muted access-status'+(error?' error':'')}}
  function renderOnlineCount(){const x=el('acessosOnline');if(x)x.textContent=String(accessOnlineCount||0)}
  function contarPresencas(state){
    const keys=new Set();
    Object.entries(state||{}).forEach(([key,items])=>{(items||[]).forEach(item=>keys.add(item.session_id||item.presence_ref||key))});
    accessOnlineCount=keys.size;renderOnlineCount();
  }
  async function iniciarPresencaAcessos(){
    if(accessPresenceChannel||!window.supabase?.createClient||!SUPABASE_KEY())return;
    try{
      accessRealtimeClient=window.supabase.createClient((db?.sync?.supabaseUrl||SUPABASE_DEFAULT_URL).replace(/\/$/,''),SUPABASE_KEY(),{auth:{persistSession:false,autoRefreshToken:false}});
      accessPresenceChannel=accessRealtimeClient.channel('donasos-loja-presenca',{config:{presence:{key:isPublicClient()?accessSessionId():'painel-'+accessSessionId()}}});
      accessPresenceChannel.on('presence',{event:'sync'},()=>contarPresencas(accessPresenceChannel.presenceState()));
      accessPresenceChannel.on('presence',{event:'join'},()=>contarPresencas(accessPresenceChannel.presenceState()));
      accessPresenceChannel.on('presence',{event:'leave'},()=>contarPresencas(accessPresenceChannel.presenceState()));
      accessPresenceChannel.subscribe(async status=>{
        if(status==='SUBSCRIBED'){
          if(isPublicClient())await accessPresenceChannel.track({session_id:accessSessionId(),tipo:'cliente',online_at:new Date().toISOString()});
          else contarPresencas(accessPresenceChannel.presenceState());
          setAccessStatus('Contador online conectado ao Supabase.');
        }else if(status==='CHANNEL_ERROR'||status==='TIMED_OUT')setAccessStatus('Não foi possível conectar o contador em tempo real.',true);
      });
    }catch(e){console.warn('Presença online indisponível',e);setAccessStatus('Contador online temporariamente indisponível.',true)}
  }
  async function registrarAcessoLoja(){
    if(!isPublicClient()||!supabaseAtivo())return;
    const marker=ACCESS_SESSION_KEY+'_registered';
    try{if(sessionStorage.getItem(marker))return;sessionStorage.setItem(marker,'pending')}catch(e){}
    try{
      const response=await supabaseRequest('/'+ACCESS_TABLE,{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({session_id:accessSessionId(),created_at:new Date().toISOString()})});
      if(response===false)throw new Error('Falha ao registrar acesso');
      try{sessionStorage.setItem(marker,'ok')}catch(e){}
    }catch(e){console.warn('Histórico de acessos não configurado',e);try{sessionStorage.removeItem(marker)}catch(_){} }
  }
  function inicioDiaLocal(offset=0){const d=new Date();d.setHours(0,0,0,0);d.setDate(d.getDate()+offset);return d}
  function renderHistoricoAcessos(){
    const now=new Date(),hoje=inicioDiaLocal(0),amanha=inicioDiaLocal(1),ontem=inicioDiaLocal(-1),sete=inicioDiaLocal(-6);
    const datas=accessHistoryCache.map(r=>new Date(r.created_at)).filter(d=>!isNaN(d));
    const count=(a,b)=>datas.filter(d=>d>=a&&d<b).length;
    if(el('acessosHoje'))el('acessosHoje').textContent=count(hoje,amanha);
    if(el('acessosOntem'))el('acessosOntem').textContent=count(ontem,hoje);
    if(el('acessosSeteDias'))el('acessosSeteDias').textContent=count(sete,amanha);
    const box=el('acessosResumoDiario');if(box){
      const rows=[];for(let i=0;i<7;i++){const a=inicioDiaLocal(-i),b=inicioDiaLocal(1-i);const label=i===0?'Hoje':i===1?'Ontem':a.toLocaleDateString('pt-BR',{weekday:'short',day:'2-digit',month:'2-digit'});rows.push(`<div class="access-day-row"><span>${label}</span><strong>${count(a,b)}</strong></div>`)}
      box.innerHTML=rows.join('');
    }
    renderOnlineCount();
  }
  async function atualizarPainelAcessos(force=false){
    renderOnlineCount();if(accessHistoryLoading)return;if(accessHistoryCache.length&&!force){renderHistoricoAcessos();return}
    accessHistoryLoading=true;setAccessStatus('Atualizando o histórico de acessos...');
    try{
      const desde=inicioDiaLocal(-6).toISOString();
      const rows=await supabaseRequest('/'+ACCESS_TABLE+'?select=created_at&created_at=gte.'+encodeURIComponent(desde)+'&order=created_at.desc&limit=10000');
      accessHistoryCache=Array.isArray(rows)?rows:[];renderHistoricoAcessos();setAccessStatus('Dados atualizados. O contador online é atualizado em tempo real.');
    }catch(e){console.warn(e);setAccessStatus('Execute o arquivo SQL da Parte 3 no Supabase para ativar o histórico.',true)}finally{accessHistoryLoading=false}
  }
  function iniciarModuloAcessos(){iniciarPresencaAcessos();registrarAcessoLoja();if(!isPublicClient())atualizarPainelAcessos()}

  function renderAll(){
    const main=document.querySelector('.main');
    const keepScroll=main?main.scrollTop:null;
    const pagePedidos=el('page-pedidos');
    const pagePedidosScroll=pagePedidos?pagePedidos.scrollTop:null;
    const pedidosBox=el('listaPedidos');
    const pedidosScroll=pedidosBox?pedidosBox.scrollTop:null;
    const kanbanScrolls={};
    document.querySelectorAll('#kanbanPedidos .kanban-col[data-status]').forEach(col=>{
      const sc=col.querySelector('.kanban-scroll');
      if(sc)kanbanScrolls[col.dataset.status]=sc.scrollTop;
    });
    const active=document.querySelector('.page.active')?.id||'';
    renderSelects();renderFavoritos();renderSeletorTamanhos('interno');renderSteps();renderResumo();renderEdicaoPedidoBanner();renderListas();renderReservados();renderDashboard();renderConfig();renderTamanhosPrato();renderInsights();renderRelatorios();renderOnlineReady();renderPortalCliente();renderOperacao();renderCupons();renderOnlineCount();renderListaPratosFeitos();renderAssistentePorcoes();aplicarPerfilAcesso();
    if(keepScroll!==null){
      const restore=()=>{
        if(active==='page-pedidos'){
          if(main)main.scrollTop=keepScroll;
          if(pagePedidos&&pagePedidosScroll!==null)pagePedidos.scrollTop=pagePedidosScroll;
          if(pedidosBox&&pedidosScroll!==null)pedidosBox.scrollTop=pedidosScroll;
          document.querySelectorAll('#kanbanPedidos .kanban-col[data-status]').forEach(col=>{
            const sc=col.querySelector('.kanban-scroll');
            const y=kanbanScrolls[col.dataset.status];
            if(sc&&typeof y==='number')sc.scrollTop=y;
          });
        }
      };
  ...
  function init(){try{const q=new URLSearchParams(location.search);const hash=String(location.hash||'').toLowerCase();const pediuAdmin=q.has('admin')||hash.includes('admin');const pediuLoja=q.has('loja')||hash.includes('loja');const aberturaOfflineAdmin=location.protocol==='file:'&&!pediuAdmin&&!pediuLoja;if(aberturaOfflineAdmin){history.replaceState(null,'',location.pathname+location.search+'#admin');}const modoAdmin=pediuAdmin||aberturaOfflineAdmin;if(modoAdmin)document.body.classList.add('admin-mode');else document.body.classList.add('public-client');}catch(e){document.body.classList.add('public-client')}if(document.body.classList.contains('admin-mode')&&!sessionStorage.getItem(ADMIN_SESSION_KEY)){mostrarLoginAdmin();document.body.classList.remove('booting');document.body.classList.add('boot-ready');return;}if(document.body.classList.contains('admin-mode')&&!sessionStorage.getItem(ADMIN_ROLE_KEY))sessionStorage.setItem(ADMIN_ROLE_KEY,'admin');document.querySelectorAll('button:not([type])').forEach(b=>b.setAttribute('type','button'));document.addEventListener('submit',ev=>ev.preventDefault());document.querySelectorAll('[data-toggle-password]').forEach(btn=>btn.addEventListener('click',()=>togglePasswordInput(btn.dataset.togglePassword,btn)));document.querySelectorAll('.nav button').forEach(b=>b.addEventListener('click',ev=>{ev.preventDefault();page(b.dataset.page)}));document.querySelectorAll('.mobile-op-nav button').forEach(b=>b.addEventListener('click',ev=>{ev.preventDefault();page(b.dataset.opPage)}));['clientePedido','tipoPedido','atendimentoPedido','reservaDataPedido','reservaHorarioPedido','pagamentoPedido','obsPedido','pedidoCep','pedidoRua','pedidoNumeroEndereco','pedidoComplemento','enderecoPedido','pedidoBairro','bairroPedido','pedidoCidade','pedidoUf'].forEach(id=>{if(el(id))el(id).addEventListener('input',renderResumo)});if(el('portalPagamento'))el('portalPagamento').addEventListener('change',()=>alternarCamposTroco('portal'));if(el('pagamentoPedido'))el('pagamentoPedido').addEventListener('change',()=>alternarCamposTroco('pedido'));['portalRua','portalNumero','portalBairro','portalCidade','portalUf'].forEach(id=>{if(el(id))el(id).addEventListener('input',()=>agendarCalculoFrete('portal'))});['pedidoRua','pedidoNumeroEndereco','pedidoBairro','pedidoCidade','pedidoUf'].forEach(id=>{if(el(id))el(id).addEventListener('input',()=>agendarCalculoFrete('pedido'))});
    ['cfgNome','cfgSlogan','cfgCnpj','cfgEmail','cfgEnderecoEmpresa','cfgInstagram','cfgPix','cfgMensagemPagamento','cfgWhatsapp','cfgTaxa','cfgRaioEntrega','cfgOrigemCep','cfgOrigemNumero','cfgEnderecoSaida','cfgTempoPreparo','cfgGratis','cfgProteinasGratis','cfgFinalizacoesGratis','cfgMetaDiaria','cfgNomeInterno','cfgAdminSenha','cfgOperacaoSenha','cfgModoOperacao'].forEach(id=>{const x=el(id);if(x){['focus','input','change'].forEach(ev=>x.addEventListener(ev,()=>{configEditing=true;configLastUserInput=Date.now();}));}});
