const App=(()=>{
  ...
  function criarPedidoPortal(){
    ...
    el('portalWhatsapp').innerHTML=`<strong>Pedido #${p.numero} criado como Aguardando confirmação.</strong><br><br><div class="whatsapp-actions"><a class="primary inline-link" href="${whats}" target="_blank">Abrir WhatsApp da loja</a><button class="secondary" onclick="App.copiarMensagemPortal()">Copiar mensagem</button></div><pre class="whatsapp-preview">${msg.replace(/</g,'&lt;')}</pre><small>O pedido já aparece na aba Pedidos. Confirme o pagamento e envie para preparo.</small>`;
    portalPedido={itens:{},qtd:{}};portalPratos=[];renderPortalCliente();toast('Pedido externo criado')
  }
  ...
  function init(){document.querySelectorAll('.nav button').forEach(b=>b.addEventListener('click',()=>page(b.dataset.page)));['clientePedido','tipoPedido','pagamentoPedido','obsPedido','enderecoPedido','bairroPedido'].forEach(id=>{if(el(id))el(id).addEventListener('input',renderResumo)});
