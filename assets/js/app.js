const App=(()=>{
  ...
  function renderInsights(){
    ...
    const consumo={};
    pedidosHoje.forEach(p=>(p.pratos||[{itens:p.itens||[]}]).forEach(pr=>(pr.itens||[]).forEach(i=>{const cat=i.categoria||'Outros';if(porCat[cat])porCat[cat][i.nome]=(porCat[cat][i.nome]||0)+Math.max(1,Number(i.porcoes||1));consumo[i.nome]=consumo[i.nome]||{qtd:0,unidade:i.unidade||'un',categoria:cat};consumo[i.nome].qtd+=Number(i.qtdUsada||i.qtd||1)})));
  ...
  function mensagemWhatsPedido(p,itens,total){
    ...
    pratosMsg.forEach((pr,idx)=>{
      ...
      ['Massa','Molho','Proteína','Complemento','Finalização','Bebida','Sobremesa','Item'].forEach(cat=>{
        if(porCat[cat]?.size){
          const arr=[...porCat[cat].entries()].map(([nome,q])=>q>1?`${q}x ${nome}`:nome);
